...
for ang = -pi:0.3:pi
    ...
    s = orProblemSendCommand(['iktest robot ' probs.robotname ' matrix ' sprintf('%f ',T(1:12))],probs.ikfast)
    if( isempty(s) )
        % empty so continue
        continue;
    end
    ...
    % check the IK of the destination
    s = orProblemSendCommand(['iktest robot ' probs.robotname ' matrix ' sprintf('%f ',Tnewhand(1:12))],probs.ikfast)
    if( isempty(s) )       
        % empty so continue
        'Tnewhand invalid'
        continue;
    ...
    % check the IK of the destination
    s = orProblemSendCommand(['iktest robot ' probs.robotname ' matrix ' sprintf('%f ',Tnewhand2(1:12))],probs.ikfast)
    if( isempty(s) )
        % empty so continue
        'Tnewhand2 invalid'
        continue;
