...
from openravepy import IkParameterizationType, IkParameterization, RaveCreateKinBody, raveLogInfo, raveLogWarn, \
    IkFilterOptions
...
def main(env, options):
    ...
    with env:
        ...
        for ikmodel in ikmodels:
            ...
            picker.InitFromBoxes(
                array([[0.05, 0, 0, 0.05, 0.01, 0.01], [0, 0.05, 0, 0.01, 0.05, 0.01], [0, 0, 0.05, 0.01, 0.01, 0.05]]),
                True)
...
def run(args=None):
    ...
    parser.add_option('--iktype', action='store', type='string', dest='iktype', default=None,
                      help='The ik type to build the solver current types are: %s' % (
                          ', '.join(iktype.name for iktype in list(IkParameterization.Type.values.values()))))
