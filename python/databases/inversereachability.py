...
from ..openravepy_int import RaveFindDatabaseFile, RaveCreateRobot, IkParameterization, rotationMatrixFromAxisAngle, \
    poseFromMatrix, matrixFromPose, matrixFromQuat, matrixFromAxisAngle, poseMult, quatFromAxisAngle, IkFilterOptions
...
class InverseReachabilityModel(DatabaseGenerator):
    ...
    def preprocess(self):
        ...
        self.equivalenceoffset = array(
            [self.classnormalizationconst(e[1] + samplingbandwidth) for e in self.equivalenceclasses])

    def save(self):
        DatabaseGenerator.save(self, (
        self.equivalenceclasses, self.rotweight, self.xyzdelta, self.quatdelta, self.jointvalues))
    ...
    def getdofindices(manip):
        ...
        dofindices = [arange(joint.GetDOFIndex(), joint.GetDOFIndex() + joint.GetDOF()) for joint in joints if
                      joint.GetDOFIndex() >= 0 and not joint.IsStatic()]
    ...
    def generate(self, heightthresh=None, quatthresh=None, Nminimum=None):
        ...
        try:
            ...
            quatrolls = array(
                [quatFromAxisAngle(array((0, 0, 1)), roll) for roll in arange(0, 2 * pi, quatthresh * 0.5)])
            self.equivalenceclasses = []
            while len(basetrans) > 0:
                searchtrans = c_[basetrans[:, 0:4], basetrans[:, 6:7]]
                kdtree = kinematicreachability.ReachabilityModel.QuaternionKDTree(searchtrans, 1.0 / self.rotweight)
                querypoints = c_[
                    quatArrayTMult(quatrolls, searchtrans[0][0:4]), tile(searchtrans[0][4:], (len(quatrolls), 1))]
                foundindices = zeros(len(searchtrans), bool)
                for querypoint in querypoints:
                    k = min(len(searchtrans), 1000)
                    neighs, dists, kball = kdtree.kFRSearchArray(reshape(querypoint, (1, 5)), quateucdist2, k,
                                                                 quatthresh * 0.01)
                    if k < kball:
                        neighs, dists, kball = kdtree.kFRSearchArray(reshape(querypoint, (1, 5)), quateucdist2, kball,
                                                                     quatthresh * 0.01)
                ...
                normalizedqarray[flatnonzero(
                    sum((normalizedqarray + identityquat) ** 2, 1) < sum((normalizedqarray - identityquat) ** 2, 1)),
                0:4] *= -1
                q0 = sum(normalizedqarray, axis=0)
                q0 /= sqrt(sum(q0 ** 2))
                if len(normalizedqarray) >= Nminimum:
                    qmean, success = leastsq(lambda q: quatArrayTDist(q / sqrt(sum(q ** 2)), normalizedqarray),
                                             normalizedqarray[0], maxfev=10000)
                ...
                equivalenttransinv = -c_[
                    czangles * equivalenttrans[:, 4] + szangles * equivalenttrans[:, 5], -szangles * equivalenttrans[:,
                                                                                                     4] + czangles * equivalenttrans[
                                                                                                                     :,
                                                                                                                     5]]
                ...
                log.info('new equivalence class outliers: %d/%d, left over trans: %d',
                         self.testEquivalenceClass(equivalenceclass) * len(zangles), len(zangles), len(basetrans))
        finally:
            statesaver.Release()
    ...
    def getEquivalenceClass(self, Tgrasp):
        ...
        logll = quatArrayTDist(qnormalized[0], self.equivalencemeans[:, 0:4]) ** 2 * self.equivalenceweights[:, 0] + (
                    posetarget[6] - self.equivalencemeans[:, 4]) ** 2 * self.equivalenceweights[:,
                                                                        1] + self.equivalenceoffset
    ...
    def computeBaseDistribution(self, Tgrasp, logllthresh=2.0, zaxis=None):
        ...
        points = c_[equivalenceclass[2][:, 0] + znormangle + zbaseangle, dot(equivalenceclass[2][:, 1:3],
                                                                             transpose(Trot[0:2, 0:2])) + tile(
            Trot[0:2, 2], (len(equivalenceclass[2]), 1))]
        ...
        def gaussiankerneldensity(poses):
            ...
            for i in range(p.shape[0]):
                ...
                if len(inds) > 0:
                    probs[i] = dot(weights[inds],
                                   numpy.exp(dot((points[inds, :] - tile(p[i, :], (len(inds), 1))) ** 2, ibandwidth)))
            return probs

        def gaussiankernelsampler(N=1, weight=1.0):
            """samples the distribution and returns a transform as a pose"""
            samples = random.normal(array([points[bisect.bisect(cumweights, random.rand()), :] for i in range(N)]),
                                    bandwidth * weight)
            samples[:, 0] *= 0.5 * irotweight
            return poseMultArrayT(poserobot, c_[
                cos(samples[:, 0]), zeros((N, 2)), sin(samples[:, 0]), samples[:, 1:3], tile(Tbase[2, 3],
                                                                                             N)]), self.necessaryjointstate()
    ...
    def computeAggregateBaseDistribution(self, Tgrasps, logllthresh=2.0, zaxis=None):
        ...
        for Tgrasp, graspindex in Tgrasps:
            ...
            qnormalized, znormangle = normalizeZRotation(reshape(posetarget[0:4], (1, 4)))
            # find the closest cluster
            logll = quatArrayTDist(qnormalized[0], self.equivalencemeans[:, 0:4]) ** 2 * self.equivalenceweights[:,
                                                                                         0] + (
                                posetarget[6] - self.equivalencemeans[:, 4]) ** 2 * self.equivalenceweights[:,
                                                                                    1] + self.equivalenceoffset
            bestindex = argmax(logll)
            ...
            newpoints = c_[equivalenceclass[2][:, 0] + znormangle, dot(equivalenceclass[2][:, 1:3], transpose(
                rotationMatrixFromAxisAngle([0, 0, 1], znormangle)[0:2, 0:2])) + tile(posetarget[4:6],
                                                                                      (len(equivalenceclass[2]), 1))]
            points = r_[points, newpoints]
            weights = r_[weights, equivalenceclass[2][:, 3] * normalizationconst]

        if len(points) == 0:
            log.info('inversereachability: could not find base distribution, logllthresh too high? logll=%f',
                     highestlogll)
            return None, None, None

        # transform points by the base pose
        points[:, 0] += zbaseangle
        points[:, 1:3] = dot(points[:, 1:3],
                             transpose(rotationMatrixFromAxisAngle([0, 0, 1], zbaseangle)[0:2, 0:2])) + tile(
            posebase[4:6], (len(points), 1))
        ...
        def gaussiankernelsampler(N=1, weight=1.0):
            ...
            return poseMultArrayT(poserobot, c_[
                cos(samples[:, 0]), zeros((N, 2)), sin(samples[:, 0]), samples[:, 1:3], tile(Tbase[2, 3],
                                                                                             N)]), sampledgraspindices, self.necessaryjointstate()
    ...
    def sampleBaseDistributionIterator(self, Tgrasps, logllthresh=2.0, weight=1.0, Nprematuresamples=1, zaxis=None):
        ...
        for Tgrasp, graspindex in Tgrasps:
            ...
            Ttargetrot = c_[rotationMatrixFromAxisAngle([0, 0, 1], znormangle)[0:2, 0:2], posetarget[4:6]]
            Trot = dot(Tbaserot, r_[Ttargetrot, [[0, 0, 1]]])
            newpoints = c_[equivalenceclass[2][:, 0] + znormangle + zbaseangle, dot(equivalenceclass[2][:, 1:3],
                                                                                    transpose(Trot[0:2, 0:2])) + tile(
                Trot[0:2, 2], (len(equivalenceclass[2]), 1))]
            ...
            if Nprematuresamples > 0:
                ...
                for i in range(Nprematuresamples):
                    ...
                    yield poseMult(poserobot, r_[cos(sample[0]), 0, 0, sin(sample[0]), sample[1:3], Tbase[
                        2, 3]]), graspindex, self.necessaryjointstate()
            graspindices.append(graspindex)
            graspindexoffsets.append(len(points))
            points = r_[points, newpoints]
        ...
        while True:
            ...
            sample[0] *= 0.5 * irotweight
            yield poseMult(poserobot, r_[cos(sample[0]), 0, 0, sin(sample[0]), sample[1:3], Tbase[
                2, 3]]), sampledgraspindex, self.necessaryjointstate()
    ...
    def randomBaseDistributionIterator(self, Tgrasps, Nprematuresamples=1, bounds=None, **kwargs):
        ...
        for Tgrasp, graspindex in Tgrasps:
            for i in range(Nprematuresamples):
                ...
                yield r_[cos(angle), 0, 0, sin(angle), Tgrasp[0:2, 3] + (bounds[0, 1:3] + r[1:3] * dbounds[1:3]),
                         Trobot[2, 3]], graspindex, self.necessaryjointstate()
            grasps.append((Tgrasp, graspindex))
        while True:
            Tgrasp, graspindex = grasps[random.randint(len(grasps))]
            r = random.rand(3)
            angle = 0.5 * (bounds[0, 0] + r[0] * dbounds[0])
            yield r_[cos(angle), 0, 0, sin(angle), Tgrasp[0:2, 3] + (bounds[0, 1:3] + r[1:3] * dbounds[1:3]), Trobot[
                2, 3]], graspindex, self.necessaryjointstate()
    ...
    def showBaseDistribution(env, densityfn, bounds, zoffset=0, thresh=1.0, maxprob=None, marginalizeangle=True):
        ...
        A, Y, X = mgrid[bounds[0, 0]:bounds[1, 0]:discretization[0], bounds[0, 2]:bounds[1, 2]:discretization[2],
                  bounds[0, 1]:bounds[1, 1]:discretization[1]]
        N = prod(A.shape)
    ...
    def show(self, options=None):
        ...
        while True:
            ...
            with self.robot:
                while True:
                    self.robot.SetDOFValues(random.rand() * (upper - lower) + lower,
                                            self.manip.GetArmIndices())  # set random values
