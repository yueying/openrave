...
from ..openravepy_int import RaveCreateTrajectory, IkParameterization, IkParameterizationType, IkFilterOptions, \
    RaveFindDatabaseFile, \
    DOFAffine, CollisionReport, RaveCreateCollisionChecker, quatRotateDirection, rotationMatrixFromQuat, Ray, \
    poseFromMatrix
...
class GraspingModel(DatabaseGenerator):
    ...
    def __init__(self, robot, target, maxvelmult=None, graspparametersdof=None):
        ...
        graspdof = {'igraspdir': 3, 'igrasppos': 3, 'igrasproll': 1, 'igraspstandoff': 1,
                    'igrasppreshape': len(self.manip.GetGripperIndices()), 'igrasptrans': 12,
                    'imanipulatordirection': 3, 'forceclosure': 1, 'grasptrans_nocol': 12, 'performance': 1,
                    'vintersectplane': 4, 'igraspfinalfingers': len(self.manip.GetGripperIndices()),
                    'ichuckingdirection': len(self.manip.GetGripperIndices()), 'graspikparam_nocol': 8,
                    'approachdirectionmanip': 3, 'igrasptranslationoffset': 3}
    ...
    def load(self, filename=None):
        ...
        try:
            ...
            self.basemanip = interfaces.BaseManipulation(self.robot, maxvelmult=self.maxvelmult)
            self.grasper = interfaces.Grasper(self.robot, friction,
                                              avoidlinks=[self.robot.GetLink(name) for name in linknames],
    ...
    def save(self):
        DatabaseGenerator.save(self, (
        self.grasps, self.graspindices, self.grasper.friction, [link.GetName() for link in self.grasper.avoidlinks],
        self.grasper.plannername, self.translationstepmult, self.finestep, self.graspsetname))

    def getfilename(self, read=False):
        return RaveFindDatabaseFile(os.path.join('robot.' + self.robot.GetKinematicsGeometryHash(),
                                                 'graspset.' + self.manip.GetStructureHash() + '.' + self.target.GetKinematicsGeometryHash() + '.pp'),
                                    read)
            ...
            for i, joint in enumerate(self.robot.GetJoints()):
                ...
                if childlink is not None:
                    ...
                    if joint.IsPrismatic(0):
                        ...
                        self.jointmaxlengths[i] = sqrt(
                            numpy.max(sum(vertices ** 2, 1) - dot(vertices, joint.GetAxis(0)) ** 2)) if len(
                            vertices) > 0 else 0
    ...
    def autogenerateparams(self, options=None):
        ...
        if options is not None:
            ...
            if hasattr(options, 'manipulatordirections') and options.manipulatordirections is not None:
                manipulatordirections = array(
                    [array([float(s) for s in md.split()]) for md in options.manipulatordirections])
            if hasattr(options, 'boxdelta') and options.boxdelta is not None:
                approachrays = self.computeBoxApproachRays(delta=options.boxdelta,
                ...
                approachrays = self.computeSphereApproachRays(delta=options.spheredelta,
                                                              normalanglerange=options.normalanglerange,
                                                              directiondelta=options.directiondelta)
        ...
        if approachrays is None:
            approachrays = self.computeBoxApproachRays(delta=0.02, normalanglerange=normalanglerange,
    ...
    def generatepcg(self, preshapes=None, standoffs=None, rolls=None, approachrays=None, graspingnoise=None,
                    forceclosure=True, forceclosurethreshold=1e-9, checkgraspfn=None, manipulatordirections=None,
                    translationstepmult=None, finestep=None, friction=None, avoidlinks=None, plannername=None,
                    boxdelta=None, spheredelta=None, normalanglerange=None):
        ...
        gapproachrays = c_[dot(approachrays[:, 0:3], transpose(Ttarget[0:3, 0:3])) + tile(Ttarget[0:3, 3], (N, 1)), dot(
        ...
        def consumer(approachray, roll, preshape, standoff, manipulatordirection):
            ...
            try:
                contacts, finalconfig, mindist, volume = self.testGrasp(grasp=grasp, graspingnoise=graspingnoise,
                                                                        translate=True, forceclosure=forceclosure,
            ...
            with self.robot:
                ...
                grasp[self.graspindices.get('graspikparam_nocol')] = r_[
                    int(IkParameterizationType.Transform6D), poseFromMatrix(Tlocalgrasp_nocol)]
                ...
                if not forceclosure or mindist >= forceclosurethreshold:
                    grasp[self.graspindices.get('performance')] = self._ComputeGraspPerformance(grasp,
                                                                                                ...
                                                                                                translate=True,
                                                                                                forceclosure=False)
                    if checkgraspfn is None or checkgraspfn(contacts, finalconfig, grasp,
                                                            {'mindist': mindist, 'volume': volume}):
    ...
    def _generateThreaded(self, preshapes=None, standoffs=None, rolls=None, approachrays=None, graspingnoise=None,
                          forceclosure=True, forceclosurethreshold=1e-9, checkgraspfn=None, manipulatordirections=None,
                          translationstepmult=None, finestep=None, friction=None, avoidlinks=None, plannername=None):
        """Generates a grasp set by searching space and evaluating contact points.
        ...
        with self.robot:  # lock the environment and save the robot state
            ...
            self.robot.SetActiveDOFs(self.manip.GetGripperIndices(),
                                     DOFAffine.X | DOFAffine.Y | DOFAffine.Z if translate else 0)
            approachrays[:, 3:6] = -approachrays[:, 3:6]
            self.nextid, self.resultgrasps = self.grasper.GraspThreaded(approachrays=approachrays, rolls=rolls,
                                                                        standoffs=standoffs, preshapes=preshapes,
            ...
            for resultgrasp in self.resultgrasps:
                ...
                with self.robot:
                    ...
                    grasp[self.graspindices.get('igrasptrans')] = reshape(transpose(Tlocalgrasp[0:3, 0:4]), 12)
                    grasp[self.graspindices.get('grasptrans_nocol')] = reshape(transpose(Tlocalgrasp_nocol[0:3, 0:4]),
                                                                               12)
                    grasp[self.graspindices.get('graspikparam_nocol')] = r_[
                        int(IkParameterizationType.Transform6D), poseFromMatrix(Tlocalgrasp_nocol)]
                    ...
                    if not forceclosurethreshold or mindist >= forceclosurethreshold:
                        grasp[self.graspindices.get('performance')] = self._ComputeGraspPerformance(grasp)
                        if checkgraspfn is None or checkgraspfn(contacts, [Tfinal, finalshape], grasp,
                                                                {'mindist': mindist, 'volume': volume}):
                            self.grasps.append(grasp)
        ...
        with self.robot.CreateRobotStateSaver():
            ...
            with self.GripperVisibility(self.manip):
                ...
                for i, grasp in enumerate(grasps):
                    ...
                    try:
                        with self.env:
                            contacts, finalconfig, mindist, volume = self.testGrasp(grasp=grasp, translate=True,
    ...
    def testGrasp(self, graspingnoise=None, Ngraspingtries=20, forceclosurethreshold=1e-9, **kwargs):
        ...
        if mindist >= forceclosurethreshold and graspingnoise > 0:
            ...
            if mindist > 0:
                ...
                maxjointvaluestd = max([sum([jointvaluesstd[i] for i in range(len(self.robot.GetJoints())) if
                                             self.robot.DoesAffect(i, link.GetIndex())]) for link in
                                        self.robot.GetLinks()])
    ...
    def runGrasp(self, grasp, graspingnoise=None, translate=True, forceclosure=False, translationstepmult=None,
                 finestep=None):
        ...
        with self.robot:  # lock the environment and save the robot state
            ...
            return self.grasper.Grasp(direction=grasp[self.graspindices.get('igraspdir')],
                                      ...
                                      target=self.target, graspingnoise=graspingnoise, forceclosure=forceclosure,
                                      execute=False, outputfinal=True, translationstepmult=translationstepmult,
                                      finestep=finestep, vintersectplane=vintersectplane,
    ...
    def runGraspFromTrans(self, grasp, finestep=None):
        ...
        with self.robot:
            ...
            self.robot.SetTransform(
                dot(self.getGlobalGraspTransform(grasp), dot(linalg.inv(Tmanip), self.robot.GetTransform())))
            self.robot.SetActiveDOFs(self.manip.GetGripperIndices())
            ...
            if 'ichuckingdirection' in self.graspindices:
                chuckingdirection = grasp[self.graspindices['ichuckingdirection']]
            return self.grasper.Grasp(transformrobot=False, target=self.target, onlycontacttarget=True,
                                      forceclosure=False, execute=False, outputfinal=True,
                                      translationstepmult=self.translationstepmult, finestep=finestep,
                                      chuckingdirection=chuckingdirection)

    def GetLocalGraspTransform(self, grasp, collisionfree=False):
        Tlocalgrasp = eye(4)
        Tlocalgrasp[0:3, 0:4] = transpose(
            reshape(grasp[self.graspindices['grasptrans_nocol' if collisionfree else 'igrasptrans']], (4, 3)))
        return Tlocalgrasp
    ...
    def moveToPreshape(self, grasp, execute=True, outputtraj=None, outputtrajobj=None):
        ...
        with self.robot:
            ...
            trajdata.append(self.basemanip.MoveUnsyncJoints(jointvalues=grasp[self.graspindices['igrasppreshape']],
                                                            jointinds=self.manip.GetGripperIndices(), execute=execute,
                                                            outputtraj=outputtraj, outputtrajobj=outputtrajobj))
        ...
        with self.robot:
            if not execute:
                ...
                values = traj.GetConfigurationSpecification().ExtractJointValues(GetWaypoint(-1), self.robot,
                                                                                 self.robot.GetActiveDOFIndices(), 0)
                self.robot.SetActiveDOFValues(values)
            self.robot.SetActiveDOFs(self.manip.GetGripperIndices())
            trajdata.append(
                self.basemanip.MoveActiveJoints(goal=grasp[self.graspindices['igrasppreshape']], execute=execute,
                                                outputtraj=outputtraj, outputtrajobj=outputtrajobj))
        while execute and not self.robot.GetController().IsDone():  # busy wait
            time.sleep(0.01)
        return trajdata

    def computeValidGrasps(self, startindex=0, checkcollision=True, checkik=True, checkgrasper=True, backupdist=0.0,
                           returnnum=inf):
                ...
                if checkik:
                    if self.manip.GetIkSolver().Supports(IkParameterization.Type.Transform6D):
                        ...
                        ikparam = IkParameterization(
                            Ray(Tglobalgrasp[0:3, 3], dot(Tglobalgrasp[0:3, 0:3], self.manip.GetLocalToolDirection())),
                        ...
                        with self.robot.CreateRobotStateSaver():
                            ...
                            grasp[self.graspindices.get('graspikparam_nocol')] = r_[
                                int(IkParameterizationType.Transform6D), poseFromMatrix(Tglobalgrasp)]
                    else:
    ...
    def validGraspIterator(self, startindex=0, checkcollision=True, checkik=True, checkgrasper=True, backupdist=0.0,
                           randomgrasps=False, returnfinal=False):
        ...
        if randomgrasps:
            ...
            order = list(range(startindex, len(self.grasps)))
        for i in order:
            grasp = self.grasps[i]
        ...
        try:
            with self.env:
                ...
                if normalanglerange > 0:
                    ...
                    dirs = array([dir for dir in dirs if
                                  arccos(dir[2]) <= normalanglerange])  # find all dirs within normalanglerange
                    if len(dirs) == 0:
                        dirs = array([[0, 0, 1]])
                    ...
                    for approachray in approachrays:
                        ...
                        newapproachrays = r_[
                            newapproachrays, c_[tile(approachray[0:3], (len(dirs), 1)), dot(dirs, transpose(R))]]
    ...
    def computeBoxApproachRays(self, delta=0.02, normalanglerange=0, directiondelta=0.4):
        return self._computeBoxApproachRays(self.env, self.target, delta=delta, normalanglerange=normalanglerange,
                                            directiondelta=directiondelta)
    ...
    def drawContacts(self, contacts, conelength=0.03, transparency=0.5):
        ...
        conepoints = r_[[[0, 0, 0]], conelength * c_[
            self.grasper.friction * cos(angs), self.grasper.friction * sin(angs), ones(len(angs))]]
        triinds = array(
            c_[zeros(len(angs)), list(range(2, 1 + len(angs))) + [1], list(range(1, 1 + len(angs)))].flatten(), int)
        allpoints = zeros((0, 3))
        for c in contacts:
    ...
    def CreateOptionParser():
        ...
        parser.add_option('--translationstepmult', action='store', type='float', dest='translationstepmult',
                          default=None,
        ...
        parser.add_option('--manipulatordirection', action='append', type='string', dest='manipulatordirections',
                          default=None,
