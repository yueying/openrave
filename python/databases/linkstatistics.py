...
from ..openravepy_int import RaveFindDatabaseFile, RaveDestroy, Environment
...
class LinkStatisticsModel(DatabaseGenerator):
    ...
    def getfilename(self, read=False):
        return RaveFindDatabaseFile(
            os.path.join('robot.' + self.robot.GetKinematicsGeometryHash(), 'linkstatistics.pp'), read)
        ...
        with self.env:
            ...
            if type == 0:
                ...
                if totalweight > 1e-7:
                    ...
                    log.debug('total weight (%s) for robot %s is too small, so do not normalize', totalweight,
                              self.robot.GetName())
    ...
    def _GetJointSpheresFromGrabbed(self, grabbedinfo):
        for testgrabbedinfo, testjointspheres in self.grabbedjointspheres:
            if len(testgrabbedinfo) == len(grabbedinfo):
                if all([(grabbedinfo[i]._grabbedname == testgrabbedinfo[i]._grabbedname and grabbedinfo[
                    i]._robotlinkname == testgrabbedinfo[i]._robotlinkname and sum(
                        abs(grabbedinfo[i]._trelative - testgrabbedinfo[i]._trelative)) <= 1e-7 and grabbedinfo[
                             i]._setRobotLinksToIgnore == testgrabbedinfo[i]._setRobotLinksToIgnore) for i in
                        range(len(grabbedinfo))]):
