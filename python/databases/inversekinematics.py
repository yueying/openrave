...
from ..openravepy_int import RaveCreateModule, RaveCreateIkSolver, IkParameterization, IkParameterizationType, \
    RaveFindDatabaseFile, RaveDestroy, Environment, openravepyCompilerVersion, IkFilterOptions, KinBody, \
...
class InverseKinematicsModel(DatabaseGenerator):
    ...
    class ArmVisibility:
        ...
        def __enter__(self):
            ...
            with self.robot.GetEnv():
                childlinks = self.robot.GetChain(self.manip.GetBase().GetIndex(),
                                                 self.manip.GetEndEffector().GetIndex(), returnjoints=False)
    ...
    def __init__(self, robot=None, iktype=None, forceikfast=False, freeindices=None, freejoints=None, manip=None,
        ...
        for link in robot.GetChain(manip.GetBase().GetIndex(), manip.GetEndEffector().GetIndex(), returnjoints=False)[
                    1:]:
        ...
        if self.freeindices is None:
            ...
            if not all([ifree in manip.GetArmIndices() for ifree in self.freeindices]):
                raise InverseKinematicsError('not all free indices %r are part of the manipulator indices %r' % (
    ...
    def has(self):
        return self.iksolver is not None and self.manip.GetIkSolver() is not None and self.manip.GetIkSolver().Supports(
            self.iktype) and self.iksolver.GetXMLId() == self.manip.GetIkSolver().GetXMLId()
    ...
    def save(self, filepermissions=None):
        ...
        try:
            ...
            with open(statsfilename, 'w') as f:
                pickle.dump((
                            self.getversion(), self.statistics, self.ikfeasibility, self.solveindices, self.freeindices,
                            self.freeinc), f)
    ...
    def load(self, freeinc=None, checkforloaded=True, *args, **kwargs):
        try:
            ...
            with open(filename, 'r') as f:
                modelversion, self.statistics, self.ikfeasibility, self.solveindices, self.freeindices, self.freeinc = pickle.load(
                    f)
            if modelversion != self.getversion():
                log.warn('version is wrong %s!=%s', modelversion, self.getversion())
                return checkforloaded and self.manip.GetIkSolver() is not None and self.manip.GetIkSolver().Supports(
                    self.iktype)  # might have ik already loaded
    ...
    def setrobot(self, freeinc=None):
        ...
        if self.iksolver is None:
            with self.env:
                ...
                iktype = self.ikfastproblem.SendCommand(
                    'AddIkLibrary %s %s' % (ikname.split()[1], self.getfilename(True)))
                if iktype is None:
                    if self.forceikfast:
                        return False

                    self.iksolver = RaveCreateIkSolver(self.env, self.manip.GetIkSolver().GetXMLId().split(' ', 1)[
                        0] + iksuffix) if self.manip.GetIkSolver() is not None else None
                else:
        ...
        if self.iksolver is not None and self.iksolver.Supports(self.iktype):
            ...
            if success and self.freeinc is not None:
                ...
                self.iksolver.SendCommand('SetDefaultIncrements %f 100 %f 10' % (
                freeincvalue, pi / 8))  # the default values are for all joints
                if len(self.freeindices) > 0 and self.freeinc is not None and len(self.freeinc) == len(
                        self.freeindices):
    ...
    def getDefaultFreeIncrements(self, freeincrot, freeinctrans):
        ...
        with self.env:
            ...
            armlength = 0
            orderedarmindices = [j for j in self.robot.GetDependencyOrderedJoints() if
                                 j.GetJointIndex() in self.manip.GetArmIndices()]
    ...
    def GetDefaultIndices(self, avoidPrismaticAsFree=False):
        ...
        if len(remainingindices) > dofexpected:
            ...
            for i in range(1, N - 1):
                if jointtypes[i - 1] == KinBody.JointType.Revolute and jointtypes[i] == KinBody.JointType.Revolute and \
                        jointtypes[i + 1] == KinBody.JointType.Revolute:
                    if intersectingaxes[i - 1, i] and intersectingaxes[i, i + 1] and intersectingaxes[i - 1, i + 1]:
                        ...
                        intersection = jointanchors[i] + jointaxes[i] * dot(jointaxes[i],
                                                                            jointanchors[i - 1] - jointanchors[i])
            ...
            for i in range(N - dofexpected):
                ...
                if self.iktype == IkParameterizationType.Transform6D:
                    ...
                    if abs(dot(jointaxes[-1], dot(self.manip.GetTransform()[0:3, 0:3],
                                                  self.manip.GetLocalToolDirection()))) > 0.99999 and linalg.norm(
                            cross(jointaxes[-1], dirfromanchor)) <= 1e-5:
    ...
    def getfilename(self, read=False):
        ...
        allfreeindices = None
        while True:
            basename = 'ikfast%s.%s.%s.' % (self.ikfast.__version__, self.iktype, platform.machine()) + '_'.join(
                str(ind) for ind in sorted(solveindices))
            if len(freeindices) > 0:
                basename += '_f' + '_'.join(str(ind) for ind in sorted(freeindices))
            filename = RaveFindDatabaseFile(
                os.path.join('kinematics.' + self.manip.GetInverseKinematicsStructureHash(self.iktype),
                             ccompiler.new_compiler().shared_object_filename(basename=basename)), read)
            ...
            if allfreeindices is None:
                if self.manip.GetArmDOF() > dofexpected:
                    allfreeindices = [f for f in self.ikfast.permutations(self.manip.GetArmIndices(),
                                                                          self.manip.GetArmDOF() - dofexpected)]
                else:
                    allfreeindices = []
    ...
    def getsourcefilename(self, read=False, outputlang='cpp'):
        ...
        if len(freeindices) > 0:
            basename += '_f' + '_'.join(str(ind) for ind in sorted(freeindices))
        basename += '.' + outputlang
        return RaveFindDatabaseFile(
            os.path.join('kinematics.' + self.manip.GetInverseKinematicsStructureHash(self.iktype), basename), read)
    ...
    def getstatsfilename(self, read=False):
        ...
        while True:
            ...
            for freeindicesstring, fi in freeindicesstrings:
                ...
                basename += '.pp'
                filename = RaveFindDatabaseFile(
                    os.path.join('kinematics.' + self.manip.GetInverseKinematicsStructureHash(self.iktype), basename),
                    read)
                if not read or len(filename) > 0 or self.freeindices is not None:
                    self.freeindices = fi
                    return filename
            ...
            index += 1
            dofexpected = IkParameterization.GetDOFFromType(self.iktype)
            allfreeindices = [f for f in self.ikfast.combinations(self.manip.GetArmIndices(),
                                                                  len(self.manip.GetArmIndices()) - dofexpected)]
            if index >= len(allfreeindices):
                break
            freeindices = allfreeindices[index]
            solveindices = [i for i in self.manip.GetArmIndices() if not i in freeindices]
    ...
    def autogenerate(self, options=None):
        ...
        if self.manip.GetKinematicsStructureHash() == 'f17f58ee53cc9d185c2634e721af7cd3':  # wam 4dof
            ...
            if iktype == IkParameterizationType.Translation3D or (
                    iktype == None and self.iktype == IkParameterizationType.Translation3D):
        ...
        self.generate(iktype=iktype, freejoints=freejoints, precision=precision, forceikbuild=forceikbuild,
                      outputlang=outputlang, ipython=ipython, ikfastmaxcasedepth=ikfastmaxcasedepth)
        ...
        for jointname in freejoints:
            if type(jointname) == int:
                ...
                if not dofindices[0] in self.manip.GetArmIndices():
                    raise LookupError("cannot find joint '%s(%d)' in solve joints: %s" % (
                    jointname, dofindices[0], self.manip.GetArmIndices()))
                freeindices.append(dofindices[0])
        print('getIndicesFromJointNames', freeindices, freejoints)
        return freeindices

    def generate(self, iktype=None, freejoints=None, freeinc=None, freeindices=None, precision=None, forceikbuild=True,
                 outputlang=None, avoidPrismaticAsFree=False, ipython=False, ikfastoptions=0, ikfastmaxcasedepth=3):
        """
        ...
        #             solvefn=solveFullIK_TranslationXYOrientation3D
            ...
            Tmanipraw = eye(
                4)  # newer ikfast versions don't compile with self.manip.GetLocalToolTransform() in order to re-use the same 6D IK for multiple local transforms
        ...
        if len(self.solveindices) > dofexpected:  # allow for 5DOF to solve for Transform6D
            raise InverseKinematicsError(
                'Manipulator %(manip)s (indices=%(manipindices)r) joint indices to solve for (%(solveindices)r) is not equal to number of expected joints (%(dofexpected)d) for IK type %(iktype)s. Perhaps the manipulator base link %(baselink)s or end link %(endlink)s are not correct.' % {
                    'manip': self.manip.GetName(), 'manipindices': list(self.manip.GetArmIndices()),
                    'solveindices': list(self.solveindices), 'dofexpected': dofexpected, 'iktype': self.iktype.name,
                    'baselink': self.manip.GetBase().GetName(), 'endlink': self.manip.GetEndEffector().GetName()})
        ...
        log.info(
            'Generating inverse kinematics for manip %s: %s %s, precision=%s, maxcasedepth=%d (this might take up to 10 min)',
            self.manip.GetName(), self.iktype, self.solveindices, precision, ikfastmaxcasedepth)
        ...
        if forceikbuild or not os.path.isfile(sourcefilename):
            ...
            solver = self.ikfast.IKFastSolver(kinbody=self.robot,
                                              kinematicshash=self.manip.GetInverseKinematicsStructureHash(self.iktype),
                                              precision=precision, checkpreemptfn=self._checkpreemptfn)
            ...
            if ipython:
                ...
                if IPython.__version__.startswith("0.10"):
                    ipshell = IPython.Shell.IPShellEmbed(argv='', banner='inversekinematics dropping into ipython',
                                                         exit_msg='Leaving Interpreter and continuing solver.')
            ...
            try:
                ...
                chaintree = solver.generateIkSolver(baselink=baselink, eelink=eelink, freeindices=self.freeindices,
                                                    solvefn=solvefn)
                self.ikfeasibility = None
                code = solver.writeIkSolver(chaintree, lang=outputlang)
                if len(code) == 0:
                    raise InverseKinematicsError(
                        'failed to generate ik solver for robot %s:%s' % (self.robot.GetName(), self.manip.GetName()))
                ...
                log.info('successfully generated c++ ik in %fs, file=%s', self.statistics['generationtime'],
                         sourcefilename)
        ...
        if self.ikfeasibility is None:
            ...
            if outputlang == 'cpp':
                ...
                platformsourcefilename = os.path.splitext(output_filename)[
                                             0] + '.cpp'  # needed in order to prevent interference with machines with different architectures
                ...
                try:
                    objectfiles = compiler.compile(sources=[platformsourcefilename],
                                                   ...
                                                   extra_postargs=compile_flags, output_dir=output_dir)
    ...
    def testik(self, iktests, jacobianthreshold=None):
        ...
        with self.robot:
            ...
            solutionresults = []
            index = 2
            numvalues = 1 + IkParameterization.GetNumberOfValuesFromType(
                self.iktype) + self.manip.GetIkSolver().GetNumFreeParameters()
            ...
            log.info('success rate: %f, wrong solutions: %f, no solutions: %f, missing solution: %f',
                     float(res[1]) / numtested, wrongrate, len(solutionresults[1]) / numtested,
                     len(solutionresults[2]) / numtested)
            ...
            with self.ArmVisibility(self.manip, 0.95):
                ...
                while True:
                    with self.env:
                        ...
                        self.robot.SetDOFValues(lower + random.rand(len(lower)) * (upper - lower),
                                                self.manip.GetArmIndices())
            ...
            try:
                ...
                if cname == 'msvc':
                    ...
                    if abs(compiler._MSVCCompiler__version - majorVersion + minorVersion) > 0.001:
                        log.warn(
                            'default compiler v %s the same version as openrave compiler v %f, look for a different compiler',
                            compiler._MSVCCompiler__version, majorVersion + minorVersion);
    ...
    def CreateOptionParser():
        ...
        parser.add_option('--iktype', action='store', type='string', dest='iktype', default=None,
                          help='The ik type to build the solver current types are: %s' % (', '.join(
                              iktype.name for iktype in list(IkParameterizationType.values.values()) if
                              not int(iktype) & IkParameterizationType.VelocityDataBit)))
            ...
            try:
                ...
                if options.manipname is None:
                    ...
                    for robot in env.GetRobots():
                        ...
                        if manip is not None:
                            break
                ikmodel = InverseKinematicsModel(robot, iktype=model.iktype, forceikfast=True,
                                                 freeindices=model.freeindices, manip=manip)
                ...
                if options.iktests is not None:
                    ...
                    log.info('running time mean: %fs, median: %fs, min: %fs, max: %fs', mean(results), median(results),
