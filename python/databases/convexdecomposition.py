...
from ..openravepy_int import KinBody, RaveFindDatabaseFile, RaveDestroy, Environment, TriMesh, RaveCreateModule, \
    GeometryType, RaveGetDefaultViewerType
...
class ConvexDecompositionModel(DatabaseGenerator):
            ...
            for ilink, glink in glinkgeometry.items():
                ...
                for ig, glinkhulls in glink.items():
                    ...
                    for j, ghull in ghulls.items():
                        if 'vertices' in ghull and len(ghull['vertices'].shape) == 2 and 'indices' in ghull and len(
                                ghull['indices'].shape) == 2 and 'planes' in ghull and len(ghull['planes'].shape) == 2:
    ...
    def getfilename(self, read=False):
        ...
        return RaveFindDatabaseFile(os.path.join('robot.' + self.robot.GetKinematicsGeometryHash(),
                                                 'convexdecomposition_%.3f.pp' % self._padding), read)

    def autogenerate(self, options=None):
        if options is not None:
            self.generate(padding=options.padding, skinWidth=options.skinWidth,
                          decompositionDepth=options.decompositionDepth, maxHullVertices=options.maxHullVertices,
                          concavityThresholdPercent=options.concavityThresholdPercent,
                          mergeThresholdPercent=options.mergeThresholdPercent,
                          volumeSplitThresholdPercent=options.volumeSplitThresholdPercent,
                          useInitialIslandGeneration=options.useInitialIslandGeneration,
                          useIslandGeneration=options.useIslandGeneration,
                          convexHullLinks=options.convexHullLinks.split(','))
        else:
            self.generate()
        self.save()
                ...
                for ig, geom in enumerate(geometries):
                    if geom.GetType() == KinBody.Link.GeomType.Trimesh or padding > 0:
                        ...
                        if len(trimesh.indices) == 0:
                            ...
                            trimesh = geom.GetCollisionMesh()
                        if link.GetName() in convexHullLinks or (minTriangleConvexHullThresh is not None and len(
                                trimesh.indices) > minTriangleConvexHullThresh):
                            log.info('computing hull for link %d/%d geom %d/%d: vertices=%d, indices=%d', il,
                                     len(links), ig, len(geometries), len(trimesh.vertices), len(trimesh.indices))
                            orghulls = [self.ComputePaddedConvexHullFromTriMesh(trimesh, padding)]
                        else:
                            log.info('computing decomposition for link %d/%d geom %d/%d type %s', il, len(links), ig,
                                     len(geometries), geom.GetType())
    ...
    def ComputePaddedConvexDecompositionFromTriMesh(self, trimesh, padding=0.0):
        if len(trimesh.indices) > 0:
            orghulls = convexdecompositionpy.computeConvexDecomposition(trimesh.vertices, trimesh.indices,
                                                                        **self.convexparams)
        else:
            orghulls = []
        if len(orghulls) > 0:
    ...
    def ComputePaddedConvexHullFromTriMesh(self, trimesh, padding=0.0):
        ...
        if len(trimesh.indices) > 0:
            ...
            cmd = StringIO()
            cmd.write(
                'ConvexHull returnplanes 0 returnfaces 0 returntriangles 1 points %d %d ' % (len(trimesh.vertices), 3))
            ...
            # offset += 4*numplanes
            numtriangles = int(res[offset]);
            offset += 1
            return self.PadMesh(trimesh.vertices,
                                reshape(array(res[offset:(offset + 3 * numtriangles)], int32), (numtriangles, 3)),
                                padding)

    @staticmethod
    def PadMesh(vertices, indices, padding, mergeDuplicated=True, rtol=1e-05, atol=1e-08,
                setNormalsAwayFromCenter=False):
        ...
        for i, edge in enumerate(originaledges):
            ...
            if len(inds) > 0:
                ...
                if edge[4] == 0:
                    newindices = r_[newindices, [[edge[3], cedge[2], verticesofinterest[edge[0]]],
                                                 [cedge[3], edge[2], verticesofinterest[edge[1]]]]]
                else:
                    newindices = r_[newindices, [[cedge[3], edge[2], verticesofinterest[edge[0]]],
                                                 [edge[3], cedge[2], verticesofinterest[edge[1]]]]]
        if offset > 0:
    ...
    def ComputeHullPlanes(hull, thresh=0.99999):
        ...
        normals = cross(v1, v2, 1) / sqrt(
            sum(v1 ** 2) * sum(v2 ** 2))  # need to multiply by lengths because of floating-point precision problems
                ...
                for ig, geom in enumerate(link.GetGeometries()):
                    ...
                    if len(cdhulls) > 0:
                        for ihull, hull in enumerate(cdhulls[0]):
                            insideinds = numpy.all(dot(localpoints, transpose(hull[2][:, 0:3])) + tile(hull[2][:, 3], (
                            len(localpoints), 1)) <= 0, 1)
                        ...
                        insideinds = numpy.less_equal(abs(localpoints[:, 1]),
                                                      0.5 * geom.GetCylinderHeight()) and numpy.less_equal(
                            localpoint[:, 0] ** 2 + localpoint[:2] ** 2, geom.GetCylinderRadius() ** 2)
                    else:
                        continue
                    inside[leftinds[flatnonzero(insideinds)]] = True
                    outsideinds = flatnonzero(insideinds == 0)
            ...
            for ig, hulls in self.linkgeometry[ilink]:
                ...
                for hull in hulls:
                    if preservetransform:
                        ...
                        ginfo._meshcollision.vertices[voffset:(voffset + len(hull[0])), :] = transformPoints(
                            geometries[ig].GetTransform(), hull[0])
    ...
    def show(self, options=None):
        ...
        volumecolors = array(((1, 0, 0, 0.5), (0, 1, 0, 0.5), (0, 0, 1, 0.5), (0, 1, 1, 0.5), (1, 0, 1, 0.5),
                              (1, 1, 0, 0.5), (0.5, 1, 0, 0.5), (0.5, 0, 1, 0.5), (0, 0.5, 1, 0.5), (1, 0.5, 0, 0.5),
                ...
                for ilink, link in enumerate(self.robot.GetLinks()):
                    ...
                    for ig, geom in enumerate(link.GetGeometries()):
                        ...
                        if len(cdhulls) > 0:
                            ...
                            hulls.append(self.transformHull(geom.GetTransform(),
                                                            ComputeGeodesicSphereMesh(geom.GetSphereRadius(), level=1)))
                        elif geom.GetType() == KinBody.Link.GeomType.Cylinder:
                            hulls.append(self.transformHull(geom.GetTransform(),
                                                            ComputeCylinderYMesh(radius=geom.GetCylinderRadius(),
                                                                                 height=geom.GetCylinderHeight())))
                    handles += [
                        self.env.drawtrimesh(points=transformPoints(link.GetTransform(), hull[0]), indices=hull[1],
                                             colors=volumecolors[mod(colorindex + i, len(volumecolors))]) for i, hull in
                        enumerate(hulls)]
    ...
    def ShowLink(self, ilink, progressive=False):
        ...
        for ig, geom in enumerate(link.GetGeometries()):
            ...
            if len(cdhulls) > 0:
                ...
                hulls.append(
                    self.transformHull(geom.GetTransform(), ComputeGeodesicSphereMesh(geom.GetSphereRadius(), level=1)))
            elif geom.GetType() == KinBody.Link.GeomType.Cylinder:
                hulls.append(self.transformHull(geom.GetTransform(),
                                                ComputeCylinderYMesh(radius=geom.GetCylinderRadius(),
                                                                     height=geom.GetCylinderHeight())))
        try:
            if not progressive:
                handles = [self.env.drawtrimesh(points=transformPoints(link.GetTransform(), hull[0]), indices=hull[1],
                                                colors=volumecolors[mod(i, len(volumecolors))]) for i, hull in
                           enumerate(hulls)]
                input('Press any key to exit: ')
            else:
                ihull = 0
                while ihull < len(hulls):
                    ...
                    handles = [
                        self.env.drawtrimesh(points=transformPoints(link.GetTransform(), hull[0]), indices=hull[1],
                                             colors=volumecolors[mod(ihull, len(volumecolors))])]
                    cmd = input(str(ihull))
                    if cmd == 'p':
                        ihull -= 1
                    else:
    ...
    def CreateOptionParser():
        ...
        parser.add_option('--concavityThresholdPercent', action='store', type='float', dest='concavityThresholdPercent',
                          default=5.0,
                          help='The percentage of concavity allowed without causing a split to occur (default=%default).')
        parser.add_option('--mergeThresholdPercent', action='store', type='float', dest='mergeThresholdPercent',
                          default=30.0,
                          help='The percentage of volume difference allowed to merge two convex hulls (default=%default).')
        parser.add_option('--volumeSplitThresholdPercent', action='store', type='float',
                          dest='volumeSplitThresholdPercent', default=5.0,
                          help='The percentage of the total volume of the object above which splits will still occur (default=%default).')
        parser.add_option('--useInitialIslandGeneration', action='store', type='int', dest='useInitialIslandGeneration',
                          default=1,
