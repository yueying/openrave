...
class IKFastSolver(AutoReloader):
    ...
    def checkForDivideByZero(self, eq):
        ...
        if len(checkforzeros) > 0:
            ...
            for eqtemp in checkforzeros:
                ...
                if self.isExpressionUnique(newcheckforzeros, checkeq) and self.isExpressionUnique(newcheckforzeros,
                                                                                                  -checkeq):
    ...
    def writeIkSolver(self, chaintree, lang=None):
        ...
        return CodeGenerators[lang](kinematicshash=self.kinematicshash, version=__version__,
                                    iktypestr=self._iktype).generate(chaintree)
    ...
    def solveFullIK_Direction3D(self, LinksRaw, jointvars, isolvejointvars,
                                rawbasedir=Matrix(3, 1, [S.Zero, S.Zero, S.One])):
        ...
        return AST.SolverIKChainDirection3D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                            ...
                                            Dfk=Tfinal[0, 0:3].transpose())

    def solveFullIK_Lookat3D(self, LinksRaw, jointvars, isolvejointvars,
                             rawbasedir=Matrix(3, 1, [S.Zero, S.Zero, S.One]),
                             rawbasepos=Matrix(3, 1, [S.Zero, S.Zero, S.Zero])):
        ...
        chaintree = AST.SolverIKChainLookat3D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                              ...
                                              Dfk=Tfinal[0, 0:3].transpose(), Pfk=Tfinal[0:3, 3])
    ...
    def solveFullIK_Rotation3D(self, LinksRaw, jointvars, isolvejointvars, Rbaseraw=eye(3)):
        ...
        return AST.SolverIKChainRotation3D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                           ...
                                           (self.Tee[0:3, 0:3] * self.affineInverse(Tfirstright)[0:3, 0:3]).subs(
                                               self.freevarsubs), tree, Rfk=Tfinal[0:3, 0:3] * Tfirstright[0:3, 0:3])
    ...
    def _solveFullIK_Translation3D(self, LinksRaw, jointvars, isolvejointvars, basepos, check=True):
        ...
        chaintree = AST.SolverIKChainTranslation3D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                                   [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                                   Pee=self.Tee[0:3, 3], jointtree=transtree, Pfk=Tfinal[0:3, 3])
        chaintree.dictequations += self.ppsubs
        return chaintree

    def solveFullIK_TranslationXY2D(self, LinksRaw, jointvars, isolvejointvars,
                                    rawbasepos=Matrix(2, 1, [S.Zero, S.Zero])):
        ...
        chaintree = AST.SolverIKChainTranslationXY2D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                                     [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                                     Pee=self.Tee[0:2, 3], jointtree=transtree, Pfk=Tfinal[0:2, 3])
        chaintree.dictequations += self.ppsubs
        return chaintree

    def solveFullIK_TranslationXYOrientation3D(self, LinksRaw, jointvars, isolvejointvars,
                                               rawbasepos=Matrix(2, 1, [S.Zero, S.Zero]), rawangle=S.Zero):
        raise self.CannotSolveError('TranslationXYOrientation3D not implemented yet')

    def solveFullIK_Ray4D(self, LinksRaw, jointvars, isolvejointvars, rawbasedir=Matrix(3, 1, [S.Zero, S.Zero, S.One]),
                          rawbasepos=Matrix(3, 1, [S.Zero, S.Zero, S.Zero])):
    ...
    def solveFullIK_TranslationDirection5D(self, LinksRaw, jointvars, isolvejointvars,
        ...
        basedir /= sqrt(basedir[0] * basedir[0] + basedir[1] * basedir[1] + basedir[2] * basedir[
        ...
        if tree is None:
            ...
            if len(usedvars) < len(solvejointvars):
                ...
                endbranchtree2 += self.solveAllEquations(AllEquations, curvars=curvars,
        ...
        chaintree = AST.SolverIKChainRay([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                         ...
                                         jointtree=tree, Dfk=Tfinal[0, 0:3].transpose(), Pfk=Tfinal[0:3, 3],
                                         is5dray=True)
        chaintree.dictequations += self.ppsubs
        return chaintree
    ...
    def solve5DIntersectingAxes(self, T0links, basepos, D, solvejointvars, endbranchtree):
        ...
        AllEquations = self.buildEquationsFromTwoSides([D], [T0[0:3, 0:3] * self.Tee[0, 0:3].transpose()],
    ...
    def solveFullIK_6D(self, LinksRaw, jointvars, isolvejointvars, Tgripperraw=eye(4)):
        ...
        for T0links, T1links, transvars, rotvars, solveRotationFirst in self.iterateThreeIntersectingAxes(
            ...
            try:
                tree = self.solve6DIntersectingAxes(T0links, T1links, transvars, rotvars,
    ...
    def solveFullIK_6DGeneral(self, T0links, T1links, solvejointvars, endbranchtree):
        ...
        for var in usedvars:
            ...
            solsubs += self.Variable(var).subs
        self.checkSolvability(AllEquations, curvars, self.freejointvars + usedvars)
        leftovervarstree += self.solveAllEquations(AllEquations, curvars=curvars,
                                                   othersolvedvars=self.freejointvars + usedvars, solsubs=solsubs,
                                                   endbranchtree=endbranchtree)
        return coupledsolutions

    def solveFullIK_TranslationAxisAngle4D(self, LinksRaw, jointvars, isolvejointvars,
                                           rawbasedir=Matrix(3, 1, [S.One, S.Zero, S.Zero]),
                                           rawbasepos=Matrix(3, 1, [S.Zero, S.Zero, S.Zero]),
                                           rawglobaldir=Matrix(3, 1, [S.Zero, S.Zero, S.One]), rawnormaldir=None):
        ...
        if normaldir is None:
            ...
            Tfinal[0, 0] = atan2(binormaldir.dot(Tallmult[0:3, 0:3] * basedir),
                                 globaldir.dot(Tallmult[0:3, 0:3] * basedir))
        ...
        for index in range(len(ilinks)):
            ...
            if normaldir is not None:
                ...
                eq = self.simplifyTransform(self.trigsimp(binormaldir2.dot(basedir2), solvejointvars)) - sin(
                    self.Tee[0])
                if self.isExpressionUnique(AllEquations, eq) and self.isExpressionUnique(AllEquations, -eq):
                    AllEquations.append(eq)
        ...
        try:
            ...
            rawpolyeqs = [Poly(eq.subs(polysubs), *polyvars).subs(trigsubs) for eq in AllEquations if
                          eq.has_any_symbols(*curvars)]
            ...
            coupledsolution = AST.SolverCoeffFunction(jointnames=[v.name for v in curvars],
                                                      ...
                                                      jointevalcos=[dummysubs[2 * i][1] for i in range(len(curvars))],
        ...
        chaintree = AST.SolverIKChainAxisAngle([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                               ...
                                               Pfk=Tfinal[0:3, 3], anglefk=Tfinal[0, 0], iktype=iktype)
        chaintree.dictequations += self.ppsubs
        return chaintree
            ...
            try:
                ...
                if len(reducedeqs2) > 0:
                    ...
                    reducedeqs += [[Poly(eq[0], *usedvars[j]), Poly(eq[1], *usedvars[1 - j])] for eq in reducedeqs2] + [
                        [Poly(S.Zero, *polyeq[j].symbols), polyeq[1 - j] - polyeq[j].as_basic()] for polyeq in polyeqs
                        if polyeq[j].degree == 0]
                    if len(reducedeqs) > 0:
                        break;
            except self.CannotSolveError as e:
                print(e)
    ...
    def reduceBothSidesInverseMatrix(self, leftsideeqs, rightsideeqs):
        ...
        for i in range(len(allmonomsleft)):
            ...
            for k in range(len(allmonomsleft)):
                if Aadj[i, k] != S.Zero:
                    pright += Asymbols[i][k] * (
                                rightsideeqs[eqindices[k]].as_basic() - leftsideeqs[eqindices[k]].coeff())
        ...
        def _computereducedequations():
            ...
            for eqindices in combinations(list(range(len(newleftsideeqs))), len(newunknowns)):
                ...
                if solution is not None and all(
                    ...
                    for i in othereqindices:
                        reducedeqs.append(
                            [S.Zero, (newleftsideeqs[i].subs(solsubs).subs(localsymbols)).as_basic().expand()])
                    break
        ...
        if len(singlevariables) > 0:
            ...
            if 0:
                ...
                for peq0, peq1 in combinations(singledegreeeqs, 2):
                    AllEquations.append(
                        simplify((peq0.coeff(0) * peq1.coeff(1) - peq0.coeff(1) * peq1.coeff(0)).subs(self.invsubs)))
    ...
    def solveLiWoernleHiller(self, rawpolyeqs, solvejointvars, endbranchtree):
        ...
        for i in range(0, len(polyeqs), 2):
            ...
            if self.equal(p0.coeff(1, 0), p1.coeff(0, 1)) and self.equal(p0.coeff(0, 1), -p1.coeff(1, 0)):
                ...
                neweqs.append(
                    [Poly(p0.coeff(1, 0) + p0.coeff(0, 1) * tvar + p0.coeff(0, 0) + tvar * p1.coeff(0, 0), *symbols),
                     Poly(r0 + tvar * r1, *othersymbols)])
                neweqs.append(
                    [Poly(p0.coeff(1, 0) * tvar - p0.coeff(0, 1) - p0.coeff(0, 0) * tvar + p1.coeff(0, 0), *symbols),
        ...
        for nextindex in [0, 2]:
            ...
            htvarsubs += [(othersymbols[nextindex], (1 - htvar ** 2) / (1 + htvar ** 2)),
                          (othersymbols[nextindex + 1], 2 * htvar / (1 + htvar ** 2))]
            htvars.append(htvar)
            ...
            for curvar in usedvars[:-1]:
                try:
                    ...
                    jointtrees3.append(AST.SolverCheckZeros('sanitycheck', jointcheckeqs, zerobranch=endbranchtree,
                                                            nonzerobranch=[AST.SolverBreak()], anycondition=False,
                                                            thresh=0.001))
        ...
        if hassinglevariable:
            ...
            for i in range(3):
                try:
                    ...
                    unknownvars.pop(i)
                    endbranchtree2 = []
                    solutiontree = self.solveAllEquations(AllEquations, curvars=[usedvars[i]],
                                                          othersolvedvars=self.freejointvars[:],
                                                          solsubs=self.freevarsubs[:],
                                                          endbranchtree=[AST.SolverSequence([endbranchtree2])],
                                                          unknownvars=unknownvars)
                    endbranchtree2 += self.solveAllEquations(AllEquations, curvars=unknownvars[0:2],
                                                             othersolvedvars=self.freejointvars[:] + [usedvars[i]],
                                                             solsubs=self.freevarsubs[:] + self.Variable(
                                                                 usedvars[i]).subs, endbranchtree=endbranchtree)
        ...
        coupledsolution = AST.SolverCoeffFunction(jointnames=[v.name for v in usedvars],
                                                  ...
                                                  isHinges=[self.isHinge(v.name) for v in usedvars],
                                                  exportvar=[v.name for v in htvars], exportcoeffeqs=exportcoeffeqs,
                                                  exportfnname='solvedialyticpoly8qep', rootmaxdim=16)
    ...
    def solveDialytically(self, newreducedeqs, ileftvar, returnmatrix=False, getsubs=None):
        ...
        if len(allmonoms) > 2 * len(newreducedeqs):
            raise self.CannotSolveError(
                'solveDialytically: more unknowns than equations %d>%d' % (len(allmonoms), 2 * len(newreducedeqs)))
        ...
        for ipeq, peq in enumerate(newreducedeqs):
            for c, m in peq.iter_terms():
                ...
                exportindex = degree * len(origmonoms) * len(newreducedeqs) + len(origmonoms) * ipeq + origmonoms.index(
                    tuple(mlist))
                    ...
                    if self.equal(p.coeff(*m0), p.coeff(*m1)):
                        for i, j, k in [(0, 1, 2), (0, 2, 1), (1, 2, 0)]:
                            if ((m0[i] == 2 and m1[j] == 2) or (m0[j] == 2 and m1[i] == 2)) and m0[k] == m1[k]:
                                ...
                                p = p + p.coeff(*m0) * (
                                            group[3] - p.symbols[0] ** 2 - p.symbols[1] ** 2 - p.symbols[2] ** 2) * \
                                    p.symbols[k] ** (m0[k])
                                neweq = p.as_basic()
                                eq = neweq
                                break
                        ...
                        for (c0, m0), (c1, m1) in [comb, comb[::-1]]:
                            if m0[cg[0][0]] == 1 and m0[cg[0][1]] == 1 and m1[cg[1][0]] == 1 and m1[cg[1][1]] == 1:
                                ...
                                if tuple(m0l) == tuple(m1l):
                                    ...
                                    p = Poly(p.as_basic() - pzero.add_term(c0, m0).as_basic() - pzero.add_term(c1,
                                                                                                               m1).as_basic() + pzero.add_term(
                                        c0, tuple(m2)).as_basic(), *allsymbols)
                                    changed = True
                                    break
                        if changed:
                            break
    ...
    def verifyAllEquations(self, AllEquations, unsolvedvars, solsubs, tree=None):
        ...
        for i in range(len(AllEquations)):
            ...
            if not expr.has_any_symbols(*unsolvedvars) and (
                    self.isExpressionUnique(extrazerochecks, expr) or self.isExpressionUnique(extrazerochecks, -expr)):
            ...
            if len(usedsolutions) == 0:
                ...
                for usedsolution, usedvar in usedsolutions:
                    if len(solution.checkforzeros) == len(usedsolution.checkforzeros):
                        if not any([self.isExpressionUnique(usedsolution.checkforzeros, -eq) or self.isExpressionUnique(
                                usedsolution.checkforzeros, eq) for eq in solution.checkforzeros]):
                            match = True
                            break
                if not match:
                    usedsolutions.append((solution, var))
                    ...
                    for othervar in othersolvedvars:
                        ...
                        if checkzero.has_any_symbols(othervar, sothervar, cothervar):
                            ...
                            for value in [S.Zero, pi / 2, pi, -pi / 2]:
                                try:
                                    checkzerosub = checkzero.subs([(othervar, value), (sothervar, sin(value).evalf()),
                                                                   (cothervar, cos(value).evalf())])
                            ...
                            if s.jointeval is not None and len(s.jointeval) > 0:
                                ...
                                ss = []
                            try:
                                ss += self.solveSingleVariable(
                                    [checkzero.subs([(sothervar, sin(othervar)), (cothervar, cos(othervar))])],
                                    othervar, othersolvedvars)
                                ...
                                if s.jointeval is not None:
                                    for eq in s.jointeval:
                                        if eq.is_number:
                                            ...
                                            if self.isExpressionUnique(handledconds + [tempeq[0] for tempeq in eqs],
                                                                       -cond) and self.isExpressionUnique(
                                                    handledconds + [tempeq[0] for tempeq in eqs], cond):
                                                if self.isHinge(othervar.name):
                                                    evalcond = fmod(cond + pi, 2 * pi) - pi
                                                else:
                                                    evalcond = cond
                                                eqs.append([cond, evalcond, [(sothervar, sin(eq).evalf()),
                                                                             (sin(othervar), sin(eq).evalf()),
                                                                             (cothervar, cos(eq).evalf()),
                                                                             (cos(othervar), cos(eq).evalf()),
                                                                             (othervar, eq)]])
                                        ...
                                        if eq.is_number:
                                            ...
                                            if self.isExpressionUnique(handledconds + [tempeq[0] for tempeq in eqs],
                                                ...
                                                if self.isHinge(othervar.name):
                                                    ...
                                                    evalcond = cond
                                                eqs.append([cond, evalcond, [(sothervar, eq), (sin(othervar), eq),
                                                                             (cothervar, sqrt(1 - eq * eq).evalf()),
                                                                             (cos(othervar), sqrt(1 - eq * eq).evalf()),
                                                                             (othervar, asin(eq).evalf())]])
                                        ...
                                        if eq.is_number:
                                            ...
                                            if self.isExpressionUnique(handledconds + [tempeq[0] for tempeq in eqs],
                                                ...
                                                if self.isHinge(othervar.name):
                                                    ...
                                                    evalcond = cond
                                                eqs.append([cond, evalcond, [(sothervar, sqrt(1 - eq * eq).evalf()),
                                                                             (sin(othervar), sqrt(1 - eq * eq).evalf()),
                                                                             (cothervar, eq), (cos(othervar), eq),
                                                                             (othervar, acos(eq).evalf())]])
                                            cond = othervar + acos(eq).evalf()
                                            if self.isExpressionUnique(handledconds + [tempeq[0] for tempeq in eqs],
                                                                       -cond) and self.isExpressionUnique(
                                                    handledconds + [tempeq[0] for tempeq in eqs], cond):
                                                if self.isHinge(othervar.name):
                                                    ...
                                                    evalcond = cond
                                                eqs.append([cond, evalcond, [(sothervar, -sqrt(1 - eq * eq).evalf()), (
                                                sin(othervar), -sqrt(1 - eq * eq).evalf()), (cothervar, eq),
                                                                             (cos(othervar), eq),
                                                                             (othervar, -acos(eq).evalf())]])
                    ...
                    for preal in [Symbol('px'), Symbol('py'), Symbol('pz')]:
                        if checkzero.has_any_symbols(preal):
                            ...
                            for othervar in othersolvedvars:
                                ...
                                if checkzero.has_any_symbols(othervar, sothervar, cothervar):
                                    for value in [S.Zero, pi / 2, pi, -pi / 2]:
                                        eq = checkzero.subs([(othervar, value), (sothervar, sin(value).evalf()),
                                                             (cothervar, cos(value).evalf()), (preal, S.Zero)]).evalf()
                                        if eq == S.Zero:
                                            ...
                                            if self.isExpressionUnique(handledconds + [tempeq[0] for tempeq in eqs],
                                                                       -cond) and self.isExpressionUnique(
                                                    handledconds + [tempeq[0] for tempeq in eqs], cond):
                                                eqs.append([cond, evalcond, [(sothervar, sin(value).evalf()),
                                                                             (sin(othervar), sin(value).evalf()),
                                                                             (cothervar, cos(value).evalf()),
                                                                             (cos(othervar), cos(value).evalf()),
                                                                             (preal, S.Zero), (othervar, value)]])
            ...
            try:
                ...
                for i in range(len(NewEquations)):
                    ...
                    if not self.isValidSolution(expr):
                        ...
                        break
                    if not expr.has_any_symbols(*allvars) and (
                            self.isExpressionUnique(extrazerochecks, expr) or self.isExpressionUnique(extrazerochecks,
                                                                                                      -expr)):
                        extrazerochecks.append(expr.subs(solsubs).evalf())
                if extrazerochecks is not None:
                    newcases = set(currentcases)
                    newcases.add(cond)
                    newtree = self.solveAllEquations(NewEquations, curvars, othersolvedvars, solsubs, endbranchtree,
    ...
    def solvePairVariablesHalfAngle(self, raweqns, var0, var1, othersolvedvars, subs=None):
        ...
        polyeqs = []
        for eq in raweqns:
            peq = Poly(eq.subs(varsubs).subs(varsym0.svar ** 2, 1 - varsym0.cvar ** 2).expand().subs(varsym1.svar ** 2,
                                                                                                     1 - varsym1.cvar ** 2),
                       *unknownvars)
        ...
        try:
            return self.solveSingleVariable([e.as_basic() for e in polyeqs if not e.has_any_symbols(varsym1.htvar)],
                                            varsym0.var, othersolvedvars, unknownvars=[])
        except self.CannotSolveError:
            pass
        try:
            return self.solveSingleVariable([e.as_basic() for e in polyeqs if not e.has_any_symbols(varsym0.htvar)],
                                            varsym1.var, othersolvedvars, unknownvars=[])
        except self.CannotSolveError:
            pass
            ...
            for degree in range(mindegree, maxdegree + 1):
                ...
                if degree + 1 <= len(newpolyeqs2):
                    ...
                    if len(possibilities) > 0:
                        ...
                        equationdegrees = [(peq.degree * 100000 + self.codeComplexity(peq.as_basic()), peq) for peq in
                                           possibilities]
        ...
        if solutions[0] is not None:
            if solutions[1] is not None:
                if solutions[1][0].degree < solutions[0][0].degree:
                    pfinals = solutions[1]
                    ileftvar = 1
                elif solutions[1][0].degree == solutions[0][0].degree and self.codeComplexity(
                        solutions[1][0].as_basic()) < self.codeComplexity(solutions[0][0].as_basic()):
                    pfinals = solutions[1]
                    ileftvar = 1
                else:
                    pfinals = solutions[0]
        ...
        if pfinals is None:
            raise self.CannotSolveError(
                'solvePairVariablesHalfAngle: solve dialytically with %d equations' % (len(polyeqs)))
    ...
    def solveSingleVariableLinearly(self, raweqns, solvevar, othervars, maxnumeqs=2, douniquecheck=True):
        ...
        for p0, p1 in combinations(eqpolys, 2):
            M = Matrix(2, 3,
                       [p0.coeff(1, 0), p0.coeff(0, 1), p0.coeff(0, 0), p1.coeff(1, 0), p1.coeff(0, 1), p1.coeff(0, 0)])
        ...
        if douniquecheck:
            ...
            while i < len(neweqs2):
                ...
                if neweqs2[i] != S.Zero and self.isExpressionUnique(reducedeqs, reducedeq) and self.isExpressionUnique(
                        reducedeqs, -reducedeq):
    ...
    def checkFinalEquation(self, pfinal, subs=None):
        ...
        if pfinal.LC.evalf() == S.Zero or all(
                [pfinal.LC.subs(subs).subs(testconsistentvalue).evalf() == S.Zero for testconsistentvalue in
                 self.testconsistentvalues]):
            return None
        ...
        for testconsistentvalue in self.testconsistentvalues:
            ...
            for degree in range(pfinal.degree, -1, -1):
                coeffs.append(
                    pfinal.coeff(degree).subs(subs).subs(globalsymbols + testconsistentvalue).evalf() / common.evalf())
    ...
    def solveSingleVariable(self, raweqns, var, othersolvedvars, maxsolutions=4, maxdegree=2, subs=None,
        ...
        if len(eqns) > 1:
            ...
            for e in eqns:
                ...
                if Poly(enew, varsym.svar).coeff() == S.Zero or Poly(enew, varsym.cvar) == S.Zero or Poly(enew,
                                                                                                          varsym.var) == S.Zero:
                ...
                if s is not None:
                    ...
                    for svarsol, cvarsol in sollist:
                        ...
                        if (svarsol - cvarsol).subs(listsymbols).expand() == S.Zero:
                            break
                        if svarsol.subs(listsymbols).expand() == S.Zero and abs(
                                cvarsol.subs(listsymbols).expand()) - S.One != S.Zero:
                            break
                        if cvarsol.subs(listsymbols).expand() == S.Zero and abs(
                                svarsol.subs(listsymbols).expand()) - S.One != S.Zero:
                            break
                        ...
                        if unknownvars is not None:
                            ...
                            if len(unsolvedsymbols) > 0:
                                solversolution.equationsused = [eq for eq in eqns if
                                                                not eq.has_any_symbols(*unsolvedsymbols)]
                            else:
            ...
            if unknownvars is not None:
                ...
                if len(unsolvedsymbols) > 0:
                    equationsused = [eq2 for ieq2, eq2 in enumerate(eqns) if
                                     ieq2 != ieq and not eq2.has_any_symbols(*unsolvedsymbols)]
                else:
                    equationsused = eqns[:]
            ...
            if numcvar > 0:
                try:
                    ...
                    if self.countVariables(eqnew, varsym.svar) <= 1 or (
                        ...
                        jointsolutions = [self.trigsimp(s.subs(symbols + varsym.subsinv), othersolvedvars) for s in
                                          tempsolutions]
                        if all([self.isValidSolution(s) and self.isValidSolution(s) for s in jointsolutions]):
    ...
    def solvePairVariables(self, raweqns, var0, var1, othersolvedvars, maxcomplexity=50, unknownvars=None):
        ...
        reduceeqns = [Poly(eq.as_basic().subs(pairwisesubs), *pairwisevars) for rank, eq in orgeqns if
        ...
        while len(eqcombs) > 0 and len(neweqns) < 20:
            ...
            for i in range(6):
                ...
                if eq0.coeff(*monom) != 0 and eq1.coeff(*monom) != 0:
                    tempeq = (eq0.as_basic() * eq1.coeff(*monom) - eq0.coeff(*monom) * eq1.as_basic()).subs(
                ...
                if len(polyunknown) >= 2:
                    ...
                    for curiter in range(2):
                        ...
                        for i in range(len(polyunknown)):
                            eq = (polyunknown[i] * pbase.coeff(*monom) - pbase * polyunknown[i].coeff(
                                *monom)).as_basic().subs(allsymbols).expand()
                            if eq != S.Zero and self.isExpressionUnique(addedeqs, eq) and self.isExpressionUnique(
                                    addedeqs, -eq):
                                eqnew, symbols = self.groupTerms(eq, unknownvars, symbolgen)
                                allsymbols += symbols
        ...
        try:
            rawsolutions += self.solveSingleVariable([e.as_basic().subs(varsubsinv).expand() for score, e in neweqns if
                                                      not e.has_any_symbols(cvar1, svar1, var1)], var0, othersolvedvars,
                                                     subs=allsymbols, unknownvars=unknownvars)
        except self.CannotSolveError:
            pass

        try:
            rawsolutions += self.solveSingleVariable([e.as_basic().subs(varsubsinv).expand() for score, e in neweqns if
                                                      not e.has_any_symbols(cvar0, svar0, var0)], var1, othersolvedvars,
                                                     subs=allsymbols, unknownvars=unknownvars)
        except self.CannotSolveError:
            pass
        ...
        for i, unknownvar in enumerate(unknownvars):
            ...
            for rank, eq in neweqns:
                ...
                if all([m[i] == 0 or (builtins.sum(m) == m[i] and m[i] > 0) for m in eq.iter_monoms()]) and any(
                        [m[i] > 0 for m in eq.iter_monoms()]):
                ...
                if len(neweqs) > 0:
                    for ivar in range(2):
                        ...
                        for paireq0, paireq1 in combinations(lineareqs, 2):
                            ...
                            eq0 = paireq0[ivar]
                            eq1 = paireq1[ivar]
                            disc = (eq0.coeff(1, 0) * eq1.coeff(0, 1) - eq0.coeff(0, 1) * eq1.coeff(1, 0)).subs(
                                allsymbols).expand()
                            if disc == S.Zero:
                                continue
                            ...
                            ptotal_cos = Poly(totaleq, *polysymbols).subs(polysymbols[0] ** 2,
                                                                          1 - polysymbols[1] ** 2).subs(
                                polysymbols[1] ** 2, 1 - polysymbols[0] ** 2)
                            ...
                            finaleq = (ptotal_cos.as_basic() ** 2 - (
                                        1 - polysymbols[0] ** 2) * ptotal_sin.as_basic() ** 2).expand()
        ...
        if solutions:
            ...
            if (varindex % 2) == 0:
                solversolution.jointevalcos = [
                    self.trigsimp(s.subs(allsymbols + varsubsinv), othersolvedvars).subs(varsubs) for s in solutions]
            else:
                solversolution.jointevalsin = [
                    self.trigsimp(s.subs(allsymbols + varsubsinv), othersolvedvars).subs(varsubs) for s in solutions]
