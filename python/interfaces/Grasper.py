...
class Grasper:
    ...
    def Grasp(self, direction=None, roll=None, position=None, standoff=None, target=None, stablecontacts=False,
              forceclosure=False, transformrobot=True, onlycontacttarget=True, tightgrasp=False, graspingnoise=None,
              execute=None, translationstepmult=None, outputfinal=False, manipulatordirection=None, coarsestep=None,
              finestep=None, vintersectplane=None, chuckingdirection=None, ordereddofindices=None, avoidcontact=False):
        ...
        if transformrobot:
            cmd += 'roll %.15e position %.15e %.15e %.15e manipulatordirection %.15e %.15e %.15e ' % (
            roll, position[0], position[1], position[2], manipulatordirection[0], manipulatordirection[1],
            manipulatordirection[2])
        if standoff is not None:
            cmd += 'standoff %.15e ' % standoff
        if target is not None:
            cmd += 'target %s ' % target.GetName()
        cmd += 'stablecontacts %d forceclosure %d transformrobot %d onlycontacttarget %d tightgrasp %d outputfinal %d ' % (
        stablecontacts, forceclosure, transformrobot, onlycontacttarget, tightgrasp, outputfinal)
        ...
        if vintersectplane is not None:
            cmd += 'vintersectplane %.15e %.15e %.15e %.15e ' % (
            vintersectplane[0], vintersectplane[1], vintersectplane[2], vintersectplane[3])
    ...
    def GraspThreaded(self, approachrays, standoffs, preshapes, rolls, manipulatordirections=None, target=None,
                      transformrobot=True, onlycontacttarget=True, tightgrasp=False, graspingnoise=None,
                      forceclosurethreshold=None, collisionchecker=None, translationstepmult=None, numthreads=None,
                      startindex=None, maxgrasps=None, finestep=None):
    ...
    def ConvexHull(self, points, returnplanes=True, returnfaces=True, returntriangles=True):
        ...
        if returnplanes:
            ...
            planes = reshape(array([float64(resvalues.pop(0)) for i in range((dim + 1) * numplanes)], float64),
                             (numplanes, dim + 1))
        if returnfaces:
