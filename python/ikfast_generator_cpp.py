...
"""
    ...
    def getClassInit(self, node, iktype, userotation=7, usetranslation=7):
        ...
        self._solutioncounter = 0
        code += 'unsigned char ' + ','.join(
            '_i%s[2], _n%s' % (var[0].name, var[0].name) for var in node.solvejointvars + node.freejointvars) + ';\n\n'
        # special variable
    ...
    def GetIkFunctionPreamble(self, node):
        ...
        for var in node.solvejointvars:
            code += '%s=numeric_limits<IkReal>::quiet_NaN(); _i%s[0] = -1; _i%s[1] = -1; _n%s = -1; ' % (
    ...
    def generateChain(self, node):
        ...
        if node.Tfk:
            ...
            subexprs, reduced_exprs = customcse(
                node.Tfk[0:3, 0:4].subs([(v[0], Symbol('j[%d]' % v[1])) for v in allvars]), self.symbolgen)
            outputnames = ['eerot[0]', 'eerot[1]', 'eerot[2]', 'eetrans[0]', 'eerot[3]', 'eerot[4]', 'eerot[5]',
                           'eetrans[1]', 'eerot[6]', 'eerot[7]', 'eerot[8]', 'eetrans[2]']
            fcode = ''
            if len(subexprs) > 0:
    ...
    def generateIKChainRotation3D(self, node):
        ...
        if node.Rfk:
            ...
            allvars = node.solvejointvars + node.freejointvars
            subexprs, reduced_exprs = customcse(
                node.Rfk[0:3, 0:3].subs([(v[0], Symbol('j[%d]' % v[1])) for v in allvars]), self.symbolgen)
            outputnames = ['eerot[0]', 'eerot[1]', 'eerot[2]', 'eerot[3]', 'eerot[4]', 'eerot[5]', 'eerot[6]',
                           'eerot[7]', 'eerot[8]']
            fcode = ''
            if len(subexprs) > 0:
                vars = [var for var, expr in subexprs]
...
"""
    ...
    def generateSolution(self, node, declarearray=True, acceptfreevars=True):
        ...
        if node.jointeval is not None:
                    ...
                    for freevar in self.freevars:
                        if expr.has(Symbol(freevar)):
                            ...
                            if m is not None:
                                ...
                                log.error('failed to extract free variable %s for %s from: eq=%s', freevar,
                                          node.jointname, expr)
                #                             m = dict()
                #                             m[a] = Float(-1,30)
                #                             m[b] = Float(0,30)
            ...
            if node.AddPiIfNegativeEq:
                for i in range(numsolutions):
                    ...
                    name, allnumsolutions + numsolutions + i, name, allnumsolutions + i, name, allnumsolutions + i,
            ...
            for i in range(numsolutions):
                if node.isHinge:
                    eqcode.write(
                        ...
                        name, allnumsolutions + i, name, allnumsolutions + i, name, allnumsolutions + i, name,
                        allnumsolutions + i))
        ...
        if node.jointevalcos is not None:
            ...
            self.WriteEquations2(lambda i: 'c%sarray[%d]' % (name, allnumsolutions + 2 * i), node.jointevalcos,
                                 code=eqcode)
            for i in range(len(node.jointevalcos)):
                eqcode.write(
                    'if( c%sarray[%d] >= -1-IKFAST_SINCOS_THRESH && c%sarray[%d] <= 1+IKFAST_SINCOS_THRESH )\n{\n' % (
        ...
        if node.jointevalsin is not None:
            ...
            self.WriteEquations2(lambda i: 's%sarray[%d]' % (name, allnumsolutions + 2 * i), node.jointevalsin,
                                 code=eqcode)
            for i in range(len(node.jointevalsin)):
                eqcode.write(
                    'if( s%sarray[%d] >= -1-IKFAST_SINCOS_THRESH && s%sarray[%d] <= 1+IKFAST_SINCOS_THRESH )\n{\n' % (
                    name, allnumsolutions + 2 * i, name, allnumsolutions + 2 * i))
                eqcode.write('    %svalid[%d] = %svalid[%d] = true;\n' % (
                name, allnumsolutions + 2 * i, name, allnumsolutions + 2 * i + 1))
                ...
                name, allnumsolutions + 2 * i + 1, name, allnumsolutions + 2 * i, name, allnumsolutions + 2 * i, name,
                allnumsolutions + 2 * i))
                eqcode.write('    c%sarray[%d] = -c%sarray[%d];\n' % (
                name, allnumsolutions + 2 * i + 1, name, allnumsolutions + 2 * i))
                eqcode.write('}\n')
        ...
        code.write(
            'if( %svalid[ii%s] && IKabs(c%sarray[i%s]-c%sarray[ii%s]) < IKFAST_SOLUTION_THRESH && IKabs(s%sarray[i%s]-s%sarray[ii%s]) < IKFAST_SOLUTION_THRESH )\n{\n    %svalid[ii%s]=false; _i%s[1] = ii%s; break; \n}\n' % (
        ...
        for solversolution in node.solversolutions:
            if len(solversolution.checkforzeros) > 0:
                ...
                for i in range(len(solversolution.checkforzeros)):
                    if i != 0:
                        checkcode += ' && '
                    checkcode += 'IKabs(evalcond[%d]) %s %.16f ' % (
                    i, '<=' if solversolution.FeasibleIsZeros else '>', node.thresh)
                checkcode += ' )\n'
            checkcode += '{\n'
    ...
    def generatePolynomialRoots(self, node):
        ...
        name, len(node.jointeval) * D, name, len(node.jointeval) * D, name, len(node.jointeval) * D, name,
        len(node.jointeval))
        ...
        if node.postcheckforrange is not None and len(node.postcheckforrange) > 0:
            ...
            for i in range(len(node.postcheckforrange)):
                if i != 0:
                    fcode += ' || '
                fcode += ' (%sevalpoly[%d] <= %.16f || %sevalpoly[%d] > %.16f) ' % (
                name, i, -1.0 - node.postcheckforrangeThresh, name, i, 1.0 + node.postcheckforrangeThresh)
            fcode += ' )\n{\n    continue;\n}\n'
            code += fcode
        if node.postcheckforNumDenom is not None and len(node.postcheckforNumDenom) > 0:
            ...
            for i in range(len(node.postcheckforNumDenom)):
                if i != 0:
                    fcode += ' || '
                fcode += ' (IKabs(%sevalpoly[%d]) <= %.16f && IKabs(%sevalpoly[%d]) > %.16f) ' % (
                name, 2 * i, node.postcheckforNumDenomThresh, name, 2 * i + 1, node.postcheckforNumDenomThresh)
            fcode += ' )\n{\n    continue;\n}\n'
            code += fcode
    ...
    def generateCoeffFunction(self, node):
        ...
        for i, name in enumerate(node.jointnames):
            code.write('%sarray[%d], c%sarray[%d], s%sarray[%d]' % (
            name, node.rootmaxdim, name, node.rootmaxdim, name, node.rootmaxdim))
        ...
        for ivar, exportvar in enumerate(node.exportvar):
            ...
            fcode += self.writeEquations(lambda i: 'c%sarray[numsolutions]' % (node.jointnames[ivar]),
                                         node.jointevalcos[ivar:(ivar + 1)])
            fcode += self.writeEquations(lambda i: 's%sarray[numsolutions]' % (node.jointnames[ivar]),
                                         node.jointevalsin[ivar:(ivar + 1)])
            fcode += '}\n'
        self.dictequations = origequations
        ...
        # check for a similar solution
        code.write(
            'for(int ii%s = i%s+1; ii%s < numsolutions; ++ii%s)\n{\n' % (firstname, firstname, firstname, firstname))
        code.write('if( !%svalid[ii%s] ) { continue; }\n' % (firstname, firstname))
        code.write('if( ')
        for name in node.jointnames:
            code.write(
                'IKabs(c%sarray[i%s]-c%sarray[ii%s]) < IKFAST_SOLUTION_THRESH && IKabs(s%sarray[i%s]-s%sarray[ii%s]) < IKFAST_SOLUTION_THRESH && ' % (
    ...
    def _WriteExprCode(self, expr, code=None):
        ...
        if expr.is_Function:
            if expr.func == Abs:
                ...
                code.seek(pos1)
                sepcodelist.append(
                    'if( (%s) < -1-IKFAST_SINCOS_THRESH || (%s) > 1+IKFAST_SINCOS_THRESH )\n    continue;\n' % (
                    exprbase, exprbase))
            elif expr.func == asin:
                code.write('IKasin(')
                pos0 = code.tell()
                ...
                code.seek(pos1)
                sepcodelist.append(
                    'if( IKabs(%s) < IKFAST_ATAN2_MAGTHRESH && IKabs(%s) < IKFAST_ATAN2_MAGTHRESH && IKabs(IKsqr(%s)+IKsqr(%s)-1) <= IKFAST_SINCOS_THRESH )\n    continue;\n' % (
