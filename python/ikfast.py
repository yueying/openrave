...
class AST:
    ...
    class SolverSolution(SolverBase):
        ...
        def __init__(self, jointname, jointeval=None, jointevalcos=None, jointevalsin=None, AddPiIfNegativeEq=None,
                     isHinge=True, thresh=0.000001):
    ...
    class SolverCoeffFunction(SolverBase):
        ...
        def __init__(self, jointnames, jointeval=None, exportvar=None, exportcoeffeqs=None, exportfnname=None,
                     isHinges=None, rootmaxdim=16, jointevalcos=None, jointevalsin=None):
    ...
    class SolverBreak(SolverBase):
        ...
        def __init__(self, comment, varsubs=list(), othersolvedvars=list(), solsubs=list(), globalsymbols=list(),
                     endbranchtree=None):
...
class IKFastSolver(AutoReloader):
    ...
    class Variable:
        ...
        def __init__(self, var):
            ...
            self.subs = [(cos(self.var), self.cvar), (sin(self.var), self.svar), (tan(self.var), self.tvar),
    ...
    def RoundMatrix(self, T):
        ...
        if axisAngleFromRotationMatrix is not None:
            ...
            axisangle = axisAngleFromRotationMatrix(
                [[Teval[0, 0], Teval[0, 1], Teval[0, 2]], [Teval[1, 0], Teval[1, 1], Teval[1, 2]],
            ...
            if abs(angle) < 10 ** (-self.precision):
                ...
                log.debug('rotation angle: %f, axis=[%f,%f,%f]', (angle * 180 / pi).evalf(), axisangle[0], axisangle[1],
                          axisangle[2])
                ...
                if abs(angle - 3 * pi / 2) < 10 ** (-self.precision + 2):
                    quat = [-S.One / sqrt(2), accurateaxisangle[0] / sqrt(2), accurateaxisangle[1] / sqrt(2),
                    ...
                    quat = [Rational(1, 2), accurateaxisangle[0] * sqrt(3) / 2, accurateaxisangle[1] * sqrt(3) / 2,
                    ...
                    quat = [sqrt(sqrt(2) + 2) / 2, sqrt(-sqrt(2) + 2) / 2 * accurateaxisangle[0],
                    ...
                    quat = [sqrt(2) / 4 + sqrt(6) / 4, (-sqrt(2) / 4 + sqrt(6) / 4) * accurateaxisangle[0],
    ...
    def forwardKinematicsChain(self, chainlinks, chainjoints):
        ...
        with self.kinbody:
            ...
            for i, joint in enumerate(chainjoints):
                if len(joint.GetName()) == 0:
                    ...
                    chainlinks[0].GetName(), chainlinks[-1].GetName()))
                ...
                if joint.IsStatic():
                    ...
                    for iaxis in range(joint.GetDOF()):
                        ...
                        if joint.GetDOFIndex() >= 0:
                            ...
                            raise ValueError(
                                'cannot solve for mechanism when a non-mimic passive joint %s is in chain' % str(joint))
                    ...
                    if axisAngleFromRotationMatrix is not None:
                        axisangle = axisAngleFromRotationMatrix(
                            numpy.array(numpy.array(Tright * TLeftjoint), numpy.float64))
                        angle = sqrt(axisangle[0] ** 2 + axisangle[1] ** 2 + axisangle[2] ** 2)
                        if angle > 1e-8:
                            axisangle = axisangle / angle
                        log.debug('rotation angle of Links[%d]: %f, axis=[%f,%f,%f]', len(Links),
        ...
        if len(
                jointinds) > 3:  # last 3 axes always have to be intersecting, move the translation of the first axis to the left
            ileft = jointinds[-3]
            separated_trans = Links[ileft][0:3, 0:3] * Links[ileft + 1][0:3, 3]
            Ttrans = eye(4)
            for j in range(0, 3):
            ...
            if incos:
                ...
                if len(eq.args) > 2:
                    for ieq in range(2, len(eq.args)):
                        lefteq += eq.args[ieq]
                neweq = self.SimplifyAtan2(eq.args[0], incos=True) * self.SimplifyAtan2(lefteq,
                                                                                        incos=True) - self.SimplifyAtan2(
                    eq.args[0], insin=True) * self.SimplifyAtan2(lefteq, insin=True)
                processed = True
            elif insin:
                lefteq = eq.args[1]
                if len(eq.args) > 2:
                    for ieq in range(2, len(eq.args)):
                        lefteq += eq.args[ieq]
                neweq = self.SimplifyAtan2(eq.args[0], incos=True) * self.SimplifyAtan2(lefteq,
                                                                                        insin=True) + self.SimplifyAtan2(
                    eq.args[0], insin=True) * self.SimplifyAtan2(lefteq, incos=True)
                processed = True
            else:
                neweq = S.Zero
                for subeq in eq.args:
                        ...
                        if not substitutedargs[0].is_number or substitutedargs[0] == S.Zero:
                            if not substitutedargs[1].is_number or substitutedargs[1] == S.Zero:
                                ...
                                if self.codeComplexity(sumeq) < 400:
                                    testeq = self.SimplifyAtan2(
                                        (substitutedargs[0] ** 2 + substitutedargs[1] ** 2).expand())
                                else:
                                    testeq = sumeq
                                ...
                                log.info('adding atan2(%r, %r) = %r all zeros check', substitutedargs[0],
                                         substitutedargs[1], checkforzeros[-1])
                for arg in eq.args:
    ...
    def writeIkSolver(self, chaintree, lang=None):
        ...
        return CodeGenerators[lang](kinematicshash=self.kinematicshash, version=__version__, iktypestr=self._iktype,
    ...
    def generateIkSolver(self, baselink, eelink, freeindices=None, solvefn=None, ikfastoptions=0):
        ...
        for T in LinksRaw:
            log.info(
                '[' + ','.join(['[%s, %s, %s, %s]' % (T[i, 0], T[i, 1], T[i, 2], T[i, 3]) for i in range(3)]) + ']')
        ...
        self.npxyzsubs = [(self.npxyz[i], px * self.Tee[0, i] + py * self.Tee[1, i] + pz * self.Tee[2, i]) for i in
                          range(3)]
        ...
        self.pvars = self.Tee[0:12] + self.npxyz + [self.pp] + self.rxp[0] + self.rxp[1] + self.rxp[2] + [
            Symbol('new_r00'), Symbol('new_r01'), Symbol('new_r02'), Symbol('new_r10'), Symbol('new_r11'),
            Symbol('new_r12'), Symbol('new_r20'), Symbol('new_r21'), Symbol('new_r22')]
        ...
        ipp = 15
        irxp = 16
        self._rotpossymbols = self._rotsymbols + list(self.Tee[0:3, 3]) + self.npxyz + [self.pp] + self.rxp[0] + \
                              self.rxp[1] + self.rxp[2]
    ...
    def ComputeConsistentValues(self, jointvars, T, numsolutions=1, subs=None):
        ...
        """
        possibleangles = [S.Zero, pi.evalf() / 2, asin(3.0 / 5).evalf(), asin(4.0 / 5).evalf(), asin(5.0 / 13).evalf(),
                          asin(12.0 / 13).evalf()]
    ...
    def solveFullIK_Direction3D(self, LinksRaw, jointvars, isolvejointvars,
        ...
        tree = self.SolveAllEquations(AllEquations, curvars=solvejointvars, othersolvedvars=self.freejointvars[:],
                                      solsubs=self.freevarsubs[:], endbranchtree=endbranchtree)
        ...
        return AST.SolverIKChainDirection3D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                            [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                            Dee=self.Tee[0, 0:3].transpose().subs(self.freevarsubs), jointtree=tree,
    ...
    def solveFullIK_Lookat3D(self, LinksRaw, jointvars, isolvejointvars,
        ...
        chaintree = AST.SolverIKChainLookat3D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                              [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                              Pee=self.Tee[0:3, 3].subs(self.freevarsubs), jointtree=tree,
    ...
    def solveFullIK_Rotation3D(self, LinksRaw, jointvars, isolvejointvars, Rbaseraw=eye(3)):
        ...
        return AST.SolverIKChainRotation3D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                           [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                           (self.Tee[0:3, 0:3] * self.affineInverse(Tfirstright)[0:3, 0:3]).subs(
    ...
    def solveFullIK_TranslationLocalGlobal6D(self, LinksRaw, jointvars, isolvejointvars, Tmanipraw=eye(4)):
        ...
        chain = self._solveFullIK_Translation3D(LinksRaw, jointvars, isolvejointvars,
                                                Tgripper[0:3, 3] + Tgripper[0:3, 0:3] * localpos, False)
    ...
    def _solveFullIK_Translation3D(self, LinksRaw, jointvars, isolvejointvars, manippos, check=True):
        ...
        AllEquations = self.buildEquationsFromPositions(T1links, T1linksinv, solvejointvars, self.freejointvars,
                                                        uselength=True)
        if check:
            self.checkSolvability(AllEquations, solvejointvars, self.freejointvars)
        transtree = self.SolveAllEquations(AllEquations, curvars=solvejointvars[:], othersolvedvars=self.freejointvars,
                                           solsubs=self.freevarsubs[:], endbranchtree=endbranchtree)
        ...
        chaintree = AST.SolverIKChainTranslation3D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                                   [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                                   Pee=self.Tee[0:3, 3], jointtree=transtree, Pfk=self.Tfinal[0:3, 3])
        chaintree.dictequations += self.ppsubs
        return chaintree
    ...
    def solveFullIK_TranslationXY2D(self, LinksRaw, jointvars, isolvejointvars,
        ...
        AllEquations = self.buildEquationsFromTwoSides(Positions, Positionsee, solvejointvars + self.freejointvars,
                                                       uselength=True)
        ...
        chaintree = AST.SolverIKChainTranslationXY2D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                                     [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                                     Pee=self.Tee[0:2, 3], jointtree=transtree, Pfk=self.Tfinal[0:2, 3])
        chaintree.dictequations += self.ppsubs
        return chaintree

    def solveFullIK_TranslationXYOrientation3D(self, LinksRaw, jointvars, isolvejointvars,
                                               rawmanippos=Matrix(2, 1, [S.Zero, S.Zero]), rawangle=S.Zero):
        self._iktype = 'translationxyorientation3d'
        raise self.CannotSolveError('TranslationXYOrientation3D not implemented yet')

    def solveFullIK_Ray4D(self, LinksRaw, jointvars, isolvejointvars, rawmanipdir=Matrix(3, 1, [S.Zero, S.Zero, S.One]),
                          rawmanippos=Matrix(3, 1, [S.Zero, S.Zero, S.Zero])):
    ...
    def solveFullIK_TranslationDirection5D(self, LinksRaw, jointvars, isolvejointvars,
        ...
        manipdirlen2 = trigsimp(manipdir[0] * manipdir[0] + manipdir[1] * manipdir[1] + manipdir[2] * manipdir[
            2])  # unfortunately have to do it again...
        ...
        if tree is None:
            ...
            for solvemethod in [self.solveLiWoernleHiller, self.solveKohliOsvatic]:  # , self.solveManochaCanny]:
                ...
                for index in [2, 3]:
                    ...
                    if checkorientationjoints is not None:
                        ...
                        cvar3 = cos(
                            checkorientationjoints[0] + checkorientationjoints[1] + checkorientationjoints[2]).expand(
                            trig=True)
                        svar3 = sin(
                            checkorientationjoints[0] + checkorientationjoints[1] + checkorientationjoints[2]).expand(
                            trig=True)
                        ...
                        for i in range(3):
                            for j in range(3):
                                if Tcheckorientation[i, j] != S.Zero and not self.equal(Tcheckorientation[i, j],
                                                                                        cvar3) and not self.equal(
                                        Tcheckorientation[i, j], -cvar3) and not self.equal(Tcheckorientation[i, j],
                                                                                            svar3) and not self.equal(
                                        Tcheckorientation[i, j], -svar3) and Tcheckorientation[i, j] != S.One:
                                    sameorientation = False
                                    break
                        if sameorientation:
                            log.info('found joints %r to have same orientation, adding more equations',
                            ...
                            if not leftside:
                                ...
                                newp1 = newp1.subs(sin(checkorientationjoints[2]), sin(
                                    sumjoint - checkorientationjoints[0] - checkorientationjoints[1]).expand(
                                    trig=True)).expand()
                                newl1 = newl1.subs(sin(checkorientationjoints[2]), sin(
                                    sumjoint - checkorientationjoints[0] - checkorientationjoints[1]).expand(
                                    trig=True)).expand()
                                for i in range(3):
                                    newp1[i] = self.trigsimp(newp1[i], [sumjoint, checkorientationjoints[0],
                                                                        checkorientationjoints[1]])
                                    newl1[i] = self.trigsimp(newl1[i], [sumjoint, checkorientationjoints[0],
                                                                        checkorientationjoints[1]])
                                ...
                                AllEquations.append(
                                    checkorientationjoints[0] + checkorientationjoints[1] + checkorientationjoints[
                                        2] - sumjoint)
                                AllEquations.append((sin(checkorientationjoints[0] + checkorientationjoints[1]) - sin(
                                    sumjoint - checkorientationjoints[2])).expand(trig=True))
                                AllEquations.append((cos(checkorientationjoints[0] + checkorientationjoints[1]) - cos(
                                    sumjoint - checkorientationjoints[2])).expand(trig=True))
                                AllEquations.append((sin(checkorientationjoints[1] + checkorientationjoints[2]) - sin(
                                    sumjoint - checkorientationjoints[0])).expand(trig=True))
                                AllEquations.append((cos(checkorientationjoints[1] + checkorientationjoints[2]) - cos(
                                    sumjoint - checkorientationjoints[0])).expand(trig=True))
                                AllEquations.append((sin(checkorientationjoints[2] + checkorientationjoints[0]) - sin(
                                    sumjoint - checkorientationjoints[1])).expand(trig=True))
                                AllEquations.append((cos(checkorientationjoints[2] + checkorientationjoints[0]) - cos(
                                    sumjoint - checkorientationjoints[1])).expand(trig=True))
                                for consistentvalues in self.testconsistentvalues:
                                    var = self.Variable(sumjoint)
                                    consistentvalues += var.getsubs((checkorientationjoints[0] + checkorientationjoints[
                                        1] + checkorientationjoints[2]).subs(consistentvalues))
                    ...
                    try:
                        coupledsolutions, usedvars = solvemethod(rawpolyeqs2[index], newsolvejointvars,
            ...
            if len(usedvars) < len(solvejointvars):
                ...
                localtree = self.SolveAllEquations(AllEquations, curvars=curvars,
                                                   othersolvedvars=self.freejointvars + usedvars, solsubs=solsubs,
                                                   endbranchtree=endbranchtree)
                # make it a function so compiled code is smaller
                endbranchtree2.append(
                    AST.SolverFunction('innerfn', self.verifyAllEquations(AllEquations, curvars, solsubs, localtree)))
        ...
        chaintree = AST.SolverIKChainRay([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                         [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                         Pee=(self.Tee[0:3, 3] - self.Tee[0, 0:3].transpose() * offsetdist).subs(
                                             self.freevarsubs), Dee=self.Tee[0, 0:3].transpose().subs(self.freevarsubs),
                                         jointtree=tree, Dfk=self.Tfinal[0, 0:3].transpose(), Pfk=self.Tfinal[0:3, 3],
                                         is5dray=True)
        chaintree.dictequations += self.ppsubs
        return chaintree
    ...
    def solve5DIntersectingAxes(self, T0links, manippos, D, solvejointvars, endbranchtree):
        ...
        transtree = self.SolveAllEquations(AllEquations, curvars=transvars[:], othersolvedvars=self.freejointvars,
                                           solsubs=self.freevarsubs[:], endbranchtree=newendbranchtree)
        transtree = self.verifyAllEquations(AllEquations, solvejointvars, self.freevarsubs, transtree)
        rotvars = [v for v in solvejointvars if self.has(D, v)]
        solsubs = self.freevarsubs[:]
        for v in transvars:
            solsubs += self.Variable(v).subs
        AllEquations = self.buildEquationsFromTwoSides([D], [T0[0:3, 0:3].transpose() * self.Tee[0, 0:3].transpose()],
                                                       solvejointvars, uselength=False)
        self.checkSolvability(AllEquations, rotvars, self.freejointvars + transvars)
        localdirtree = self.SolveAllEquations(AllEquations, curvars=rotvars[:],
                                              othersolvedvars=self.freejointvars + transvars, solsubs=solsubs,
                                              endbranchtree=endbranchtree)
        # make it a function so compiled code is smaller
        dirtree.append(
            AST.SolverFunction('innerfn', self.verifyAllEquations(AllEquations, rotvars, solsubs, localdirtree)))
        return transtree
    ...
    def solveFullIK_6D(self, LinksRaw, jointvars, isolvejointvars, Tmanipraw=eye(4)):
        ...
        chaintree = AST.SolverIKChainTransform6D([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                                 ...
                                                 tree, Tfk=self.Tfinal * Tfirstright)
    ...
    def TestIntersectingAxes(self, solvejointvars, Links, LinksInv, endbranchtree):
        for T0links, T1links, transvars, rotvars, solveRotationFirst in self.iterateThreeIntersectingAxes(
                solvejointvars, Links, LinksInv):
            try:
                return self.solve6DIntersectingAxes(T0links, T1links, transvars, rotvars,
                                                    solveRotationFirst=solveRotationFirst, endbranchtree=endbranchtree)
        ...
        for i in range(len(ilinks) - (numExpectedRotation - 1) - 1, -1, -1):
            ...
            if solveRotationFirst is not None:
                ...
                if len(rotvars) == numExpectedRotation and len(transvars) == 3:
                    log.info('found 3 consecutive intersecting axes links[%d:%d], rotvars=%s, translationvars=%s',
        ...
        for imode in range(2):
            for i in range(len(ilinks) - 2):
                ...
                if (imode == 0 and has0 and has1) or (imode == 1 and (has0 or has1)):
                    ...
                    log.info('found 3 consecutive non-intersecting axes links[%d:%d], vars=%s', startindex, endindex,
                             str(usedvars))
    ...
    def solve6DIntersectingAxes(self, T0links, T1links, transvars, rotvars, solveRotationFirst, endbranchtree):
        ...
        curvars = transvars[:]
        solsubs = self.freevarsubs[:]
        transtree = self.SolveAllEquations(AllEquations, curvars=curvars, othersolvedvars=othersolvedvars[:],
                                           solsubs=solsubs, endbranchtree=newendbranchtree)
        transtree = self.verifyAllEquations(AllEquations, rotvars if solveRotationFirst else transvars + rotvars,
        ...
        try:
            ...
            rottree += self.SolveAllEquations(AllEquations, curvars=currotvars, othersolvedvars=othersolvedvars,
                                              solsubs=self.freevarsubs[:], endbranchtree=storesolutiontree)
            ...
            if len(rottree) == 0:
                raise self.CannotSolveError(
                    'could not solve for all rotation variables: %s:%s' % (str(freevar), str(freevalue)))
    ...
    def solveFullIK_6DGeneral(self, T0links, T1links, solvejointvars, endbranchtree, usesolvers=7):
        ...
        for solvemethod in solvemethods:
            ...
            for splitindex in [0, 1]:
                ...
                complexities[splitindex] = sum(
                    [self.ComputePolyComplexity(peq0) + self.ComputePolyComplexity(peq1) for peq0, peq1 in
                     rawpolyeqs2[splitindex]])
            ...
            for complexity, splitindex in sortedindices:
                ...
                try:
                    if rawpolyeqs2[splitindex] is not None:
                        ...
                        unusedsymbols = []
                        for solvejointvar in solvejointvars:
                            usedinequs = any([var in rawpolyeqs[0][0].gens or var in rawpolyeqs[0][1].gens for var in
                                              self.Variable(solvejointvar).vars])
                            if not usedinequs:
                        ...
                        coupledsolutions, usedvars = solvemethod(rawpolyeqs, solvejointvars,
                                                                 endbranchtree=endbranchtree,
        ...
        if len(curvars) > 0:
            ...
            leftovertree = self.SolveAllEquations(AllEquationsExtra, curvars=curvars,
                                                  othersolvedvars=self.freejointvars + usedvars, solsubs=solsubs,
    ...
    def solveFullIK_TranslationAxisAngle4D(self, LinksRaw, jointvars, isolvejointvars,
                                           ...
                                           rawmanippos=Matrix(3, 1, [S.Zero, S.Zero, S.Zero]),
                                           rawglobaldir=Matrix(3, 1, [S.Zero, S.Zero, S.One]), rawglobalnormaldir=None,
                                           ignoreaxis=None, rawmanipnormaldir=None, Tmanipraw=None):
        ...
        for i in range(3):
            manipdir[i] = self.convertRealToRational(manipdir[i], 5)
        manipdir /= sqrt(manipdir[0] * manipdir[0] + manipdir[1] * manipdir[1] + manipdir[2] * manipdir[
            2])  # unfortunately have to do it again...
        Links = LinksRaw[:]
        ...
        if self.Tfinal[0, 0] == nan:
            raise self.CannotSolveError(
                'cannot solve 4D axis angle IK. Most likely manipulator direction is aligned with the rotation axis')
        ...
        log.info('ikfast translation axis %dd, globaldir=%s, manipdir=%s: %s', expecteddof, globaldir, manipdir,
                 solvejointvars)
        ...
        T1linksinv = [self.affineInverse(Tmanipposinv)] + Links[::-1] + [self.Teeinv]
        AllEquations = self.buildEquationsFromPositions(T1links, T1linksinv, solvejointvars, self.freejointvars,
                                                        uselength=True, ignoreaxis=ignoreaxis)

        if not all([abs(eq.subs(self.testconsistentvalues[0]).evalf()) <= 1e-10 for eq in AllEquations]):
            raise self.CannotSolveError(
                'some equations are not consistent with the IK, double check if using correct IK type')
        ...
        for index in range(len(ilinks)):
            ...
            if globalnormaldir is not None:
                ...
                eq = self.SimplifyTransform(self.trigsimp(binormaldir2.dot(manipdir2), solvejointvars)) - sin(
                    self.Tee[0])
                if self.CheckExpressionUnique(AllEquations, eq):
                    AllEquations.append(eq)
        ...
        if globalnormaldir is not None:
            if Tallmult[0:3, 0:3] * manipnormaldir == globalnormaldir:
                ...
                AllEquations = [self.trigsimp(eq.subs([(cos(angles[-1]), coseq), (sin(angles[-1]), sineq)]).expand(),
        ...
        if extravar is not None:
            solution = AST.SolverSolution(extravar[0].name, jointeval=[extravar[1]],
        ...
        try:
            ...
            if 0:
                ...
                newsolution = AST.SolverSolution(solvejointvars[0].name, jointeval=solvejointvar0sols,
                                                 isHinge=self.IsHinge(solvejointvars[0].name))
                endbranchtree.insert(0, newsolution)
                tree = self.SolveAllEquations(NewEquations, curvars=solvejointvars[1:],
                                              othersolvedvars=self.freejointvars, solsubs=self.freevarsubs[:],
                                              endbranchtree=endbranchtree)
            else:
                ...
                for curvar in solvejointvars:
                    ...
                    for e in AllEquations:
                        if (len(othervars) == 0 or not e.has(*othervars)) and e.has(curvar, curvarsym.htvar,
                                                                                    curvarsym.cvar, curvarsym.svar):
                            eq = e.subs(self.freevarsubs + solsubs)
                            if self.CheckExpressionUnique(raweqns, eq):
                                raweqns.append(eq)
                    if len(raweqns) > 0:
                        try:
                            rawsolutions = self.solveSingleVariable(self.sortComplexity(raweqns), curvar,
                                                                    othersolvedvars, unknownvars=solvejointvars)
                ...
                if len(solutions) == 0:
                    raise self.CannotSolveError(
                        'failed to solve for equations. Possible errors are %s' % gatheredexceptions)
                ...
                rawpolyeqs = [Poly(Poly(eq.subs(polysubs), *polyvars).subs(trigsubs), *polyvars) for eq in AllEquations
                              if eq.has(*curvars)]
                ...
                coupledsolution = AST.SolverCoeffFunction(jointnames=[v.name for v in curvars],
                                                          ...
                                                          jointevalcos=[dummysubs[2 * i][1] for i in
                                                                        range(len(curvars))],
                                                          jointevalsin=[dummysubs[2 * i + 1][1] for i in
                                                                        range(len(curvars))],
                                                          isHinges=[self.IsHinge(v.name) for v in curvars],
                                                          exportvar=[v.name for v in dummys],
                                                          exportcoeffeqs=exportcoeffeqs,
                                                          exportfnname='solvedialyticpoly12qep', rootmaxdim=16)
                self.usinglapack = True
                tree = [firstsolution, coupledsolution] + endbranchtree

        # package final solution
        chaintree = AST.SolverIKChainAxisAngle([(jointvars[ijoint], ijoint) for ijoint in isolvejointvars],
                                               [(v, i) for v, i in zip(self.freejointvars, self.ifreejointvars)],
                                               Pee=self.Tee[0:3, 3].subs(self.freevarsubs),
                                               angleee=self.Tee[0, 0].subs(self.freevarsubs), jointtree=tree,
        ...
        for i in range(len(leftside)):
            ...
            if uselength:
                ...
                if self.codeComplexity(p2) < 1200 and self.codeComplexity(pe2) < 1200:
                    ...
                    log.info('length equations too big, skipping %d,%d', self.codeComplexity(p2),
                             self.codeComplexity(pe2))
        self.sortComplexity(AllEquations)
        return AllEquations

    def buildEquationsFromPositions(self, T1links, T1linksinv, transvars, othersolvedvars, uselength=True,
                                    removesmallnumbers=True, ignoreaxis=None):
    ...
    def buildRaghavanRothEquationsOld(self, p0, p1, l0, l1, solvejointvars):
        ...
        eqs = [(eq0, eq1) for eq0, eq1 in eqs if
               not self.CheckEquationForVarying(eq0) and not self.CheckEquationForVarying(eq1)]
        trigsubs = []
        polysubs = []
        polyvars = []
    ...
    def buildRaghavanRothEquations(self, p0, p1, l0, l1, solvejointvars, simplify=True, currentcasesubs=None):
        ...
        # prune any that have varying symbols
        # remove all fractions? having big integers could blow things up...
        return [[peq0, peq1] for peq0, peq1 in polyeqs if
                peq0 is not None and peq1 is not None and not self.CheckEquationForVarying(
                    peq0) and not self.CheckEquationForVarying(peq1)]
                ...
                if all([all(d <= 2 for d in eq.degree_list()) for eq in leftsideeqs]):
                    try:
                        numsymbolcoeffs, _computereducedequations = self.reduceBothSidesSymbolicallyDelayed(leftsideeqs,
                                                                                                            rightsideeqs)
                        reducedelayed.append(
                            [j, leftsideeqs, rightsideeqs, builtins.sum(numsymbolcoeffs), _computereducedequations])
                    except self.CannotSolveError:
                        continue
        ...
        for j, leftsideeqs, rightsideeqs, numsymbolcoeffs, _computereducedequations in reducedelayed:
            ...
            try:
                ...
                if len(reducedeqs2) > 0:
                    ...
                    reducedeqs += [[Poly(eq[0], *usedvars[j]), Poly(eq[1], *usedvars[1 - j])] for eq in reducedeqs2] + [
                        [Poly(S.Zero, *polyeq[j].gens), polyeq[1 - j] - polyeq[j].as_expr()] for polyeq in polyeqs if
                        sum(polyeq[j].degree_list()) == 0]
                    if len(reducedeqs) > 0:
                        break;
            except self.CannotSolveError as e:
                log.warn(e)
    ...
    def reduceBothSidesInverseMatrix(self, leftsideeqs, rightsideeqs):
        ...
        solution = None
        for eqindices in combinations(list(range(len(leftsideeqs))), len(allmonomsleft)):
            self._CheckPreemptFn(progress=0.06)
            ...
            if nummatrixsymbols > 10:
                ...
                if not self.IsDeterminantNonZeroByEval(A,
                                                       evalfirst=nummatrixsymbols > 60):  # pi_robot has 55 symbols and still finishes ok
                    continue
    ...
    def reduceBothSidesSymbolicallyDelayed(self, leftsideeqs, rightsideeqs, maxsymbols=10, usesymbols=True):
        ...
        dividesymbols = []
        allmonoms = dict()
        for left, right in zip(leftsideeqs, rightsideeqs):
            if right != S.Zero:
        ...
        if len(allmonoms) == 0:
            def _returnequations():
                return [[left, right] for left, right in zip(leftsideeqs, rightsideeqs)]
        ...
        newleftsideeqs = []
        numsymbolcoeffs = []
        for left, right in zip(leftsideeqs, rightsidedummy):
            left = left - right
            newleft = Poly(S.Zero, *list(allmonoms.values()))
        ...
        def _computereducedequations():
            ...
            log.info('solving for all pairwise variables in %s, number of symbol coeffs are %s', unknownvars,
                     builtins.sum(numsymbolcoeffs))
            systemcoeffs = []
            ...
            for eqindices in combinations(list(range(len(newleftsideeqs))), len(newunknowns)):
                ...
                if solution is not None and all(
                        [self.isValidSolution(value.subs(localsymbols)) for key, value in solution.items()]):
                    # substitute
                    solsubs = []
                    allvalid = True
                    ...
                    for i in othereqindices:
                        reducedeqs.append(
                            [S.Zero, (newleftsideeqs[i].subs(solsubs).subs(localsymbols)).as_expr().expand()])
                    break
    ...
    def solveManochaCanny(self, rawpolyeqs, solvejointvars, endbranchtree, AllEquationsExtra=None, currentcases=None,
        ...
        while i < len(symbols):
            ...
            if not isinstance(var, Symbol):
                ...
                var = var.args[0]
                dummys.append(dummy)
                dummysubs += [(symbols[i], (1 - dummy ** 2) / (1 + dummy ** 2)),
                              (symbols[i + 1], 2 * dummy / (1 + dummy ** 2))]
                dummysubs2.append((var, 2 * atan(dummy)))
                dummyvars.append((dummy, tan(0.5 * var)))
        ...
        if len(singlevariables) > 0:
            try:
                ...
                tree = self.SolveAllEquations(AllEquations, curvars=dummys, othersolvedvars=[],
                                              solsubs=self.freevarsubs, endbranchtree=endbranchtree,
                                              currentcases=currentcases, currentcasesubs=currentcasesubs)
        ...
        if ileftvar > 0:
            raise self.CannotSolveError(
                'solving equations dialytically succeeded with var index %d, unfortunately code generation supports only index 0' % ileftvar)
        ...
        coupledsolution = AST.SolverCoeffFunction(jointnames=[v.name for v in usedvars],
                                                  jointeval=[v[1] for v in dummysubs2], jointevalcos=jointevalcos,
                                                  jointevalsin=jointevalsin,
                                                  isHinges=[self.IsHinge(v.name) for v in usedvars],
                                                  exportvar=[v.name for v in dummys], exportcoeffeqs=exportcoeffeqs,
                                                  exportfnname='solvedialyticpoly12qep', rootmaxdim=16)
        self.usinglapack = True
        return raghavansolutiontree + [coupledsolution] + endbranchtree, usedvars

    def solveLiWoernleHiller(self, rawpolyeqs, solvejointvars, endbranchtree, AllEquationsExtra=[], currentcases=None,
                             currentcasesubs=None):
        ...
        if len(originalsymbols) != 6:
            ...
            raise self.CannotSolveError(
                'symbols %r are not all rotational, is this really necessary?' % originalsymbols)
        ...
        for i in range(len(originalsymbols)):
            ...
            if originalsymbols[i].name[0] == 'c':
                ...
                if 8 == builtins.sum(
                        [int(peq[0].has(originalsymbols[i], originalsymbols[i + 1])) for peq in rawpolyeqs]):
        ...
        for allowedindex in allowedindices:
            ...
            for i in range(len(polyeqs)):
                ...
                for j in unusedindices:
                    ...
                    if self.equal(p0dict.get((1, 0), S.Zero), -p1dict.get((0, 1), S.Zero)) and self.equal(
                            p0dict.get((0, 1), S.Zero), p1dict.get((1, 0), S.Zero)):
                        p0, p1 = p1, p0
                    ...
                    if self.equal(p0dict.get((1, 0), S.Zero), p1dict.get((0, 1), S.Zero)) and self.equal(
                        ...
                        neweqs.append([Poly(
                            p0dict.get((1, 0), S.Zero) + p0dict.get((0, 1), S.Zero) * tvar + p0.TC() + tvar * p1.TC(),
                            *symbols), Poly(r0 + tvar * r1, *othersymbols)])
                        neweqs.append([Poly(
                            p0dict.get((1, 0), S.Zero) * tvar - p0dict.get((0, 1), S.Zero) - p0.TC() * tvar + p1.TC(),
                            *symbols), Poly(r1 - tvar * r0, *othersymbols)])
                        unusedindices.remove(i)
                        unusedindices.remove(j)
                        break
            if len(neweqs) >= 8:
                break
            log.warn('allowedindex %d found %d equations where coefficients of equations match', allowedindex,
                     len(neweqs))

        if len(neweqs) < 8:
            raise self.CannotSolveError(
                'found %d equations where coefficients of equations match! need at least 8' % len(neweqs))
        ...
        for polyeq in [polyeqs[ipeq] for ipeq in unusedindices] + AllPolyEquationsExtra:
            ...
            for monoms, c in p0.terms():
                term = c * ((1 - tvar ** 2) ** monoms[0]) * (2 * tvar) ** monoms[1] * (1 + tvar ** 2) ** (
                            maxdenom - monoms[0] - monoms[1])
                eqnew += term
            neweqs.append([Poly(eqnew, *symbols), Poly(p1.as_expr() * (1 + tvar ** 2) ** maxdenom, *othersymbols)])
            neweqs.append(
                [Poly(eqnew * tvar, *symbols), Poly(p1.as_expr() * tvar * (1 + tvar ** 2) ** maxdenom, *othersymbols)])
                    ...
                    for ipeq2, peq2 in enumerate(neweqs):
                        ...
                        for monoms, c in peq2[0].terms():
                            if monoms == monomkey:
                                ...
                                if denom2.is_number and denom3.is_number and abs(denom2.evalf()) > abs(
                                        denom3.evalf()):  # have to select one with least abs value, or else equation will skyrocket
                                    denom2 = denom3
                                    num2 = num3
            ...
            if len(factors) > 1:
                ...
                if peq[0] != S.Zero:
                    ...
                    for test0, test1 in neweqs_full:
                        if (self.equal(test0, peq0norm) and self.equal(test1, peq1norm)) or (
                                self.equal(test0, -peq0norm) and self.equal(test1, -peq1norm)):
                            isunique = False
                            break
                    if isunique:
                ...
                if peq[0] != S.Zero:
                    ...
                    isunique = True
                    for test0, test1 in neweqs_full:
                        if (self.equal(test0, peq[0]) and self.equal(test1, peq[1])) or (
                                self.equal(test0, -peq[0]) and self.equal(test1, -peq[1])):
                            isunique = False
                            break
                    if isunique:
        ...
        neweqs2 = neweqs_simple + [(Poly(peq[0] * tvar, peq[0].gens), Poly(peq[1] * tvar, peq[1].gens)) for peq in
                                   neweqs_simple if not peq[0].has(tvar)]
        ...
        if 1:
            for itest in range(0, len(neweqs_simple) - 1, 2):
                ...
                if neweqs_simple[itest + 1][0] * tvar - neweqs_simple[itest][0] == S.Zero:
                    ...
                    if eq != S.Zero and self.CheckExpressionUnique(reducedeqs, eq):
                        reducedeqs.append(eq)
            for testrational in [Rational(-103651, 500000), Rational(-413850340369, 2000000000000), Rational(151, 500),
                                 Rational(301463, 1000000)]:
                if ((neweqs_simple[0][0] * tvar - neweqs_simple[1][0]) * testrational + neweqs_simple[6][0] * tvar -
                    neweqs_simple[7][0]).expand() == S.Zero:
                    if (neweqs_simple[0][0] * tvar - neweqs_simple[1][0]) == S.Zero:
                        eq = (neweqs_simple[6][1] * tvar - neweqs_simple[7][1]).expand().as_expr()
                    else:
                        eq = ((neweqs_simple[0][1] * tvar - neweqs_simple[1][1]) * testrational + neweqs_simple[6][
                            1] * tvar - neweqs_simple[7][1]).expand().as_expr()
                    if self.CheckExpressionUnique(reducedeqs, eq):
                        reducedeqs.append(eq)
                if ((neweqs_simple[0][0] * tvar - neweqs_simple[1][0]) * testrational + (
                        neweqs_simple[2][0] * tvar - neweqs_simple[3][0]) * sqrt(2)) == S.Zero:
                    if (neweqs_simple[0][0] * tvar - neweqs_simple[1][0]) == S.Zero:
                        eq = ((neweqs_simple[2][1] * tvar - neweqs_simple[3][1]) * sqrt(2)).as_expr()
                    else:
                        eq = ((neweqs_simple[0][1] * tvar - neweqs_simple[1][1]) * testrational + (
                                    neweqs_simple[2][1] * tvar - neweqs_simple[3][1]) * sqrt(2)).as_expr()
                    if self.CheckExpressionUnique(reducedeqs, eq):
                        reducedeqs.append(eq)
        ...
        if len(allmonoms) > len(neweqs_full) and len(reducedeqs) < 3:
            raise self.CannotSolveError(
                'new monoms is %d>%d, reducedeqs=%d' % (len(allmonoms), len(neweqs_full), len(reducedeqs)))
        ...
        if len(allmonoms) < len(neweqs_test):
            ...
            if nummatrixsymbols > 150:
                log.info('found a non-singular matrix with %d symbols, but most likely there is a better one',
                         nummatrixsymbols)
                raise self.CannotSolveError(
                    'matrix has too many symbols (%d), giving up since most likely will freeze' % nummatrixsymbols)
            ...
            if self.IsDeterminantNonZeroByEval(AU):
                ...
                for i in range(1, A.shape[0]):
                    ...
                    if not self.IsDeterminantNonZeroByEval(AUdetmat, len(rows) > 9 and (
                            numaufractions > 120 or numaufractions + numausymbols > 120)):
                        log.info('skipping dependent index %d, numausymbols=%d, numausymbols=%d', i, numaufractions,
                                 numausymbols)
                        continue
                    AU = AU2
                    rows.append(i)
            ...
            if 0:  # self.has(A,*self.freevars):
                ...
                for i in range(AUinv.shape[0]):
                    ...
                    for j in range(AUinv.shape[1]):
                        ...
                        numerator = self.trigsimp(numerator.subs(self.freevarsubsinv), self.freejointvars).subs(
                            self.freevarsubs)
                        numerator, common = self.removecommonexprs(numerator, onlygcd=True, returncommon=True)
                        denominator = self.trigsimp((denominator / common).subs(self.freevarsubsinv),
                                                    self.freejointvars).subs(self.freevarsubs)
                        try:
                        ...
                        if r != S.Zero:
                            ...
                            numerator2, numerator2d, htvarsubsinv = self.ConvertSinCosEquationToHalfTan(
                                (AUdet * numerator).subs(sinsubs).expand().subs(sinsubs).expand().subs(
                                    sinsubs).expand(), self.freejointvars)
                            denominator2, denominator2d, htvarsubsinv = self.ConvertSinCosEquationToHalfTan(
                                denominator.subs(sinsubs).expand().subs(sinsubs).expand().subs(sinsubs).expand(),
                                self.freejointvars)
                            ...
                            if r != S.Zero:
                                log.warn('cannot get rid of denominator for element (%d, %d) in (%s/%s)', i, j,
                        ...
                        AUadjugate[i, j] = self.trigsimp(q.subs(self.freevarsubsinv), self.freejointvars).subs(
                            self.freevarsubs)
        ...
        for iothersymbol, othersymbol in enumerate(othersymbols):
            if othersymbol.name[0] == 'c':
                ...
                htvarsubs += [(othersymbol, (1 - htvar ** 2) / (1 + htvar ** 2)),
                              (othersymbols[iothersymbol + 1], 2 * htvar / (1 + htvar ** 2))]
                htvars.append(htvar)
        ...
        if haszeroequations:
            ...
            for curvar in usedvars[:-1]:
                try:
                    ...
                    treefirst = self.SolveAllEquations(AllEquations, curvars=[curvar],
                                                       othersolvedvars=self.freejointvars, solsubs=self.freevarsubs[:],
                                                       endbranchtree=[AST.SolverSequence([jointtrees2])],
                                                       unknownvars=unknownvars + [tvar], canguessvars=False,
                                                       currentcases=currentcases, currentcasesubs=currentcasesubs)
                    # solvable, which means we now have len(AllEquations)-1 with two variables, solve with half angles
                    halfanglesolution = \
                    self.SolvePairVariablesHalfAngle(raweqns=[eq.subs(curvarsubs) for eq in AllEquations],
                                                     var0=unknownvars[0], var1=unknownvars[1],
                                                     othersolvedvars=self.freejointvars + [curvar])[0]
                    ...
                    halfanglevar = unknownvars[0] if halfanglesolution.jointname == unknownvars[0].name else \
                    unknownvars[1]
                    ...
                    try:
                        ...
                        jointtrees2 += self.SolveAllEquations(AllEquationsOrig, curvars=curvars,
                                                              othersolvedvars=self.freejointvars + [curvar,
                                                                                                    halfanglevar],
                                                              solsubs=self.freevarsubs + curvarsubs + self.Variable(
                                                                  halfanglevar).subs, endbranchtree=endbranchtree,
                                                              canguessvars=False, currentcases=currentcases,
                    ...
                    jointtrees3 = []
                    treesecond = self.SolveAllEquations(AllEquations, curvars=unknownvars,
                                                        othersolvedvars=self.freejointvars + [curvar, halfanglevar],
                                                        solsubs=self.freevarsubs + curvarsubs + self.Variable(
                                                            halfanglevar).subs,
                                                        endbranchtree=[AST.SolverSequence([jointtrees3])],
                                                        canguessvars=False, currentcases=currentcases,
                                                        currentcasesubs=currentcasesubs)
                    for t in treesecond:
                    ...
                    if AUinv is not None:
                        ...
                        for i in [0, 2]:
                            ...
                            try:
                                ...
                                jointtrees3.append(
                                    AST.SolverSolution(jointname, jointeval=[atan2(X[indices[i + 1]], X[indices[i]])],
                        ...
                        jointtrees3.append(AST.SolverCheckZeros('sanitycheck', jointcheckeqs, zerobranch=endbranchtree,
                                                                nonzerobranch=[AST.SolverBreak(
                                                                    'sanitycheck for solveLiWoernleHiller')],
                                                                anycondition=False, thresh=0.001))
            ...
            try:
                ...
                if len(raweqns) > 1:
                    halfanglesolution = \
                    self.SolvePairVariablesHalfAngle(raweqns=raweqns, var0=usedvars[0], var1=usedvars[1],
                                                     othersolvedvars=self.freejointvars)[0]
                    ...
                    nexttree = self.SolveAllEquations(raweqns, curvars=[unknownvar],
                                                      othersolvedvars=self.freejointvars + [halfanglevar],
                                                      solsubs=self.freevarsubs + self.Variable(halfanglevar).subs,
                                                      endbranchtree=[AST.SolverSequence([jointtrees])],
                                                      canguessvars=False, currentcases=currentcases,
                                                      currentcasesubs=currentcasesubs)
                    ...
                    try:
                        finaltree = self.SolveAllEquations(AllEquations, curvars=usedvars[2:],
                                                           othersolvedvars=self.freejointvars + usedvars[0:2],
                                                           solsubs=self.freevarsubs + self.Variable(
                                                               usedvars[0]).subs + self.Variable(usedvars[1]).subs,
                                                           endbranchtree=endbranchtree, canguessvars=False,
                                                           currentcases=currentcases, currentcasesubs=currentcasesubs)
                        jointtrees += finaltree
                        return preprocesssolutiontree + [halfanglesolution] + nexttree, usedvars

                    except self.CannotSolveError as e:
                        log.debug('failed to solve for final variable %s, so returning just two: %s' % (
                        usedvars[2], str(usedvars[0:2])))
                ...
                log.debug('failed solving first two variables pairwise: %s', e)
        ...
        for eq in reducedeqs:
            ...
            if newpeq != S.Zero:
                ...
                hassinglevariable |= any(
                    [all([builtins.sum(monom) == monom[i] for monom in newpeq.monoms()]) for i in range(3)])
        ...
        if hassinglevariable:
            ...
            try:
                solutiontree = self.SolveAllEquations(AllEquations, curvars=solvejointvars,
                                                      othersolvedvars=self.freejointvars[:],
                                                      solsubs=self.freevarsubs[:], endbranchtree=endbranchtree,
                                                      canguessvars=False, currentcases=currentcases,
                                                      currentcasesubs=currentcasesubs)
                ...
                log.debug('failed solving all variables: %s', e)

            try:
                solutiontree = self.SolveAllEquations(AllEquations, curvars=usedvars,
                                                      othersolvedvars=self.freejointvars[:],
                                                      solsubs=self.freevarsubs[:], unknownvars=unusedvars,
                                                      endbranchtree=endbranchtree, canguessvars=False,
                                                      currentcases=currentcases, currentcasesubs=currentcasesubs)
                return solutiontree, usedvars
            except self.CannotSolveError as e:
                log.debug('failed solving used variables: %s', e)
            ...
            for ivar in range(3):
                try:
                    ...
                    endbranchtree2 = []
                    if 1:
                        solutiontree = self.SolveAllEquations(AllEquations, curvars=[usedvars[ivar]],
                                                              othersolvedvars=self.freejointvars[:],
                                                              solsubs=self.freevarsubs[:],
                                                              endbranchtree=[AST.SolverSequence([endbranchtree2])],
                                                              unknownvars=unknownvars + unusedvars, canguessvars=False,
                                                              currentcases=currentcases,
                                                              currentcasesubs=currentcasesubs)
                        endbranchtree2 += self.SolveAllEquations(AllEquations, curvars=unknownvars[0:2],
                                                                 othersolvedvars=self.freejointvars[:] + [
                                                                     usedvars[ivar]],
                                                                 solsubs=self.freevarsubs[:] + self.Variable(
                                                                     usedvars[ivar]).subs, unknownvars=unusedvars,
                                                                 endbranchtree=endbranchtree, canguessvars=False,
                                                                 currentcases=currentcases,
                                                                 currentcasesubs=currentcasesubs)
                    return preprocesssolutiontree + solutiontree, usedvars  # +unusedvars#[unknownvars[1], usedvars[ivar]]#
                except self.CannotSolveError as e:
                    log.debug('single variable %s failed: %s', usedvars[ivar], e)
        ...
        for ileftvar in [0]:  # range(len(htvars)):
            ...
            if len(newreducedeqs) == 3:
                ...
                if exportcoeffeqs is None:
                    filteredeqs = [peq for peq in newreducedeqs if
                                   peq.degree() <= 2]  # has never worked for higher degrees than 2
        ...
        if exportcoeffeqs is None:
                            ...
                            for peq in useequations:
                                ...
                                if complexity < 2000:
                                    ...
                                    for monoms, c in peq.terms():
                                        ...
                                        peqnew += termpoly * (Bpoly ** (monoms[ihtvar]) * Apoly ** (
                                                    maxhtvardegree - monoms[ihtvar]))
                            ...
                            if len(singlepolyequations) > 0:
                                ...
                                for eq in reducedeqs:
                                    ...
                                    if sum(peq.degree_list()) == 0:
                                        ...
                                        neweq1 = S.Zero
                                        for monoms, c in peq.terms():
                                            neweq0 += c * (svar ** monoms[0]) * (
                                                        (1 + cvar) ** (peq.degree(0) - monoms[0]))
                                            neweq1 += c * ((1 - cvar) ** monoms[0]) * (
                                                        svar ** (peq.degree(0) - monoms[0]))
                                ...
                                try:
                                    ...
                                    unusedvars = [solvejointvar for solvejointvar in solvejointvars if
                                                  not solvejointvar in usedvars]
                                    ...
                                    return preprocesssolutiontree + [
                                        firstsolution] + solutiontree, usedvars + unusedvars

                                except self.CannotSolveError as e:
                                    log.debug(
                                        'could not solve full variables from scratch, so use existing solution: %s', e)
                                    ...
                                    return preprocesssolutiontree + [firstsolution, secondsolution,
            ...
            if deg1index is not None:
                ...
                for dialyticeqs in combinations(processedequations, 3):
                    ...
                    firstsolution = AST.SolverPolynomialRoots(jointname=usedvars[ileftvar].name, poly=det,
                                                              jointeval=[jointsol],
                    ...
                    complexity = [(eq.degree(1 - ileftvar) * 100000 + self.codeComplexity(eq.as_expr()), eq) for eq in
                                  processedequations if eq.degree(1 - ileftvar) > 0]
                    complexity.sort(key=itemgetter(0))
        ...
        coupledsolution = AST.SolverCoeffFunction(jointnames=[v.name for v in usedvars],
                                                  ...
                                                  isHinges=[self.IsHinge(v.name) for v in usedvars],
                                                  exportvar=exportvar, exportcoeffeqs=exportcoeffeqs,
                                                  exportfnname=exportfnname, rootmaxdim=16)
        ...
        if 0:
            ...
            if self._iktype == 'transform6d' and possiblevar in rotsymbols and possiblevalue == S.Zero and possiblevar2 in rotsymbols and possiblevalue2 == S.Zero:
                ...
                if row1 == row2:
                    ...
                    checkexpr[2].append((Symbol('%s%d%d' % (possiblevar.name[:-2], row1, col3)) ** 2,
                                         S.One))  # squared in the corner should always be 1
                    checkexpr[2].append((Symbol('%s%d%d' % (possiblevar.name[:-2], row1, col3)) ** 3, Symbol(
                        '%s%d%d' % (possiblevar.name[:-2], row1, col3))))  # squared in the corner should always be 1
                    ...
                    checkexpr[2].append(
                        (rb ** 3, rb - rb * ra ** 2))  # need 3rd power since sympy cannot divide out the square
                    ...
                    checkexpr[2].append((Symbol('%s%d%d' % (possiblevar.name[:-2], row3, col1)) ** 2,
                                         S.One))  # squared in the corner should always be 1
                    checkexpr[2].append((Symbol('%s%d%d' % (possiblevar.name[:-2], row3, col1)) ** 3, Symbol(
                        '%s%d%d' % (possiblevar.name[:-2], row3, col1))))  # squared in the corner should always be 1
                    # furthermore can defer that the left over 4 values are [cos(ang), sin(ang), cos(ang), -sin(ang)] = abcd
                    if col1 == 1:
                        mincol = 0
                        maxcol = 2
    ...
    def solveKohliOsvatic(self, rawpolyeqs, solvejointvars, endbranchtree, AllEquationsExtra=None, currentcases=None,
                          currentcasesubs=None):
        ...
        for jother in range(len(othersymbolsnamesunique)):
            ...
            if len(othersymbols) > 2:
                reducedeqs = self.reduceBothSidesSymbolically(leftsideeqs, rightsideeqs, usesymbols=False,
                                                              maxsymbols=18)
                for peq in reducedeqs:
                    peq[0] = Poly(peq[0], *othersymbols)
            else:
                reducedeqs = [[left, right] for left, right in zip(leftsideeqs, rightsideeqs)]
            if len(reducedeqs) > 0:
                break
        ...
        if len(finaleqs) >= 2:
            ...
            try:
                ...
                reducedeqs2 = self.reduceBothSidesSymbolically(leftsideeqs, rightsideeqs, usesymbols=False,
                                                               maxsymbols=18)
                ...
                if len(AllEquations) > 0:
                    otherjointtrees = []
                    tree = self.SolveAllEquations(AllEquations, curvars=list(usedvars), othersolvedvars=[],
                                                  solsubs=self.freevarsubs,
        ...
        if len(neweqs) != 8:
            raise self.CannotSolveError(
                'coefficients of equations need to match! only got %d reduced equations' % len(neweqs))
        ...
        dummyvars.append(
            (dummyk, othersymbols[1 + 2 * jother] * (1 + dummyvars[1][1] ** 2) * (1 + dummyvars[2][1] ** 2)))
        ...
        coupledsolution = AST.SolverCoeffFunction(jointnames=[v.name for v in usedvars],
                                                  ...
                                                  isHinges=[self.IsHinge(v.name) for v in usedvars],
                                                  exportvar=dummys[0:3] + [dummyjk], exportcoeffeqs=exportcoeffeqs,
                                                  exportfnname='solvedialyticpoly16lep', rootmaxdim=16)
        self.usinglapack = True
    ...
    def solveDialytically(self, dialyticeqs, ileftvar, returnmatrix=False, getsubs=None):
        ...
        if len(allmonoms) == 0 or len(allmonoms) > 2 * len(dialyticeqs):
            raise self.CannotSolveError(
                'solveDialytically: more unknowns than equations %d>%d' % (len(allmonoms), 2 * len(dialyticeqs)))
        ...
        for ipeq, peq in enumerate(dialyticeqs):
            for m, c in peq.terms():
                ...
                exportindex = degree * len(origmonoms) * len(dialyticeqs) + len(origmonoms) * ipeq + origmonoms.index(
                    tuple(mlist))
                ...
                if all([m[igen] == 0 for m in peq.monoms()]):
                    for m, c in peq.terms():
                        ...
                        if tuple(mlist) not in origmonoms:
                            raise self.CannotSolveError(
                                'equations too simple, monom %r is not in %r' % (mlist, origmonoms))

                        exportindex = degree * len(origmonoms) * len(dialyticeqs) + len(
                            origmonoms) * ipeq + origmonoms.index(tuple(mlist))
                        assert (exportcoeffeqs[exportindex] == S.Zero)
                        exportcoeffeqs[exportindex] = c
                        Mall[degree][len(dialyticeqs) + ipeq, allmonoms.index(tuple(mlist))] = c
                        Mallindices[degree][len(dialyticeqs) + ipeq, allmonoms.index(tuple(mlist))] = exportindex
        ...
        if self.testconsistentvalues is not None:
            ...
            for itest, subs in enumerate(self.testconsistentvalues):
                ...
                try:
                    ...
                    log.warn('could not convert to numpy array: %s', e)
                    continue
                ...
                if all([Abs(f) > eps for f in eigenvals]):
                    ...
                    log.info('not all abs(eigenvalues) > %e. min is %e', eps,
                             min([Abs(f) for f in eigenvals if Abs(f) < eps]))
    ...
    def SimplifyTransform(self, eq, othervars=None):
        ...
        transformsubstitutions = [(var, value) for var, value in self.globalsymbols if
                                  var.is_Symbol and not var.name.startswith('gconst')]
    ...
    def _SimplifyRotationNorm(self, eq, groups):
        ...
        for group in groups:
            ...
            if len(listterms) == 1:
                continue
            usedindices = set()
            for index0, index1 in combinations(list(range(len(listterms))), 2):
                    ...
                    for i, j, k in [(0, 1, 2), (0, 2, 1), (1, 2, 0)]:
                        if m0[k] == m1[k]:
                            if m0[i] >= 2 and m1[j] >= 2 and m0[i] == m1[i] + 2 and m0[j] + 2 == m1[j]:
                                p = p + Poly(c0 * (group[3] - group[0] ** 2 - group[1] ** 2 - group[2] ** 2) * (
                                            group[k] ** m0[k]) * (group[i] ** (m0[i] - 2)) * (group[j] ** (m0[j])),
                                             group[0], group[1], group[2])
                                changed = True
                            if m1[i] >= 2 and m0[j] >= 2 and m1[i] == m0[i] + 2 and m1[j] + 2 == m0[j]:
                                p = p + Poly(c0 * (group[3] - group[0] ** 2 - group[1] ** 2 - group[2] ** 2) * (
                                            group[k] ** m1[k]) * (group[i] ** (m1[i] - 2)) * (group[j] ** (m1[j])),
                                             group[0], group[1], group[2])
                                changed = True
                    ...
                    for i, j, k in [(0, 1, 2), (0, 2, 1), (1, 2, 0)]:
                        if m0[k] == m1[k]:
                            if m0[i] == 4 and m1[j] == 4:
                                p = p + Poly(c0 * group[k] ** m0[k] * (
                                            (group[3] - group[k] ** 2) * (group[i] ** 2 - group[j] ** 2) - group[
                                        i] ** 4 + group[j] ** 4), group[0], group[1], group[2])
                                changed = True
                            if m0[j] == 4 and m1[i] == 4:
                                p = p + Poly(c0 * group[k] ** m0[k] * (
                                            (group[3] - group[k] ** 2) * (group[j] ** 2 - group[i] ** 2) - group[
                                        j] ** 4 + group[i] ** 4), group[0], group[1], group[2])
                                changed = True
                            if changed:
                                neweq = p  # .as_expr()
                                eq = neweq
            ...
            for i, j, k in [(0, 1, 2), (0, 2, 1), (1, 2, 0)]:
                for index0, index1 in combinations(list(range(len(listterms))), 2):
                    ...
                    if self.equal(listterms[index0][1], listterms[index1][1]):
                        for (m0, c0), (m1, c1) in [[listterms[index0], listterms[index1]],
                                                   [listterms[index1], listterms[index0]]]:
                            if m0[dg[i][0]] == 1 and m0[dg[i][1]] == 1 and m1[dg[j][0]] == 1 and m1[dg[j][1]] == 1:
                                ...
                                if tuple(m0l) == tuple(m1l):
                                    ...
                                    p = p.sub(Poly.from_dict({m0: c0}, *p.gens)).sub(
                                        Poly.from_dict({m1: c1}, *p.gens)).sub(Poly.from_dict({tuple(m2): c0}, *p.gens))
                                    if dg[3] != S.Zero:
                    ...
                    for (m0, c0), (m1, c1) in [[listterms[index0], listterms[index1]],
                        ...
                        if m0[cg[0][0]] == 1 and m0[cg[0][1]] == 1 and m1[cg[1][0]] == 1 and m1[cg[1][1]] == 1:
                            ...
                            if tuple(m0l) == tuple(m1l):
                                ...
                                p = p.sub(Poly.from_dict({m0: c0}, *p.gens)).sub(Poly.from_dict({m1: c1}, *p.gens)).add(
                                    Poly.from_dict({tuple(m2): c0}, *p.gens))
                                changed = True
                                usedindices.add(index0)
                                usedindices.add(index1)
                                break
    ...
    def verifyAllEquations(self, AllEquations, unsolvedvars, solsubs, tree=None):
        ...
        for i in range(len(AllEquations)):
            ...
            if not expr.has(*unsolvedvars) and self.CheckExpressionUnique(extrazerochecks, expr):
                extrazerochecks.append(
                    self.removecommonexprs(expr.subs(solsubs).evalf(), onlygcd=False, onlynumbers=True))
        if len(extrazerochecks) > 0:
            return [AST.SolverCheckZeros('verify', extrazerochecks, tree, [AST.SolverBreak('verifyAllEquations')],
                                         anycondition=False)]
        return tree
                            ...
                            try:
                                ...
                                if peq.degree(0) == 1:
                                    ...
                                    if constantSymbol in newsubsdict:
                                        if self.codeComplexity(newsolution) < self.codeComplexity(
                                                newsubsdict[constantSymbol]):
    ...
    def SolveAllEquations(self, AllEquations, curvars, othersolvedvars, solsubs, endbranchtree, currentcases=None,
                          unknownvars=None, currentcasesubs=None, canguessvars=True):
        """
        :param canguessvars: if True, can guess the variables given internal conditions are satisified
        """
        self._CheckPreemptFn(progress=0.15 + (0.3 - 0.3 * 100 / (
                    self._scopecounter + 100)))  # go from 0.15 - 0.45. usually scope counters go to several hundred
        if len(curvars) == 0:
        ...
        log.info('depth=%d c=%d, %s %s: cases=%r', len(currentcases) if currentcases is not None else 0,
                 self._scopecounter, othersolvedvars, curvars, currentcases)
        solsubs = solsubs[:]
        freevarinvsubs = [(f[1], f[0]) for f in self.freevarsubs]
        solinvsubs = [(f[1], f[0]) for f in solsubs]
        # single variable solutions
        ...
        for curvar in curvars:
            ...
            raweqns = []
            for e in AllEquations:
                if (len(othervars) == 0 or not e.has(*othervars)) and e.has(curvar, curvarsym.htvar, curvarsym.cvar,
                                                                            curvarsym.svar):
                    eq = e.subs(self.freevarsubs + solsubs)
                    if self.CheckExpressionUnique(raweqns, eq):
                        raweqns.append(eq)
            if len(raweqns) > 0:
                try:
                    rawsolutions = self.solveSingleVariable(self.sortComplexity(raweqns), curvar, othersolvedvars,
        ...
        if any([s[0].numsolutions() == 1 for s in solutions]):
            return self.AddSolution(solutions, AllEquations, curvars, othersolvedvars, solsubs, endbranchtree,
                                    currentcases=currentcases, currentcasesubs=currentcasesubs, unknownvars=unknownvars)
        ...
        if len(curvars) == 2 and self.IsHinge(curvars[0].name) and self.IsHinge(curvars[1].name) and len(
                curvarsubssol) > 0:
            ...
            if len(NewEquations) >= 2:
                ...
                if any([s.numsolutions() == 1 for s in dummysolutions]):
                    ...
                    for dummysolution in dummysolutions:
                        ...
                        newsubs = [(value, sin(dummyvar)) for value in sindummyvarsols] + [(value, cos(dummyvar)) for
                                                                                           value in cosdummyvarsols] + [
                                      (-value, -sin(dummyvar)) for value in sindummyvarsols] + [(-value, -cos(dummyvar))
                                                                                                for value in
                                                                                                cosdummyvarsols]
                        allzeros = True
                    ...
                    if len(solutions) > 0:
                        tree = self.AddSolution(solutions, raweqns, curvars[0:1], othersolvedvars + curvars[1:2],
                                                solsubs + self.Variable(curvars[1]).subs, endbranchtree,
                                                currentcases=currentcases, currentcasesubs=currentcasesubs,
        ...
        for var0, var1, raweqns, complexity in curvarsubssol:
            try:
                rawsolutions = self.SolvePrismaticHingePairVariables(raweqns, var0, var1, othersolvedvars,
                                                                     unknownvars=curvars + unknownvars)
                for solution in rawsolutions:
        ...
        for var0, var1, raweqns, complexity in curvarsubssol:
            ...
            for solution in rawsolutions:
                ...
                try:
                    ...
                    log.warn('equation failed to compute solution complexity: %s', solution.jointeval)
            if len(rawsolutions) > 0:  # solving a pair is rare, so any solution will do
                break
        ...
        if canguessvars and len(othersolvedvars) + len(curvars) == len(self.freejointvars) + len(
                self._solvejointvars) and (len(curvars) == 1 or (
                len(curvars) < len(self._solvejointvars) and currentcases is not None and len(
                currentcases) > 0)):  # only estimate when deep in the hierarchy, do not want the guess to be executed all the time
            ...
            return self.GuessValuesAndSolveEquations(AllEquations, curvars, othersolvedvars, solsubs, endbranchtree,
                                                     currentcases, unknownvars, currentcasesubs)
    ...
    def AddSolution(self, solutions, AllEquations, curvars, othersolvedvars, solsubs, endbranchtree, currentcases=None,
                    currentcasesubs=None, unknownvars=None):
        ...
        for solution in solutions:
            ...
            if len(checkforzeros) == 0 and solution[0].numsolutions() == 1:
                ...
                newvars = curvars[:]
                newvars.remove(var)
                return [solution[0].subs(solsubs)] + self.SolveAllEquations(AllEquations, curvars=newvars,
                                                                            othersolvedvars=othersolvedvars + [var],
                                                                            solsubs=solsubs + self.Variable(var).subs,
                                                                            endbranchtree=endbranchtree,
                                                                            currentcases=currentcases,
                                                                            currentcasesubs=currentcasesubs,
                                                                            unknownvars=unknownvars)
            ...
            if len(usedsolutions) == 0:
                ...
                for usedsolution, usedvar in usedsolutions:
                    if len(solution.checkforzeros) == len(usedsolution.checkforzeros):
                        if not any([self.CheckExpressionUnique(usedsolution.checkforzeros, eq) for eq in
        ...
        for solution, var in usedsolutions[::-1]:
            ...
            for checkzero in solution.checkforzeros:
                if checkzero.has(*allvars):
                    log.info('ignoring special check for zero since it has symbols %s: %s', str(allvars),
                             str(checkzero))
                    continue
                ...
                if checkzeroComplexity > 120:
                    ...
                    if checkzeroComplexity < 500:
                        checkforzeros.append(
                            checkzero)  # self.removecommonexprs(checkzero.evalf(),onlygcd=False,onlynumbers=True))
                else:
                    ...
                    if checkzero2Complexity < 2 * checkzeroComplexity:  # check that with substitutions, things don't get too big
                        ...
                        if checkzero2Complexity < self.codeComplexity(checkzeroeval):
                            ...
                            checkforzeros.append(
                                checkzero.evalf())  # self.removecommonexprs(checkzero.evalf(),onlygcd=False,onlynumbers=True)
                    ...
                    if not checkzero.has(*allothersolvedvars):
                        ...
                        if sumsquaresexprs is not None:
                            ...
                            if len(sumsquaresexprstozero) > 0:
                                localsubstitutioneqs.append([sumsquaresexprstozero, checkzero,
                                                             [(sumsquaresexpr, S.Zero) for sumsquaresexpr in
                                                              sumsquaresexprstozero], []])
                            ...
                            if checksimplezeroexpr.has(othervar, sothervar, cothervar):
                                ...
                                for value in [S.Zero, pi / 2, pi, -pi / 2]:
                                    try:
                                        ...
                                        checkzerosub = checksimplezeroexpr.subs(
                                            [(othervar, value), (sothervar, sin(value).evalf(n=30)),
                                ...
                                try:
                                    ...
                                    ss += self.solveSingleVariable([checksimplezeroexpr.subs(
                                        [(sothervar, sin(othervar)), (cothervar, cos(othervar))])], othervar,
                                                                   othersolvedvars)
                                except PolynomialError:
                                ...
                                for s in ss:
                                    ...
                                    if s.jointeval is not None:
                                        for eq in s.jointeval:
                                            ...
                                            if eq.is_number or (len(currentcases) <= 1 and not eq.has(
                                                    *allothersolvedvars) and self.codeComplexity(eq) < 100):
                                                isimaginary = self.AreAllImaginaryByEval(eq) or eq.evalf().has(I)
                                                # TODO should use the fact that eq is imaginary
                                                if isimaginary:
                                                    log.warn('eq %s is imaginary, but currently do not support this',
                                                             eq)
                                                    continue
                                                ...
                                                if self.CheckExpressionUnique(handledconds, cond):
                                                    if self.IsHinge(othervar.name):
                                                        ...
                                                        evalcond = cond
                                                    localsubstitutioneqs.append([[cond], evalcond, [(sothervar, sineq),
                                                                                                    (sin(othervar),
                                                                                                     sineq),
                                                                                                    (cothervar, coseq),
                                                                                                    (cos(othervar),
                                        ...
                                        for eq in s.jointevalsin:
                                            eq = self.SimplifyAtan2(
                                                self._SubstituteGlobalSymbols(eq, originalGlobalSymbols))
                                            if eq.is_number or (len(currentcases) <= 1 and not eq.has(
                                                    *allothersolvedvars) and self.codeComplexity(eq) < 100):
                                                dictequations = []
                                                ...
                                                isimaginary = self.AreAllImaginaryByEval(eq) or eq.evalf().has(I)
                                                if isimaginary:
                                                    cond = abs(sothervar) + abs((eq ** 2).evalf(n=30)) + abs(
                                                        sign(cothervar) - 1)
                                                else:
                                                    if not eq.is_number and not eq.has(*allothersolvedvars):
                                                        # not dependent on variables, so it could be in the form of atan(px,py), so convert to a global symbol since it never changes
                                                ...
                                                if self.CheckExpressionUnique(handledconds, cond):
                                                    if self.IsHinge(othervar.name):
                                                        ...
                                                        evalcond = cond
                                                    if isimaginary:
                                                        localsubstitutioneqs.append([[cond], evalcond,
                                                                                     [(sothervar, S.Zero),
                                                                                      (sin(othervar), S.Zero),
                                                                                      (cothervar, S.One),
                                                                                      (cos(othervar), S.One),
                                                        ...
                                                        localsubstitutioneqs.append([[cond], evalcond, [(sothervar, eq),
                                                                                                        (sin(othervar),
                                                                                                         eq), (
                                                                                                        cothervar, sqrt(
                                                                                                            1 - eq * eq).evalf(
                                        ...
                                        for eq in s.jointevalcos:
                                            ...
                                            if eq.is_number or (len(currentcases) <= 1 and not eq.has(
                                                ...
                                                isimaginary = self.AreAllImaginaryByEval(eq) or eq.evalf().has(I)
                                                if isimaginary:
                                                    cond = abs(cothervar) + abs((eq ** 2).evalf(n=30)) + abs(
                                                        sign(sothervar) - 1)
                                                else:
                                                    if not eq.is_number and not eq.has(*allothersolvedvars):
                                                        # not dependent on variables, so it could be in the form of atan(px,py), so convert to a global symbol since it never changes
                                                ...
                                                if self.CheckExpressionUnique(handledconds, cond):
                                                    ...
                                                    if isimaginary:
                                                        ...
                                                        localsubstitutioneqs.append([[cond], evalcond, [
                                                            ...
                                                            (cothervar, eq), (cos(othervar), eq),
                                                            (othervar, acos(eq).evalf(n=30))], dictequations])
                                                    handledconds.append(cond)
                                                # cond=othervar+acos(eq).evalf(n=30)
                                                if isimaginary:
                                                    cond = abs(cothervar) + abs((eq ** 2).evalf(n=30)) + abs(
            ...
            if not var in nextsolutions:
                try:
                    ...
                    nextsolutions[var] = self.SolveAllEquations(AllEquations, curvars=newvars,
                                                                othersolvedvars=othersolvedvars + [var],
                                                                solsubs=solsubs + self.Variable(var).subs,
                                                                endbranchtree=endbranchtree, currentcases=currentcases,
                                                                currentcasesubs=currentcasesubs,
                                                                unknownvars=unknownvars)
                finally:
            ...
            if len(checkforzeros) > 0:
                hascheckzeros = True
                solvercheckzeros = AST.SolverCheckZeros(jointname=var.name, jointcheckeqs=checkforzeros,
        ...
        if len(currentcases) >= self.maxcasedepth or (scopecounter > maxlevel2scopecounter and len(currentcases) >= 2):
            log.warn('c=%d, %d levels deep in checking degenerate cases, skipping. curvars=%r, AllEquations=%r',
                     scopecounter, len(currentcases), curvars, AllEquations)
            lastbranch.append(AST.SolverBreak('%d cases reached' % self.maxcasedepth, [
                (var, self.SimplifyAtan2(self._SubstituteGlobalSymbols(eq, originalGlobalSymbols))) for var, eq in
                currentcasesubs], othersolvedvars, solsubs, originalGlobalSymbols, endbranchtree))
            return prevbranch
        ...
        if hascheckzeros:
            ...
            if self._iktype == 'transform6d' or self._iktype == 'rotation3d':
                rotsymbols = set(self.Tee[:3, :3]).union(
                    [Symbol('new_r00'), Symbol('new_r01'), Symbol('new_r02'), Symbol('new_r10'), Symbol('new_r11'),
                     Symbol('new_r12'), Symbol('new_r20'), Symbol('new_r21'), Symbol('new_r22')])
            ...
            for isolution, (solution, var) in enumerate(usedsolutions[::-1]):
                ...
                for checkzero in solution.checkforzeros:
                    if checkzero.has(*allvars):
                        log.info('ignoring special check for zero 2 since it has symbols %s: %s', str(allvars),
                                 str(checkzero))
                        continue
                    ...
                    for othervar in othersolvedvars:
                        ...
                        if checkzero.has(*othervarobj.vars):
                            if not self.IsHinge(othervar.name):
                                ...
                                for value in [S.Zero, pi / 2, pi, -pi / 2]:
                                    possiblesubs.append([(othervar, value), (sothervar, sin(value).evalf(n=30)),
                    ...
                    for ipossiblesub, possiblesub in enumerate(possiblesubs):
                        ...
                        if ishinge[ipossiblesub]:
                            ...
                            evalcond = cond
                        if eq == S.Zero:
                            log.info('c=%d, adding case %s=%s in %s', scopecounter, possiblevar, possiblevalue,
                                     checkzero)
                            ...
                            if possiblevar in rotsymbols and (possiblevalue == S.One or possiblevalue == -S.One):
                                ...
                                col1 = int(possiblevar.name[-1])
                                possiblesub.append(
                                    (Symbol('%s%d%d' % (possiblevar.name[:-2], row1, (col1 + 1) % 3)), S.Zero))
                                possiblesub.append(
                                    (Symbol('%s%d%d' % (possiblevar.name[:-2], row1, (col1 + 2) % 3)), S.Zero))
                                possiblesub.append(
                                    (Symbol('%s%d%d' % (possiblevar.name[:-2], (row1 + 1) % 3, col1)), S.Zero))
                                possiblesub.append(
                                    (Symbol('%s%d%d' % (possiblevar.name[:-2], (row1 + 2) % 3, col1)), S.Zero))
                                ...
                                if self.CheckExpressionUnique(handledconds, cond + cond2):
                                    ...
                                    if self._iktype == 'transform6d' and possiblevar in rotsymbols and possiblevalue == S.Zero and possiblevar2 in rotsymbols and possiblevalue2 == S.Zero:
                                        ...
                                        if row1 == row2:
                                            # (row1, col3) is either 1 or -1, but don't know which.
                                            # know that (row1+1,col3) and (row1+2,col3) are zero though...
                                            checkexpr[2].append((Symbol(
                                                '%s%d%d' % (possiblevar.name[:-2], (row2 + 1) % 3, col3)), S.Zero))
                                            checkexpr[2].append((Symbol(
                                                '%s%d%d' % (possiblevar.name[:-2], (row1 + 2) % 3, col3)), S.Zero))
                                            # furthermore can defer that the left over 4 values are [cos(ang), sin(ang), cos(ang), -sin(ang)] = abcd
                                            if row1 == 1:
                                                minrow = 0
                                                maxrow = 2
                                            ...
                                            rd = Symbol('%s%d%d' % (possiblevar.name[:-2], maxrow, col2))
                                            checkexpr[2].append((rb ** 2, S.One - ra ** 2))
                                            checkexpr[2].append((rb ** 3,
                                                                 rb - rb * ra ** 2))  # need 3rd power since sympy cannot divide out the square
                                            checkexpr[2].append((rc ** 2, S.One - ra ** 2))
                                            # checkexpr[2].append((rc, -rb)) # not true
                                            # checkexpr[2].append((rd, ra)) # not true
                                        elif col1 == col2:
                                            ...
                                            # know that (row3,col1+1) and (row3,col1+2) are zero though...
                                            checkexpr[2].append((Symbol(
                                                '%s%d%d' % (possiblevar.name[:-2], row3, (col1 + 1) % 3)), S.Zero))
                                            checkexpr[2].append((Symbol(
                                                '%s%d%d' % (possiblevar.name[:-2], row3, (col1 + 2) % 3)), S.Zero))
                                            # furthermore can defer that the left over 4 values are [cos(ang), sin(ang), cos(ang), -sin(ang)] = abcd
                                            if col1 == 1:
                                                mincol = 0
                                                maxcol = 2
                                        ...
                                        if not possiblevar in rotsymbols and not possiblevar2 in rotsymbols:
                                            checkexpr = [[cond + cond2], evalcond + evalcond2,
        ...
        for iflatzerosubstitutioneqs, (cond, evalcond, othervarsubs, dictequations) in enumerate(
                flatzerosubstitutioneqs):
            ...
            try:
                ...
                if extrazerochecks is not None:
                    ...
                    if not self.degeneratecases.CheckCases(newcases):
                        log.info('depth=%d, c=%d, iter=%d/%d, starting newcases: %r', len(currentcases), scopecounter,
                                 iflatzerosubstitutioneqs, len(flatzerosubstitutioneqs), newcases)
                        if len(NewEquationsClean) > 0:
                            ...
                            for var, eq in chain(originalGlobalSymbols, dictequations):
                                ...
                                if not self.isValidSolution(neweq):
                                    raise self.CannotSolveError(
                                        'equation %s is invalid because of the following substitutions: %s' % (
                                        eq, othervarsubs))
                            ...
                            newtree = self.SolveAllEquations(NewEquationsClean, curvars, othersolvedvars, solsubs,
                                                             endbranchtree, currentcases=newcases,
                                                             currentcasesubs=newcasesubs, unknownvars=unknownvars)
                            accumequations.append(NewEquationsClean)  # store the equations for debugging purposes
                        else:
                            log.info(
                                'there are no new equations, so most likely the following variables can be freely determined: %r',
                                curvars)
                            ...
                            newtree = []
                            for curvar in curvars:
                                newtree.append(AST.SolverSolution(curvar.name, jointeval=[S.Zero, pi / 2, pi, -pi / 2],
                                                                  isHinge=self.IsHinge(curvar.name)))
                            newtree += endbranchtree
                        zerobranches.append(
                            ([evalcond] + extrazerochecks, newtree, dictequations))  # what about extradictequations?
                        log.info('depth=%d, c=%d, iter=%d/%d, adding newcases: %r', len(currentcases), scopecounter,
                                 iflatzerosubstitutioneqs, len(flatzerosubstitutioneqs), newcases)
        ...
        if len(zerobranches) > 0:
            branchconds = AST.SolverBranchConds(zerobranches + [(None, [AST.SolverBreak('branch miss %r' % curvars, [
                (var, self._SubstituteGlobalSymbols(eq, originalGlobalSymbols)) for var, eq in currentcasesubs],
                                                                                        othersolvedvars, solsubs,
            ...
            lastbranch.append(AST.SolverBreak('no branches %r' % curvars,
                                              [(var, self._SubstituteGlobalSymbols(eq, originalGlobalSymbols)) for
                                               var, eq in currentcasesubs], othersolvedvars, solsubs,
                                              originalGlobalSymbols, endbranchtree))

        return prevbranch

    def GuessValuesAndSolveEquations(self, AllEquations, curvars, othersolvedvars, solsubs, endbranchtree,
                                     currentcases=None, unknownvars=None, currentcasesubs=None):
        # perhaps there's a degree of freedom that is not trivial to compute?
        # take the highest hinge variable and set it
        scopecounter = int(self._scopecounter)
        ...
        if len(hingevariables) > 0 and len(curvars) >= 2:
            ...
            if len(zerovalues) > 0:
                ...
                newtree = []
            if len(newtree) > 0:
                log.warn('c=%d, think there is a free variable, but cannot solve relationship, so setting variable %s',
                         scopecounter, curvar)
                newtree += self.SolveAllEquations(AllEquations, leftovervars, othersolvedvars + [curvar],
                                                  solsubs + self.Variable(curvar).subs, endbranchtree,
                                                  currentcases=currentcases, currentcasesubs=currentcasesubs,
                                                  unknownvars=unknownvars)
                return newtree

        if len(curvars) == 1:
            log.info('have only one variable left %r and most likely it is not in equations %r', curvars[0],
                     AllEquations)
    ...
    def SolvePairVariablesHalfAngle(self, raweqns, var0, var1, othersolvedvars, subs=None):
        ...
        try:
            return self.solveSingleVariable(
                self.sortComplexity([e.as_expr() for e in polyeqs if not e.has(varsym1.htvar)]), varsym0.var,
                othersolvedvars, unknownvars=[])
        except self.CannotSolveError:
            pass
        try:
            return self.solveSingleVariable(
                self.sortComplexity([e.as_expr() for e in polyeqs if not e.has(varsym0.htvar)]), varsym1.var,
                othersolvedvars, unknownvars=[])
        except self.CannotSolveError:
            pass
        ...
        for ileftvar in range(2):
            ...
            for degree in range(mindegree, maxdegree + 1):
                ...
                if degree + 1 <= len(newpolyeqs2):
                    ...
                    for eqsindices in combinations(list(range(len(newpolyeqs2))), degree + 1):
                    ...
                    if len(possibilities) > 0:
                        if len(possibilities) > 1:
                            try:
                                ...
                                for divisor, linearsolution in linearsolutions:
                                    ...
                                    solversolution = AST.SolverSolution(varsyms[ileftvar].name, jointeval=[
                                        2 * atan(linearsolution[0] / divisorsymbol)],
                                                                        isHinge=self.IsHinge(varsyms[ileftvar].name))
                                    prevsolution = AST.SolverCheckZeros(varsyms[ileftvar].name, [divisorsymbol],
                                                                        zerobranch=[prevsolution],
                                                                        nonzerobranch=[solversolution], thresh=1e-6)
                        ...
                        equationdegrees = [(max(peq.degree_list()) * 100000 + self.codeComplexity(peq.as_expr()), peq)
        ...
        if solutions[0] is not None:
            if solutions[1] is not None:
                if max(solutions[1][0].degree_list()) < max(solutions[0][0].degree_list()):
                    pfinals = solutions[1]
                    ileftvar = 1
                elif max(solutions[1][0].degree_list()) == max(solutions[0][0].degree_list()) and self.codeComplexity(
                        solutions[1][0].as_expr()) < self.codeComplexity(solutions[0][0].as_expr()):
                    pfinals = solutions[1]
                    ileftvar = 1
                else:
                    pfinals = solutions[0]
        ...
        if pfinals is None:
            raise self.CannotSolveError(
                'SolvePairVariablesHalfAngle: failed to solve dialytically with %d equations' % (len(polyeqs)))

        jointsol = 2 * atan(varsyms[ileftvar].htvar)
        solution = AST.SolverPolynomialRoots(jointname=varsyms[ileftvar].name, poly=pfinals[0], jointeval=[jointsol],
                                             isHinge=self.IsHinge(varsyms[ileftvar].name))
        solution.checkforzeros = []
                    ...
                    for startrow in range(len(systemequations)):
                        ...
                        for i in range(startrow + 1, len(systemequations)):
                            ...
                            if self.IsDeterminantNonZeroByEval(M2):
                                if complexity < 5000:
                                    ...
                                    log.warn(
                                        'found solution, but matrix is too complex and determinant will most likely freeze (%d)',
                                        complexity)
                        ...
                        if M.shape[0] == M.shape[1]:
                            ...
                            for check in symbolscheck:
                                ...
                                solution.append(
                                    self.trigsimp(value[0].subs(trigsubsinv), othersolvedvars).subs(trigsubs))
    ...
    def solveSingleVariableLinearly(self, raweqns, solvevar, othervars, maxnumeqs=2, douniquecheck=True):
        ...
        for p0, p1 in combinations(eqpolys, 2):
            ...
            p1dict = p1.as_dict()
            M = Matrix(2, 3,
                       [p0dict.get((1, 0), S.Zero), p0dict.get((0, 1), S.Zero), p0.TC(), p1dict.get((1, 0), S.Zero),
                        p1dict.get((0, 1), S.Zero), p1.TC()])
            M = M.subs(othervarsubs).expand()
            partialsolution = [-M[1, 1] * M[0, 2] + M[0, 1] * M[1, 2], M[1, 0] * M[0, 2] - M[0, 0] * M[1, 2],
    ...
    def solveHighDegreeEquationsHalfAngle(self, lineareqs, varsym, subs=None):
        ...
        for peq in polyeqs:
            ...
            if pfinal is not None and pfinal.degree(0) > 0:
                ...
                solution = AST.SolverPolynomialRoots(jointname=varsym.name, poly=pfinal, jointeval=[jointsol],
        ...
        raise self.CannotSolveError(
            'half-angle substitution for joint %s failed, %d equations examined' % (varsym.var, len(polyeqs)))
    ...
    def checkFinalEquation(self, pfinal, subs=None):
        ...
        if pfinal.LC().evalf() == S.Zero or all(
                [pfinal.LC().subs(subs).subs(self.globalsymbols).subs(testconsistentvalue).evalf() == S.Zero for
                 testconsistentvalue in self.testconsistentvalues]):
            return None
        ...
        for testconsistentvalue in self.testconsistentvalues:
            coeffs = []
            globalsymbols = [(s, v.subs(self.globalsymbols).subs(testconsistentvalue).evalf()) for s, v in
                             self.globalsymbols]
            for degree in range(pfinal.degree(0), -1, -1):
                value = pfinaldict.get((degree,), S.Zero).subs(subs).subs(
                    globalsymbols + testconsistentvalue).evalf() / common.evalf()
            ...
            for root in roots:
                if Abs(float(root.imag)) < 10.0 ** -self.precision and Abs(float(root.real) - realsolution) < 10.0 ** -(
                        self.precision - 2):
                    found = True
                    break
            if found:
        ...
        for eq in eqns:
            ...
            if numvar >= 1 and numvar <= 2:
                try:
                    ...
                    jointsolutions = [self.SimplifyTransform(self.trigsimp(s.subs(symbols), othersolvedvars)) for s in
                                      tempsolutions]
                    if all([self.isValidSolution(s) and s != S.Zero for s in jointsolutions]) and len(
                            jointsolutions) > 0:
                        # check if any solutions don't have divide by zero problems
                        returnfirstsolutions.append(
                            AST.SolverSolution(var.name, jointeval=jointsolutions, isHinge=self.IsHinge(var.name)))
                        hasdividebyzero = any(
                            [len(self.checkForDivideByZero(self._SubstituteGlobalSymbols(s))) > 0 for s in
                             jointsolutions])
                    ...
                    for s in tempsolutions:
                        ...
                        try:
                            ...
                            if s4 == 0:
                                ...
                                jointsolutions.append(2 * atan(s4,
                                                               evaluate=False))  # don't evalute since chances if this being a number is very low
                        except RuntimeError as e:
        ...
        if len(eqns) > 1:
            ...
            for e in eqns:
                ...
                try:
                    ...
                    if any([(m[0] > 0) + (m[1] > 0) + (m[2] > 0) > 1 for m in
                            Poly(enew, varsym.cvar, varsym.svar, var).monoms()]):
                        continue
                except PolynomialError:
                    continue
                try:
                    # ignore any equations with degree 3 or more
                    if max(Poly(enew, varsym.svar).degree_list()) > maxdegree or max(
                            Poly(enew, varsym.cvar).degree_list()) > maxdegree:
                        log.debug('ignoring equation: ', enew)
                        continue
                except PolynomialError:
                    continue
                try:
                    if Poly(enew, varsym.svar).TC() == S.Zero or Poly(enew, varsym.cvar) == S.Zero or Poly(enew,
                                                                                                           varsym.var) == S.Zero:
            ...
            for icomb, comb in enumerate(eqcombinations):
                ...
                if s is not None:
                    ...
                    for svarsol, cvarsol in sollist:
                        ...
                        if (svarComplexity < 600 and svarsol.subs(listsymbols).expand() == S.Zero) and (
                                cvarComplexity < 600 and Abs(cvarsol.subs(listsymbols).expand()) - S.One != S.Zero):
                            break
                        if (cvarComplexity < 600 and cvarsol.subs(listsymbols).expand() == S.Zero) and (
                                svarComplexity < 600 and Abs(svarsol.subs(listsymbols).expand()) - S.One != S.Zero):
                            break
                        ...
                        if self.equal(svarfrac[0], cvarfrac[0]) and self.equal(svarfrac[1], cvarfrac[1]):
                            break
                        if not self.isValidSolution(svarfrac[0]) or not self.isValidSolution(
                                svarfrac[1]) or not self.isValidSolution(cvarfrac[0]) or not self.isValidSolution(
                                cvarfrac[1]):
                            continue
                        ...
                        testsuccess = False
                        for testconsistentvalue in self.testconsistentvalues:
                            if all([testeq.subs(self.globalsymbols).subs(testconsistentvalue).evalf() != S.Zero for
                                    testeq in testeqs]):
                                testsuccess = True
                                break
                        if not testsuccess:
                        ...
                        if scomplexity > 1200 or ccomplexity > 1200:
                            log.debug('equation too complex for single variable solution (%d,%d).... (probably wrong?)',
                                      scomplexity, ccomplexity)
                            break
                        if scomplexity < 500 and len(str(svarfrac[
                                                             1])) < 600:  # if fractions are too long, then will take a long time to simplify, so also checkout the length fo the entire equation
                        ...
                        if self.equal(svarfracsimp_denom, cvarfracsimp_denom) and not svarfracsimp_denom.is_number:
                            log.debug('%s solution: denominator is equal %s, doing a global substitution', var.name,
                                      svarfracsimp_denom)
                            ...
                            expandedsol = atan2(svarsolsimp, cvarsolsimp) + pi / 2 * (
                                        -S.One + S.One / sign(svarfracsimp_denom))
                        else:
                            ...
                            if svarsolsimp.is_number and cvarsolsimp.is_number:
                                if Abs(svarsolsimp ** 2 + cvarsolsimp ** 2 - S.One).evalf() > 1e-10:
                                    log.debug('%s solution: atan2(%s,%s), sin/cos not on circle so ignoring', var.name,
                            ...
                            if svarsolsimpcomplexity > 3000 or cvarsolsimpcomplexity > 3000:
                                log.warn('new substituted solutions too complex: %d, %d', svarsolsimpcomplexity,
                        ...
                        if unknownvars is not None:
                            ...
                            if len(solversolution.equationsused) > 0:
                                log.info('%s solution: equations used for atan2: %s', var.name,
                                         str(solversolution.equationsused))
            ...
            if len(
                    solutions) > 0 or hasgoodsolution:  # found a solution without any divides, necessary for pr2 head_torso lookat3d ik
                return solutions
        ...
        for ieq, eq in enumerate(eqns):
            ...
            if unknownvars is not None:
                ...
                for unknownvar in unknownvars:
                    if unknownvar != var:
                        unsolvedsymbols += self.Variable(unknownvar).vars
                if len(unsolvedsymbols) > 0:
                    equationsused = [eq2 for ieq2, eq2 in enumerate(eqns) if
                                     ieq2 != ieq and not eq2.has(*unsolvedsymbols)]
                else:
                    equationsused = eqns[:]
            ...
            if numcvar == 1 and numsvar == 1:
                ...
                if m is not None:
                    ...
                    if not constsol.has(I) and all(
                            [self.isValidSolution(s) and self.isValidSolution(s) for s in jointsolutions]) and len(
                            jointsolutions) > 0:
            ...
            if numcvar > 0:
                try:
                    ...
                    if self.countVariables(eqnew, varsym.svar) <= 1 or (
                            self.countVariables(eqnew, varsym.cvar) <= 2 and self.countVariables(eqnew,
                                                                                                 varsym.svar) == 0):  # anything more than 1 implies quartic equation
                        ...
                        if len(jointsolutions) > 0 and all(
                                [self.isValidSolution(s) and self.isValidSolution(s) for s in jointsolutions]):
                            solutions.append(AST.SolverSolution(var.name, jointevalcos=jointsolutions,
            ...
            if numsvar > 0:
                ...
                try:
                    if self.countVariables(eqnew, varsym.svar) <= 1 or (
                            self.countVariables(eqnew, varsym.svar) <= 2 and self.countVariables(eqnew,
                                                                                                 varsym.cvar) == 0):  # anything more than 1 implies quartic equation
                        tempsolutions = solve(eqnew.subs(varsym.cvar, sqrt(1 - varsym.svar ** 2)).expand(), varsym.svar)
                        jointsolutions = [
                            self.SimplifyTransform(self.trigsimp(s.subs(symbols + varsym.subsinv), othersolvedvars)) for
                            s in tempsolutions]
                        if all([self.isValidSolution(s) and self.isValidSolution(s) for s in jointsolutions]) and len(
                                jointsolutions) > 0:
                            solutions.append(AST.SolverSolution(var.name, jointevalsin=jointsolutions,
                                                                isHinge=self.IsHinge(var.name)))
                            solutions[-1].equationsused = equationsused
                        continue
                except self.CannotSolveError as e:
    ...
    def SolvePrismaticHingePairVariables(self, raweqns, var0, var1, othersolvedvars, unknownvars=None):
        ...
        polyeqs = [Poly(eq.subs(varsubs).subs(reducesubs).expand(), unknownvars) for eq in raweqns if
                   eq.has(prismaticSymbol, hingeSymbol)]
        if len(polyeqs) <= 1:
            raise self.CannotSolveError('not enough equations')
        ...
        for solveSymbol in [hingeSymbol, prismaticSymbol]:
            for solveSymbol2, solvesubs in solvevariables:
                if solveSymbol == solveSymbol2:
                    ...
                    try:
                        rawsolutions = self.solveSingleVariable(reducedeqs, solveSymbol, othersolvedvars,
                                                                unknownvars=unknownvars)
    ...
    def SolvePairVariables(self, raweqns, var0, var1, othersolvedvars, maxcomplexity=50, unknownvars=None):
        ...
        pairwisesubs = [(svar0 * cvar1, Symbol('s0c1')), (svar0 * svar1, Symbol('s0s1')),
                        (cvar0 * cvar1, Symbol('c0c1')), (cvar0 * svar1, Symbol('c0s1')),
                        (cvar0 * svar0, Symbol('s0c0')), (cvar1 * svar1, Symbol('c1s1'))]
        pairwiseinvsubs = [(f[1], f[0]) for f in pairwisesubs]
        pairwisevars = [f[1] for f in pairwisesubs]
        reduceeqns = [Poly(eq.as_expr().subs(pairwisesubs), *pairwisevars) for rank, eq in orgeqns if
                      rank < 4 * maxcomplexity]
        ...
        while len(eqcombs) > 0 and len(neweqns) < 20:
            ...
            for i in range(6):
                ...
                if eq0value != 0 and eq1value != 0:
                    tempeq = (eq0.as_expr() * eq1value - eq0value * eq1.as_expr()).subs(
                        allsymbols + pairwiseinvsubs).expand()
        ...
        if len(polyunknown) == 0:
            ...
            for ivar in range(2):
                ...
                for rank, eq in orgeqns:
                    p = Poly(eq, unknownvars[2 * ivar], unknownvars[2 * ivar + 1])
                    polyunknown.append(Poly(p.subs(unknownvars[2 * ivar + 1] ** 2, 1 - unknownvars[2 * ivar] ** 2),
                                            unknownvars[2 * ivar], unknownvars[2 * ivar + 1]))
                if len(polyunknown) >= 2:
                    ...
                    for curiter in range(2):
                        ...
                        for i in range(len(polyunknown)):
                            eq = (polyunknown[i] * pbasedict.get(monom, S.Zero) - pbase * polyunknown[i].as_dict().get(
                                monom, S.Zero)).as_expr().subs(allsymbols)
        ...
        try:
            rawsolutions += self.solveSingleVariable(self.sortComplexity(
                [e.as_expr().subs(varsubsinv).expand() for score, e in neweqns if not e.has(cvar1, svar1, var1)]), var0,
                                                     othersolvedvars, subs=allsymbols, unknownvars=unknownvars)
        except self.CannotSolveError:
            pass

        try:
            rawsolutions += self.solveSingleVariable(self.sortComplexity(
                [e.as_expr().subs(varsubsinv).expand() for score, e in neweqns if not e.has(cvar0, svar0, var0)]), var1,
                                                     othersolvedvars, subs=allsymbols, unknownvars=unknownvars)
        except self.CannotSolveError:
            pass
        ...
        for i, unknownvar in enumerate(unknownvars):
            ...
            for rank, eq in neweqns:
                ...
                if all([m[i] == 0 or (builtins.sum(m) == m[i] and m[i] > 0) for m in eq.monoms()]) and any(
                        [m[i] > 0 for m in eq.monoms()]):
                ...
                if len(neweqs) > 0:
                    for ivar in range(2):
                        ...
                        for paireq0, paireq1 in combinations(lineareqs, 2):
                            ...
                            eq1 = paireq1[ivar]
                            eq1dict = eq1.as_dict()
                            disc = (eq0dict.get((1, 0), S.Zero) * eq1dict.get((0, 1), S.Zero) - eq0dict.get((0, 1),
                                                                                                            S.Zero) * eq1dict.get(
                                (1, 0), S.Zero)).subs(allsymbols).expand()
                            if disc == S.Zero:
                                continue
                            ...
                            ptotal_cos = Poly(
                                Poly(totaleq, *polysymbols).subs(polysymbols[0] ** 2, 1 - polysymbols[1] ** 2).subs(
                                    polysymbols[1] ** 2, 1 - polysymbols[0] ** 2), *polysymbols)
                            ...
                            ptotalcomplexity = self.codeComplexity(ptotal_cos.as_expr()) + self.codeComplexity(
                                ptotal_sin.as_expr())
                            if ptotalcomplexity < 50000:
                                # log.info('ptotal complexity is %d', ptotalcomplexity)
                                finaleq = (ptotal_cos.as_expr() ** 2 - (
                                            1 - polysymbols[0] ** 2) * ptotal_sin.as_expr() ** 2).expand()
                                # sometimes denominators can accumulate
                                pfinal = Poly(self.removecommonexprs(finaleq, onlygcd=False, onlynumbers=True),
                                              polysymbols[0])
                                ...
                                if pfinal is not None:
                                    ...
                                    solution = AST.SolverPolynomialRoots(jointname=var.name, poly=pfinal,
                                                                         jointeval=[jointsol],
                                                                         isHinge=self.IsHinge(var.name))
        ...
        for i in range(2):
            ...
            if len(terms) > 0:
                simpleterms.append(
                    eqs[i].sub(Poly.from_dict({terms[0][1]: terms[0][0]}, *eqs[i].gens)).as_expr() / terms[0][
                        0])  # divide by the coeff
        ...
        if domagicsquare:
            ...
            finaleq = self.trigsimp(expand(
                ((complexterms[0] ** 2 + complexterms[1] ** 2) - simpleterms[0] ** 2 - simpleterms[1] ** 2).subs(
                    varsubsinv)), othersolvedvars + [var0, var1]).subs(varsubs)
            denoms = [fraction(simpleterms[0])[1], fraction(simpleterms[1])[1], fraction(complexterms[0])[1],
            ...
            if max(p0.degree_list()) > 1 and max(p1.degree_list()) > 1 and max(p0.degree_list()) == max(
                    p1.degree_list()) and p0.LM() == p1.LM():
    ...
    def tolatex(e):
        ...
        s5 = re.sub('sin', 's', s5)
        s5 = re.sub('cos', 'c', s5)
        replacements = [('px', 'p_x'), ('py', 'p_y'), ('pz', 'p_z'), ('r00', 'r_{00}'), ('r01', 'r_{01}'),
                        ('r02', 'r_{02}'), ('r10', 'r_{10}'), ('r11', 'r_{11}'), ('r12', 'r_{12}'), ('r20', 'r_{20}'),
                        ('r21', 'r_{21}'), ('r022', 'r_{22}')]
...
""", version=__version__)
    ...
    parser.add_option('--iktype', action='store', dest='iktype', default='transform6d',
                      help='The iktype to generate the ik for. Possible values are: %s' % (
                          ', '.join(name for name, fn in IKFastSolver.GetSolvers().items())))
    ...
    parser.add_option('--lang', action='store', type='string', dest='lang', default='cpp',
                      help='The language to generate the code in (default=%default), available=(' + ','.join(
                          name for name, value in CodeGenerators.items()) + ')')
