                            ...
                            const size_t ndims, npy_intp const* dims)
{
			...
			if (ndims > 0) {
				if (PyArray_ISFLOAT(pyarrayvalues)) {
					if (typeSize == sizeof(float)) {
						...
						throw OPENRAVE_EXCEPTION_FORMAT(_tr("do not support array object float with %d type size"), typeSize, ORE_InvalidArguments);
					}
				}
                ...
                if( typeSize == sizeof(int8_t) ) {
                    ...
                    if( PyArray_ISSIGNED(pyarrayvalues) ) {
						...
						throw OPENRAVE_EXCEPTION_FORMAT(_tr("do not support array object integer with %d type size"), typeSize, ORE_InvalidArguments);
					}
				}
				else {
					throw OPENRAVE_EXCEPTION_FORMAT(_tr("do not support array object with %d type size"), typeSize, ORE_InvalidArguments);
				}
			}
		}
		else
		{
			throw OPENRAVE_EXCEPTION_FORMAT0(_tr("unsupported python type"), ORE_InvalidArguments);
		}
	}
	...
	object toPyArray(const std::vector<KinBody::GeometryInfoPtr>& infos)
	{
		...
		for (size_t i = 0; i < infos.size(); ++i) {
			if (!infos[i]) {
				throw OPENRAVE_EXCEPTION_FORMAT(_tr("geometryInfo[%d] is invalid"), i, ORE_InvalidArguments);
			}
	...
	protected:
		...
		std::shared_ptr<boost::thread> _threadviewer;
	...
	bool PyEnvironmentBase::CheckCollision(object o1)
	{
		...
		throw OPENRAVE_EXCEPTION_FORMAT0(_tr("CheckCollision(object) invalid argument"), ORE_InvalidArguments);
	}
	...
	bool PyEnvironmentBase::CheckCollision(object o1, PyCollisionReportPtr pReport)
	{
		...
		if (!!plink) {
			...
			if (!!pbody) {
				...
				throw OPENRAVE_EXCEPTION_FORMAT0(_tr("invalid argument"), ORE_InvalidArguments);
			}
		}
		openravepy::UpdateCollisionReport(pReport, shared_from_this());
		return bCollision;
	...
	bool PyEnvironmentBase::CheckCollision(object o1, object o2)
	{
		...
		if (!!plink) {
			...
			throw OPENRAVE_EXCEPTION_FORMAT0(_tr("invalid argument 2"), ORE_InvalidArguments);
		}
		KinBodyConstPtr pbody = openravepy::GetKinBody(o1);
		if (!!pbody) {
			KinBody::LinkConstPtr plink2 = openravepy::GetKinBodyLinkConst(o2);
		...
		throw OPENRAVE_EXCEPTION_FORMAT0(_tr("invalid argument 1"), ORE_InvalidArguments);
	}
	...
	bool PyEnvironmentBase::CheckCollision(object o1, object bodyexcluded, object linkexcluded)
	{
		...
		if (!!preport) {
			throw OPENRAVE_EXCEPTION_FORMAT0(_tr("3rd argument should be linkexcluded, rather than CollisionReport! Try report="), ORE_InvalidArguments);
		}
	...
	bool PyEnvironmentBase::CheckCollision(std::shared_ptr<PyRay> pyray, PyKinBodyPtr pbody)
	{
		return _penv->CheckCollision(pyray->r, KinBodyConstPtr(openravepy::GetKinBody(pbody)));
	}

	bool PyEnvironmentBase::CheckCollision(std::shared_ptr<PyRay> pyray, PyKinBodyPtr pbody, PyCollisionReportPtr pReport)
	{
	...
	bool PyEnvironmentBase::CheckCollision(std::shared_ptr<PyRay> pyray, PyCollisionReportPtr pReport)
	{
	...
	size_t PyEnvironmentBase::_getGraphPoints(object opoints, std::vector<float>&vpoints)
	{
		if (PyObject_HasAttrString(opoints.ptr(), "shape")) {
			...
			case 1:
				...
				if (vpoints.size() % 3) {
					throw OPENRAVE_EXCEPTION_FORMAT(_tr("points have bad size %d"), vpoints.size(), ORE_InvalidArguments);
				}
			...
			case 2: {
				...
				if (dim % 3) {
					throw OPENRAVE_EXCEPTION_FORMAT(_tr("points have bad size %dx%d"), num%dim, ORE_InvalidArguments);
				}
				return num * (dim / 3);
			}
			default:
				throw OpenRAVEException(_tr("points have bad dimension"));
			}
		}
	...
	size_t PyEnvironmentBase::_getGraphColors(object ocolors, std::vector<float>&vcolors)
	{
		if (!IS_PYTHONOBJECT_NONE(ocolors)) {
			if (PyObject_HasAttrString(ocolors.ptr(), "shape")) {
				...
				case 2: {
					...
					if ((colordim != 3) && (colordim != 4)) {
						throw OPENRAVE_EXCEPTION_FORMAT(_tr("colors dim %d needs to be 3 or 4"), colordim, ORE_InvalidArguments);
					}
					vcolors = ExtractArray<float>(ocolors.attr("flat"));
					return numcolors;
				}
				default:
					throw OPENRAVE_EXCEPTION_FORMAT(_tr("colors has %d dimensions"), len(colorshape), ORE_InvalidArguments);
				}
			}
			...
			if (vcolors.size() == 3) {
				...
				throw OPENRAVE_EXCEPTION_FORMAT(_tr("colors has incorrect number of values %d"), vcolors.size(), ORE_InvalidArguments);
			}
			return 1;
		}
		// default
	...
	std::pair<size_t, size_t> PyEnvironmentBase::_getGraphPointsColors(object opoints, object ocolors, std::vector<float>&vpoints, std::vector<float>&vcolors)
	{
		...
		if (numpoints <= 0) {
			throw OpenRAVEException(_tr("points cannot be empty"), ORE_InvalidArguments);
		}
		if ((numcolors > 1) && (numpoints != numcolors)) {
			throw OpenRAVEException(boost::str(boost::format(_tr("number of points (%d) need to match number of colors (%d)")) % numpoints%numcolors));
		}
...
Because race conditions can pop up when trying to lock the openrave environment without releasing the GIL, if lockenv=True is specified, the system can try to safely lock the openrave environment without causing a deadlock with the python GIL and other threads.\n");
...
#ifdef USE_PYBIND11_PYTHON_BINDINGS
		class_<PyInterfaceBase, std::shared_ptr<PyInterfaceBase> >(m, "Interface", DOXY_CLASS(InterfaceBase))
#else
		class_<PyInterfaceBase, std::shared_ptr<PyInterfaceBase> >("Interface", DOXY_CLASS(InterfaceBase), no_init)
#endif
...

	{
		...
		bool (PyEnvironmentBase::*pcolyb)(std::shared_ptr<PyRay>, PyKinBodyPtr) = &PyEnvironmentBase::CheckCollision;
		bool (PyEnvironmentBase::*pcolybr)(std::shared_ptr<PyRay>, PyKinBodyPtr, PyCollisionReportPtr) = &PyEnvironmentBase::CheckCollision;
		bool (PyEnvironmentBase::*pcoly)(std::shared_ptr<PyRay>) = &PyEnvironmentBase::CheckCollision;
		bool (PyEnvironmentBase::*pcolyr)(std::shared_ptr<PyRay>, PyCollisionReportPtr) = &PyEnvironmentBase::CheckCollision;
