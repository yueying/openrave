...
protected:
    ...
    {
        if( samples.size() == 0 ) {
			ArrayFunc::array(boost::python::list(), ArrayFunc::dtype::get_builtin<uint32_t>());
        }
        int dim = _pspacesampler->GetNumberOfValues();
        npy_intp dims[] = { npy_intp(samples.size()/dim), npy_intp(dim) };
...
{
    {
        scope spacesampler = class_<PySpaceSamplerBase, std::shared_ptr<PySpaceSamplerBase>, bases<PyInterfaceBase> >("SpaceSampler", DOXY_CLASS(SpaceSamplerBase), no_init)
