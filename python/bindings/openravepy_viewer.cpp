...
public:
    ...
    object GetCameraImage(int width, int height, object extrinsic, object oKK)
    {
        ...
        if( vKK.size() != 4 ) {
            throw OpenRAVEException(_("KK needs to be of size 4"));
        }
        ...
        if( !_pviewer->GetCameraImage(memory, width,height,RaveTransform<float>(ExtractTransform(extrinsic)), KK) ) {
            throw OpenRAVEException(_("failed to get camera image"));
        }
...
void init_openravepy_viewer()
{
...

    {
        ...
        scope viewer = class_<PyViewerBase, std::shared_ptr<PyViewerBase>, bases<PyInterfaceBase> >("Viewer", DOXY_CLASS(ViewerBase), no_init)
