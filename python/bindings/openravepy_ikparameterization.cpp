...
public:
    ...
    int GetDOF(object o) {
        ...
        extract<std::shared_ptr<PyIkParameterization> > pyikptr(o);
        if( pyikptr.check() ) {
    ...
    object GetConfigurationSpecification(object ointerpolation, const std::string& robotname="", const std::string& manipname="") {
        ...
        extract<std::shared_ptr<PyIkParameterization> > pyikptr(ointerpolation);
        if( pyikptr.check() ) {
            return object(openravepy::toPyConfigurationSpecification(((std::shared_ptr<PyIkParameterization>)pyikptr)->_param.GetConfigurationSpecification(std::string(), robotname, manipname)));
        }
...
void init_openravepy_ikparameterization()
{
...

    {
        ...
        scope ikparameterization = class_<PyIkParameterization, PyIkParameterizationPtr >("IkParameterization", DOXY_CLASS(IkParameterization))
                                   ...
                                   .def(init<std::shared_ptr<PyIkParameterization> >(args("ikparam")))
