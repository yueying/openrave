...
{
    return object(std::shared_ptr<PyRay>(new PyRay(r)));
}

RAY ExtractRay(object o)
{
    extract<std::shared_ptr<PyRay> > pyray(o);
...
{
    extract<std::shared_ptr<PyAABB> > pyaabb(o);
    return ((std::shared_ptr<PyAABB>)pyaabb)->ab;
}

object toPyAABB(const AABB& ab)
{
    return object(std::shared_ptr<PyAABB>(new PyAABB(ab)));
}
...
{
    extract<std::shared_ptr<PyTriMesh> > pytrimesh(o);
    if( pytrimesh.check() ) {
...
{
    return object(std::shared_ptr<PyTriMesh>(new PyTriMesh(mesh)));
}
...
public:
    ...
    object FindCompatibleGroup(const std::string& name, bool exactmatch) const
    {
        ...
        return object(std::shared_ptr<ConfigurationSpecification::Group>(new ConfigurationSpecification::Group(*it)));
    }
...
object RaveGetPluginInfo()
{
    ...
    FOREACH(itplugin, listplugins) {
        plugins.append(boost::python::make_tuple(itplugin->first,object(std::shared_ptr<PyPluginInfo>(new PyPluginInfo(itplugin->second)))));
    }
    return plugins;
}
...
void init_openravepy_global()
{
...

    {
        ...
        {
            scope group = class_<ConfigurationSpecification::Group, std::shared_ptr<ConfigurationSpecification::Group> >("Group",DOXY_CLASS(ConfigurationSpecification::Group))
