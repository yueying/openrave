	...
	bool _AddAndCheck(std::vector<dReal>& v, std::list< vector<dReal> >& listNewNodes, bool bCheckSelfOnly)
	{
		...
		for (size_t i = 0; i < v.size(); ++i) {
			...
			if (v[i] < _parameters->config_lower_limit_vector_[i]) {
				RAVELOG_WARN_FORMAT("env=%d, dof %d does not follow lower limit %f < %f",
					GetEnv()->GetId() % i%v[i] % _parameters->config_lower_limit_vector_[i]);
				v[i] = _parameters->config_lower_limit_vector_[i];
			}
			if (v[i] > _parameters->config_upper_limit_vector_[i]) {
				RAVELOG_WARN_FORMAT("env=%d, dof %d does not follow upper limit %f > %f",
					GetEnv()->GetId() % i%v[i] % _parameters->config_upper_limit_vector_[i]);
