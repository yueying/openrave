...
";
	...
	virtual bool _InitPlan(RobotBasePtr pbase, PlannerParametersPtr params)
	{
		...
		if ((int)params->initial_config_vector_.size() % params->GetDOF()) {
			RAVELOG_ERROR_FORMAT("env=%d, initial config wrong dim: %d %% %d != 0", GetEnv()->GetId()
		...
		for (size_t index = 0; index < params->initial_config_vector_.size(); index += params->GetDOF()) {
			...
			_filterreturn->Clear();
			if (params->CheckPathAllConstraints(vinitialconfig, vinitialconfig, std::vector<dReal>(),
				std::vector<dReal>(), 0, IT_OpenStart, CFO_FillCollisionReport, _filterreturn) != 0)
			{
				RAVELOG_DEBUG_FORMAT("env=%d, initial configuration for rrt does not satisfy constraints: %s",
					GetEnv()->GetId() % _filterreturn->report_.__str__());
				continue;
			}
		...
		while (iter < _parameters->max_iterations_) {
			...
			if (iter > _parameters->max_iterations_) {
				RAVELOG_WARN_FORMAT("env=%d, iterations exceeded %d\n",
					GetEnv()->GetId() % _parameters->max_iterations_);
				break;
			}
