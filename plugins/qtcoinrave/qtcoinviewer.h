...
//#include <QtGui/QGraphicsProxyWidget>
	...
	public:
		...
		enum ToggleEnum {
			TOGGLE_GRID = 0, TOGGLE_PATH, TOGGLE_COLLISION,
		...
		//! the different views
		enum ViewEnum {
			PLAN_VIEW = 0, MAIN_VIEW, CAMERA_VIEW, VIS_VIEW,
	...
	protected:
		...
		{
		public:
			PrivateGraphHandle(std::weak_ptr<QtCoinViewer> wviewer, SoSwitch* handle)
				: _handle(handle), _wviewer(wviewer)
			{
		...
		inline QtCoinViewerPtr shared_viewer() {
			return std::static_pointer_cast<QtCoinViewer>(shared_from_this());
		}
		...
		inline QtCoinViewerConstPtr shared_viewer_const() const {
			return std::static_pointer_cast<QtCoinViewer const>(shared_from_this());
		}
