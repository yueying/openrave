	...
	public:
		...
		int ComputeSolution(const IkParameterization& ikgoal, const RobotBase::Manipulator& manip, std::vector<dReal>& vsolution, bool bIgnoreJointLimits = false)
		{
			...
			if (!manip.GetIkSolver()->Supports(ikgoal.GetType())) {
				throw OPENRAVE_EXCEPTION_FORMAT(_tr("iksolver %s of manipulator %s do not support "), manip.GetIkSolver() % manip.GetName(), ORE_InvalidArguments);
			}
			...
			_J.resize(ikdof, robot->GetActiveDOF());
			...
			T totalerror2 = _ComputeConstraintError(ikpprev, _error, max_iterations_);
			...
			for (iter = 0; iter < max_iterations_; ++iter) {
				...
				T totalerror2 = _ComputeConstraintError(ikpmanip, _error, max_iterations_ - iter);
			...
			if (bSuccess || besterror2 < firsterror2) {
				...
				robot->GetActiveDOFValues(vsolution); // have to re-get the joint values since joint limits are involved
		...
		int ComputeSolutionTranslation(const IkParameterization& ikgoal, const RobotBase::Manipulator& manip, std::vector<dReal>& vsolution, bool bIgnoreJointLimits = false)
		{
			// check if manip's iksolver suppports this ikgoal type
			if (!manip.GetIkSolver()->Supports(ikgoal.GetType())) {
				throw OPENRAVE_EXCEPTION_FORMAT(_tr("iksolver %s of manipulator %s do not support "),
					manip.GetIkSolver() % manip.GetName(), ORE_InvalidArguments);
			}
		...
		virtual T _ComputeConstraintError(const IkParameterization& ikpcur, boost::numeric::ublas::matrix<T>& error, int nMaxIterations, bool bAddRotation = true)
		{
			...
			default:
				throw OPENRAVE_EXCEPTION_FORMAT(_tr("unsupported ikparam %s."), ikpcur.GetName(), ORE_InvalidArguments);
				break;
			};
			return totalerror2;
		}
		...
		virtual void _CalculateJacobian(const RobotBase::Manipulator& manip, const IkParameterization& ikp)
		{
			...
			case IKP_TranslationZAxisAngle4D:
			{
				...
				for (size_t j = 0; j < _viweights.size(); ++j) {
					...
					//const Vector jointAxis = robot->GetJointFromDOFIndex(dof)->GetAxis();
