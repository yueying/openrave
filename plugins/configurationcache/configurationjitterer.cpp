...
";
 ...
 //");
...
#else
        throw OPENRAVE_EXCEPTION_FORMAT0(_tr("cannot set manipulator bias since lapack is not supported"), ORE_CommandNotSupported);
#endif
    }
    ...
    int Sample(std::vector<dReal>& vnewdof, IntervalType interval=IT_Closed)
    {
        ...
        const std::array<dReal, 3> rayincs = {{0.2, 0.5, 0.9}};
