...
public:
	...
	virtual bool Init(RobotBasePtr robot, const std::vector<int>& dof_indices, int nControlTransformation)
	{
		robot_ = robot;
		if (flog.is_open()) 
		{
		...
		if (!!robot)
		{
			...
			if (dof_indices_.size() > 0) {
				...
				_gjointvalues->dof = dof_indices_.size();
				stringstream ss;
				ss << "joint_values " << robot->GetName();
				FOREACHC(it, dof_indices_) {
					ss << " " << *it;
				}
	...
	virtual bool SetDesired(const std::vector<dReal>& values, TransformConstPtr trans)
	{
		if (values.size() != dof_indices_.size()) {
			throw OpenRAVEException(str(boost::format("wrong desired dimensions %d!=%d") % values.size() % dof_indices_.size()), ORE_InvalidArguments);
		}
		_fCommandTime = 0;
		_ptraj.reset();
	...
	virtual void SimulationStep(dReal fTimeElapsed)
	{
		...
		if (!!ptraj) {
			...
			if (_bTrajHasJoints && dof_indices_.size() > 0) {
				vdofvalues.resize(dof_indices_.size());
				_samplespec.ExtractJointValues(vdofvalues.begin(), sampledata.begin(), probot, dof_indices_, 0);
			}
...
private:
	...
	ofstream flog;
	int cmdid;
	bool _bPause, _bIsDone, _bCheckCollision, _bThrowExceptions, is_enable_logging_;
