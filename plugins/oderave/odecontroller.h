...
public:
    ...
    virtual void Reset(int options)
    {
        if( !!_probot ) {
            ...
            if( !!pinfo ) {
                std::shared_ptr<ODESpace> odespace(pinfo->_odespace);
    ...
    virtual ODESpace::KinBodyInfoPtr GetODESpace() {
        return std::dynamic_pointer_cast<ODESpace::KinBodyInfo>(_probot->GetUserData("odephysics"));
    }
    ...
    bool SendCommand(std::ostream& os, std::istream& is)
    {
        ...
        throw openrave_exception(str(boost::format(_("command %s supported"))%cmd),OpenRAVE::ORE_CommandNotSupported);
        return false;
    }
