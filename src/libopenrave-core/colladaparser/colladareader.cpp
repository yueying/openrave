...
namespace OpenRAVE
{
	...
	public:
		ColladaReader(EnvironmentBasePtr penv, bool bResetGlobalDae = true, bool bExtractConnectedBodies = true) : _dom(NULL), environment_(penv), _nGlobalSensorId(0), _nGlobalManipulatorId(0), _nGlobalIndex(0),
		...
		virtual ~ColladaReader() {
			...
			if (_bResetGlobalDae) {
				SetGlobalDAE(std::shared_ptr<DAE>());
			}
#endif
		}
		...
		bool InitFromURI(const std::string& uristr, const AttributesList& atts)
		{
			...
			_dom = daeSafeCast<domCOLLADA>(dae_->open(!uriresolved.empty() ? uriresolved : urioriginal.str()));
			if (!_dom) {
				return false;
			}
			if (!uriresolved.empty()) {
				_mapInverseResolvedURIList.insert(make_pair(uriresolved, daeURI(*dae_, urioriginal.str())));
			}
		...
		{
			instantiated_nodes_map_.clear();
			RAVELOG_VERBOSE(str(boost::format("init COLLADA reader version: %s, namespace: %s\n")
				% COLLADA_VERSION%COLLADA_NAMESPACE));
			dae_ = GetGlobalDAE(false);
			is_skip_geometry_ = false;
			is_read_geometry_groups_ = false;
			openrave_scheme_aliases_vector_.resize(0);
			for (auto itatt : atts)
			{
				if (itatt.first == "skipgeometry")
				{
					is_skip_geometry_ = itatt.second == "true" || itatt.second == "1";
				}
				else if (itatt.first == "prefix")
				{
					prefix_ = itatt.second;
				}
				...
				else if (itatt.first == "openravescheme")
				{
					...
					openrave_scheme_aliases_vector_ = std::vector<std::string>((std::istream_iterator<std::string>(ss)),
				...
				else if (itatt.first == "uripassword") 
				{
					...
					if (passwordindex != std::string::npos)
					{
						...
						boost::trim(name);
						std::string password = itatt.second.substr(passwordindex + 1);
						boost::trim(password);
					}
				}
				...
				{
					std::stringstream ss(itatt.second);
					std::list<std::string> newelts((std::istream_iterator<std::string>(ss)),
						std::istream_iterator<std::string>());
					is_read_geometry_groups_ = std::find(newelts.begin(), newelts.end(), "bind_instance_geometry")
						!= newelts.end();
				}
				else 
				{
			...
			if (openrave_scheme_aliases_vector_.size() == 0) 
			{
				openrave_scheme_aliases_vector_.push_back("openrave");
			}
			for(auto itname: openrave_scheme_aliases_vector_)
			{
		...
		bool Extract()
		{
			...
			if (!!_dom->getAsset()) {
				if (!!_dom->getAsset()->getUp_axis() && !!environment_->GetPhysicsEngine()) {
					float f = -9.7979302;
					if (_dom->getAsset()->getUp_axis()->getValue() == UP_AXIS_X_UP) {
						environment_->GetPhysicsEngine()->SetGravity(Vector(f, 0, 0));
					}
					else if (_dom->getAsset()->getUp_axis()->getValue() == UP_AXIS_Y_UP) {
						environment_->GetPhysicsEngine()->SetGravity(Vector(0, f, 0));
					}
					else if (_dom->getAsset()->getUp_axis()->getValue() == UP_AXIS_Z_UP) {
						environment_->GetPhysicsEngine()->SetGravity(Vector(0, 0, f));
					}
				}
			}
			...
			for (size_t iphysics = 0; iphysics < allscene->getInstance_physics_scene_array().getCount(); ++iphysics) {
				...
				if (!!pscene->getTechnique_common() && !!pscene->getTechnique_common()->getGravity()) {
					...
					if (domgravity.getCount() == 3) {
						if (!!environment_->GetPhysicsEngine()) {
							environment_->GetPhysicsEngine()->SetGravity(Vector(domgravity[0], domgravity[1], domgravity[2]));
						}
					}
				}
			}
			...
			environment_->GetSensors(vsensors);

			std::vector<KinBodyPtr> vbodies;
			environment_->GetBodies(vbodies);
		...
		bool Extract(RobotBasePtr& probot, const std::string& articulatedSystemId = std::string())
		{
			...
			for (size_t iscene = 0; iscene < allscene->getInstance_kinematics_scene_array().getCount(); iscene++) {
				...
				std::shared_ptr<KinematicsSceneBindings> bindings(new KinematicsSceneBindings());
				_ExtractKinematicsVisualBindings(allscene->getInstance_visual_scene(), kiscene, *bindings);
				_ExtractPhysicsBindings(allscene, *bindings);
				for (size_t ias = 0; ias < kscene->getInstance_articulated_system_array().getCount(); ++ias) {
				...
				for (int idof = 0; idof < (*itjoint)->GetDOF(); ++idof) {
					if ((*itjoint)->IsMimic(idof)) {
						for (int ieq = 0; ieq < 3; ++ieq) {
							string neweq;
							utils::SearchAndReplace(neweq, (*itjoint)->mimic_array_[idof]->equations_[ieq], jointnamepairs);
							(*itjoint)->mimic_array_[idof]->equations_[ieq] = neweq;
						}
					}
				}
			}
		...
		bool ExtractArticulatedSystem(KinBodyPtr& pbody, domInstance_articulated_systemRef ias, KinematicsSceneBindings& bindings, std::list<daeElementRef>& listInstanceScope, const std::string& strParentURI = std::string(), const std::string& strParentName = std::string())
		{
			...
			if (!pbody) {
				...
				if (!!pinterface_type) {
					if (pinterface_type->type == "kinbody") {
						pbody = RaveCreateKinBody(environment_, pinterface_type->name);
					}
					else if (pinterface_type->type.size() == 0 || pinterface_type->type == "robot") {
						pbody = RaveCreateRobot(environment_, pinterface_type->name);
					}
					else {
			...
			if (!!articulated_system->getMotion()) {
				...
				if (!pbody) {
					pbody = RaveCreateRobot(environment_, "GenericRobot");
					if (!pbody) {
						pbody = RaveCreateRobot(environment_, "");
					}
					pbody->str_uri_ = struri;
					_mapJointUnits.clear();
					_mapJointSids.clear();
			...
			if (!pkinbody) {
				...
				if (!!pinterface_type) {
					if (pinterface_type->type.size() == 0 || pinterface_type->type == "kinbody") {
						pkinbody = RaveCreateKinBody(environment_, pinterface_type->name);
					}
					else if (pinterface_type->type == "robot") {
						pkinbody = RaveCreateRobot(environment_, pinterface_type->name);
					}
					else {
						RAVELOG_WARN("invalid interface_type\n");
						return false;
				...
				if (!pkinbody) {
					pkinbody = RaveCreateKinBody(environment_, "");
				}
				_mapJointUnits.clear();
				_mapJointSids.clear();
			}
		...
		KinBodyPtr _ExtractKinematicsModel(domNodeRef pdomnode, const KinematicsSceneBindings& bindings, const std::vector<std::string>& vprocessednodes, std::list<daeElementRef>& listInstanceScope)
		{
			...
			_mapJointUnits.clear();
			_mapJointSids.clear();
			KinBodyPtr pkinbody = RaveCreateKinBody(environment_);
			if (pkinbody->str_uri_.size() == 0) {
				pkinbody->str_uri_ = daeURI(*dae_).str();
			}
						...
						if (pequation->getElementName() == string("equation")) {
							...
							try {
								...
								if (equationtype == "position") {
									pjoint->mimic_array_[iaxis]->equations_[0] = eq;
								}
									...
									pjoint->mimic_array_[iaxis]->equations_[1] += str(boost::format("|%s %s ") % pjointtarget->GetName() % eq);
								}
									...
									pjoint->mimic_array_[iaxis]->equations_[2] += str(boost::format("|%s %s ") % pjointtarget->GetName() % eq);
								}
								else {
			...
			if (!pdomlink) {
				...
				for (size_t iatt = 0; iatt < pdomlink->getAttachment_full_array().getCount(); ++iatt) {
					...
					if (pjoint->info_.is_active_) {
						pjoint->joint_index_ = (int)pkinbody->joints_vector_.size();
					...
					if (!!pdomjoint->getName()) {
						...
						pjoint->info_.name_ = str(boost::format("dummy%d") % pjoint->joint_index_);
					}
					...
					RAVELOG_DEBUG(str(boost::format("joint %s (%d:%d)") % pjoint->info_.name_%pjoint->joint_index_%pjoint->dof_index_));
					...
					for (size_t ic = 0; ic < vdomaxes.getCount(); ++ic) {
						...
						std::shared_ptr<uint8_t> is_circular;
		...
		std::pair<SensorBasePtr, daeElementRef> _ExtractCreateSensor(daeElementRef instance_sensor)
		{
			...
			return std::make_pair(RaveCreateSensor(environment_, domsensor->getAttribute("type")), domsensor);
		}
		...
		//            return false;
		//        }
		//        psensor = RaveCreateSensor(environment_, domsensor->getAttribute("type"));
		//        if( !psensor ) {
		//            return false;
		//        }
		//
				...
				for (size_t j = 0; j < domatts.getCount(); ++j) {
					if (std::string(domatts[j].name) == "url") {
						...
						if (itname != mapURLsToNames.end()) {
							...
							atts.emplace_back(domatts[j].name, _MakeFullURI(xsAnyURI(*dae_, domatts[j].value), elt));
						}
					}
					else {
	...
	private:
		...
		domNodeRef _InstantiateNode(daeElementRef pelt)
		{
			...
			instantiated_nodes_map_[newnode] = std::make_pair(inode, idsuffix);
			return newnode;
		}
		...
		void _ExtractKinematicsVisualBindings(domInstance_with_extraRef viscene, domInstance_kinematics_sceneRef kiscene, KinematicsSceneBindings& bindings)
		{
			...
			for (size_t imodel = 0; imodel < kiscene->getBind_kinematics_model_array().getCount(); imodel++) {
				...
				if (!inode) {
					...
					std::map<domNodeRef, std::pair<domInstance_nodeRef, std::string> >::iterator itinode = instantiated_nodes_map_.find(node);
					if (itinode != instantiated_nodes_map_.end()) {
		...
		inline dReal _GetUnitScale(daeElementRef pelt, dReal startscale)
		{
			...
			if (!!pextra && !!pextra->getAsset() && !!pextra->getAsset()->getUnit()) {
				return pextra->getAsset()->getUnit()->getMeter() / environment_->GetUnit().second;
			}
			domAssetRef passet = daeSafeCast<domAsset>(pelt->getChild("asset"));
			if (!!passet && !!passet->getUnit()) {
				return passet->getUnit()->getMeter() / environment_->GetUnit().second;
			}
			if (!!pelt->getParent()) {
		...
		std::vector<std::string> openrave_scheme_aliases_vector_;
		std::map<std::string, daeURI> _mapInverseResolvedURIList; //!< holds a list of inverse resolved relationships file:// -> openrave://
		std::map<domNodeRef, std::pair<domInstance_nodeRef, std::string> > instantiated_nodes_map_; //!< holds a map of the instantiated (cloned) node and the original instance_node elements. Also contains the idsuffix used to instantiate the node.
	...
	bool RaveParseColladaURI(EnvironmentBasePtr penv, RobotBasePtr& probot,
		const std::string& uri, const AttributesList& atts)
	{
		boost::mutex::scoped_lock lock(GetGlobalDAEMutex());
		ColladaReader reader(penv);
