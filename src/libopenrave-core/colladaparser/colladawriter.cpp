...
private:
    ...
    void _WriteDynamicRigidConstraints(domInstance_with_extraRef piscene, const std::list<std::shared_ptr<instance_articulated_system_output> >& listModelDatabase)
    {
        ...
        FOREACHC(itias, listModelDatabase) {
            ...
            std::shared_ptr<instance_physics_model_output> ipmout = (*itias)->ipmout;
    ...
    virtual std::string _GetNodeId(KinBody::LinkConstPtr plink) {
        return str(boost::format("v%d.%s")%_mapBodyIds[plink->GetParent()->GetEnvironmentId()]%_GetNodeSid(plink));
    }
    ...
    virtual void _AssignLinkSids(KinBodyPtr pBody) {
        ...
        int numericSid = 0;
		//char tempSid[strlen("link") + 9] = "link";
        char tempSid[4+9] = "link"; // temp memory space for converting indices to hex strings, enough space to convert "link" + uint32_t
        FOREACHC(itLink, vLinks) {
    ...
    virtual void _AssignJointSids(KinBodyPtr pBody) {
        ...
        // for the rest, find next available sid
        int numericSid = 0;
		//char tempSid[strlen("joint") + 9] = "joint";
        char tempSid[5+9] = "joint"; // temp memory space for converting indices to hex strings, enough space to convert "joint" + uint32_t
        FOREACHC(itJoint, vJoints) {
