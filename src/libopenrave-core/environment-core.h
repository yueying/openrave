...
//                }
    ...
    virtual void ExtractInfo(EnvironmentBaseInfo& info)
    {
        ...
        for(size_t i = 0; i < info._vBodyInfos.size(); ++i) {
            if (vBodies[i]->IsRobot()) {
                ...
                RaveInterfaceCast<RobotBase>(vBodies[i])->ExtractInfo(*(std::dynamic_pointer_cast<RobotBase::RobotBaseInfo>(info._vBodyInfos[i])));
            } else {
    ...
    virtual void UpdateFromInfo(const EnvironmentBaseInfo& info, std::vector<KinBodyPtr>& vCreatedBodies, std::vector<KinBodyPtr>& vModifiedBodies, std::vector<KinBodyPtr>& vRemovedBodies)
    {
        ...
        for(int bodyIndex = 0; bodyIndex < (int)info._vBodyInfos.size(); ++bodyIndex) {
            ...
            RobotBase::RobotBaseInfoConstPtr pRobotBaseInfo = std::dynamic_pointer_cast<const RobotBase::RobotBaseInfo>(pKinBodyInfo);
            ...
            if (!!pInitBody) {
                ...
                FOREACHC(it, pKinBodyInfo->_dofValues) {
...
protected:
    ...
    std::shared_ptr<rapidjson::MemoryPoolAllocator<> > _prLoadEnvAlloc; ///< allocator used for loading environments
