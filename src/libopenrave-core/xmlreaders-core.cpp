    ...
    virtual bool endElement(const std::string& xmlname)
    {
        ...
        if( !!_pcurreader ) {
            ...
            case KinBody::JointSpherical:
                ...
                break;
            default:
                throw OpenRAVEException(str(boost::format(_("bad joint type: 0x%x"))%_pjoint->_info._type));
                break;
            }
        }
...
{
public:
    InterfaceXMLReader(EnvironmentBasePtr penv, InterfaceBasePtr& pinterface, 
		InterfaceType type, const std::string &xmltag, const AttributesList &atts)
