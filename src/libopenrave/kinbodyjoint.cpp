...
void KinBody::Joint::SetMimicEquations(int iaxis, const std::string& poseq, const std::string& veleq, const std::string& acceleq)
{
    ...
    for(int itype = 1; itype < 3; ++itype) {
        ...
        /*
            extract from `eq` the partial derivative formulas 鈭倆/鈭倄i for joint z:=z(x1,x2,...xn) defined in `poseq`.
            `eq` takes form
         ...
         |x1 formula_鈭倆_鈭倄2
            ...
            where xi's are joints on which z depends, and ci are common subexpressions in formulas 鈭倆/鈭倄i.
        ...
        while(index != std::string::npos) {
            ...
            std::string sequation; ///< takes form "|xi formula_鈭倆_鈭倄i"
                                               ...
                                               std::map< std::pair<Mimic::DOFFormat, int>, dReal >& mTotalderivativepairValue) const
{
    ...
    for(size_t ivar = 0; ivar < nvars; ++ivar) {
        ...
        const OpenRAVEFunctionParserRealPtr velfn = pmimic->_velfns.at(ivar); ///< function that evaluates the partial derivative 鈭倆/鈭倄
        const dReal fvel = velfn->Eval(vDependedJointValues.empty() ? NULL : &vDependedJointValues[0]); ///< value of 鈭倆/鈭倄

        if( IS_DEBUGLEVEL(Level_Verbose) ) {
            RAVELOG_VERBOSE_FORMAT("鈭J%d)/鈭J%d) = 鈭%s)/鈭%s) = %.8e", thisdofformat.jointindex % jointindex % this->GetName() % dependedjoint->GetName() % fvel);
        }
        ...
        if(dependedjoint->IsMimic(dofformat.axis)) {
            ...
            dependedjoint->_ComputePartialVelocities(vLocalIndexPartialPairs, iaxis, mTotalderivativepairValue); ///< recursion: computes 鈭倅/鈭倄
            ...
            ///< dz/dx += 鈭倆/鈭倄, as z may depend on others who depend on x
            if( localmap.count(key) ) {
                localmap[key] += fvel;
            }
            else {
