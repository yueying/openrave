...
namespace OpenRAVE {
	...
	RobotBase::ConnectedBodyInfo::ConnectedBodyInfo() : is_active_(0)
	{
	}
	...
	void RobotBase::ConnectedBodyInfo::SerializeJSON(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator, dReal unit_scale, int options) const
	{
		...
		openravejson::SetJsonValueByKey(value, "isActive", (int)is_active_, allocator);
	}
	...
	bool RobotBase::ConnectedBody::SetActive(int8_t active)
	{
		...
		if (!!pattachedrobot)
		{
			...
			{
				throw OPENRAVE_EXCEPTION_FORMAT("Cannot set ConnectedBody %s active to %s since robot %s is still in the environment", info_.name_ % (int)active%pattachedrobot->GetName(), ORE_InvalidState);
			}
		}
	...
	{
		if (info_.manipulator_infos_vector_.size() == 0) {
			return false;
		}
		...
		FOREACH(itmanip, info_.manipulator_infos_vector_) {
			const RobotBase::ManipulatorInfo& manipinfo = **itmanip;
			if ((int)manipinfo.name_.size() == nCheckNameLength
				&& strncmp(manipinfo.name_.c_str(), pStartCheckName, nCheckNameLength) == 0) {
				return true;
			}
		}
	...
	void RobotBase::_ComputeConnectedBodiesInformation()
	{
		...
		FOREACH(itconnectedBody, connected_bodies_vector_) {
			...
			for (int imanipulator = 0; imanipulator < (int)connectedBodyInfo.manipulator_infos_vector_.size(); ++imanipulator) {
				...
				if (pnewmanipulator->info_._grippername.size() > 0) {
					pnewmanipulator->info_._grippername = connectedBody._nameprefix + pnewmanipulator->info_._grippername;
				}
