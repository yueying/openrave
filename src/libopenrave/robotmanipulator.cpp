...
RobotBase::Manipulator::Manipulator(RobotBasePtr probot, const RobotBase::ManipulatorInfo& info) : _info(info), robot_(probot) {
}
...
RobotBase::Manipulator::Manipulator(const RobotBase::Manipulator& r)
{
    *this = r;
    ...
    if( _info.ik_solver_xml_id_.size() > 0 ) {
        __pIkSolver = RaveCreateIkSolver(GetRobot()->GetEnv(), _info.ik_solver_xml_id_);
    }
}
...
//    if( _info.ik_solver_xml_id_.size() > 0 ) {
//        //__pIkSolver = RaveCreateIkSolver(probot->GetEnv(), _info.ik_solver_xml_id_);
...
{
    if( !!__pIkSolver || _info.ik_solver_xml_id_.size() == 0 ) {
        return __pIkSolver;
    }
...
bool RobotBase::Manipulator::SetIkSolver(IkSolverBasePtr iksolver)
{
    ...
    if( iksolver == __pIkSolver && _info.ik_solver_xml_id_ == iksolver->GetXMLId() ) {
        return true;
    }
...
{
    RobotBasePtr probot(robot_);
    probot->ComputeJacobianTranslation(effector_link_->GetIndex(), effector_link_->GetTransform() * _info._tLocalTool.trans, jacobian, __varmdofindices);
}
...
void RobotBase::Manipulator::CalculateJacobian(boost::multi_array<dReal,2>& mjacobian) const
{
    ...
    probot->ComputeJacobianTranslation(effector_link_->GetIndex(), effector_link_->GetTransform() * _info._tLocalTool.trans, vjacobian, __varmdofindices);
...
{
    RobotBasePtr probot(robot_);
    probot->ComputeJacobianAxisAngle(effector_link_->GetIndex(), jacobian, __varmdofindices);
}
...
void RobotBase::Manipulator::CalculateAngularVelocityJacobian(boost::multi_array<dReal,2>& mjacobian) const
{
    ...
    RobotBasePtr probot(robot_);
    std::vector<dReal> vjacobian;
    probot->ComputeJacobianAxisAngle(effector_link_->GetIndex(), vjacobian, __varmdofindices);
    OPENRAVE_ASSERT_OP(vjacobian.size(),==,3*__varmdofindices.size());
    vector<dReal>::const_iterator itsrc = vjacobian.begin();
    FOREACH(itdst,mjacobian) {
        std::copy(itsrc,itsrc+__varmdofindices.size(),itdst->begin());
