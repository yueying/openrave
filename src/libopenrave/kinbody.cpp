...
namespace OpenRAVE
{
	...
	public:
		ChangeCallbackData(int properties, const boost::function<void()>& callback, KinBodyConstPtr pbody)
			: _properties(properties), _callback(callback), _pweakbody(pbody) 
		{
		}
		...
		std::list< std::pair<uint32_t, std::list<UserDataWeakPtr>::iterator> > _iterators;
		int _properties;
	...
	KinBody::KinBodyStateSaver::KinBodyStateSaver(KinBodyPtr pbody, int options)
		: _options(options), _pbody(pbody), _bRestoreOnDestructor(true)
	{
	...
	void KinBody::Destroy()
	{
		...
		if (attached_bodies_list_.size() > 0) 
		{
			...
			std::stringstream ss; 
			ss << GetName() << " still has attached bodies: ";
	...
	void KinBody::SetLinkGeometriesFromGroup(const std::string& geomname)
	{
		...
		FOREACHC(itlink, links_vector_) {
			...
			if (geomname.size() == 0) {
				...
				std::map< std::string, std::vector<KinBody::GeometryInfoPtr> >::iterator it = (*itlink)->_info.extra_geometries_map_.find(geomname);
				if (it == (*itlink)->_info.extra_geometries_map_.end()) {
	...
	void KinBody::SetLinkGroupGeometries(const std::string& geomname, const std::vector< std::vector<KinBody::GeometryInfoPtr> >& linkgeometries)
	{
		...
		FOREACH(itlink, links_vector_) {
			Link& link = **itlink;
			std::map< std::string, std::vector<KinBody::GeometryInfoPtr> >::iterator it = link._info.extra_geometries_map_.insert(make_pair(geomname, std::vector<KinBody::GeometryInfoPtr>())).first;
	...
	bool KinBody::Init(const std::vector<KinBody::LinkInfoConstPtr>& linkinfos, const std::vector<KinBody::JointInfoConstPtr>& jointinfos, const std::string& uri)
	{
		...
		FOREACHC(itlinkinfo, linkinfos) {
			...
			plink->_index = static_cast<int>(links_vector_.size());
	...
	uint64_t KinBody::GetLinkEnableStatesMask() const
	{
		...
		for (size_t ilink = 0; ilink < links_vector_.size(); ++ilink) {
			linkstate |= ((uint64_t)links_vector_[ilink]->_info._bIsEnabled << ilink);
		}
		return linkstate;
	}
	...
	void KinBody::SetLinkEnableStates(const std::vector<uint8_t>& enablestates)
	{
		...
		for (size_t ilink = 0; ilink < enablestates.size(); ++ilink) {
			...
			if (links_vector_[ilink]->_info._bIsEnabled != bEnable) {
				links_vector_[ilink]->_info._bIsEnabled = bEnable;
	...
	void KinBody::ComputeInverseDynamics(std::vector<dReal>& doftorques, const std::vector<dReal>& vDOFAccelerations, const KinBody::ForceTorqueMap& mapExternalForceTorque) const
	{
		...
		for (size_t i = 0; i < vLinkVelocities.size(); ++i) {
			Vector vglobalcomfromlink = links_vector_.at(i)->GetGlobalCOM() - links_vector_.at(i)->_info._t.trans;
	...
	void KinBody::ComputeInverseDynamics(boost::array< std::vector<dReal>, 3>& vDOFTorqueComponents, const std::vector<dReal>& vDOFAccelerations, const KinBody::ForceTorqueMap& mapExternalForceTorque) const
	{
		...
		if (bHasBaseLinkAccel || bHasGravity) {
			if (bHasBaseLinkAccel) {
				...
				Vector vbasepos = links_vector_.at(0)->_info._t.trans;
				for (size_t i = 1; i < vLinkVelocities[0].size(); ++i) {
					Vector voffset = links_vector_.at(i)->_info._t.trans - vbasepos;
	...
	void KinBody::_ComputeLinkAccelerations(const std::vector<dReal>& vDOFVelocities, const std::vector<dReal>& vDOFAccelerations, const std::vector< std::pair<Vector, Vector> >& vLinkVelocities, std::vector<std::pair<Vector, Vector> >& vLinkAccelerations, AccelerationMapConstPtr pexternalaccelerations) const
	{
		...
		for (size_t ijoint = 0; ijoint < _vTopologicallySortedJointsAll.size(); ++ijoint) {
			...
			Vector xyzdelta = tchild.trans - links_vector_.at(parentindex)->_info._t.trans;
			if (!!pdofaccelerations || !!pdofvelocities) {
				tdelta = links_vector_.at(parentindex)->_info._t * pjoint->GetInternalHierarchyLeftTransform();
	...
	void KinBody::_ComputeInternalInformation()
	{
		...
		_vJointsAffectingLinks.resize(_vecjoints.size()*links_vector_.size());

		// compute the all-pairs shortest paths
		{
			_vAllPairsShortestPaths.resize(links_vector_.size()*links_vector_.size());
				...
				for (size_t i = 0; i < links_vector_.size(); ++i) {
					...
					for (size_t j = 0; j < links_vector_.size(); ++j) {
						...
						if (vcosts[j*links_vector_.size() + i] > kcost) {
							...
							_vAllPairsShortestPaths[j*links_vector_.size() + i] = _vAllPairsShortestPaths[k*links_vector_.size() + i];
						}
					}
				}
			}
		...
		if ((links_vector_.size() > 0) && (_vecjoints.size() > 0)) {
				...
				if (!(*itjoint)->GetFirstAttached() || (*itjoint)->GetFirstAttached()->IsStatic()) {
					...
					if (vlinkdepths[plink0->GetIndex()] < vlinkdepths[plink1->GetIndex()]) {
						...
						FOREACHC(itparentlink, plink0->_vParentLinks) {
							int jointindex = _vAllPairsShortestPaths[plink0->GetIndex()*links_vector_.size() + *itparentlink].second;
						...
						FOREACHC(itparentlink, plink1->_vParentLinks) {
							int jointindex = _vAllPairsShortestPaths[plink1->GetIndex()*links_vector_.size() + *itparentlink].second;
			...
			for (int i = 0; i < (int)links_vector_.size(); ++i) {
				...
				for (int j = 0; j < (int)links_vector_.size(); ++j) {
					if (vusedlinks[j] && (i != j)) {
						int jointindex = _vAllPairsShortestPaths[i*links_vector_.size() + j].second;
						...
						if (jointindex < (int)_vecjoints.size()) {
							_vJointsAffectingLinks[jointindex*links_vector_.size() + i] = pjoint->GetHierarchyParentLink()->GetIndex() == i ? -1 : 1;
						}
						if (pjoint->IsMimic()) {
							for (int idof = 0; idof < pjoint->GetDOF(); ++idof) {
								if (pjoint->IsMimic(idof)) {
									FOREACHC(itmimicdof, pjoint->_vmimic[idof]->_vmimicdofs) {
										...
										_vJointsAffectingLinks[pjoint2->GetJointIndex()*links_vector_.size() + i] = pjoint2->GetHierarchyParentLink()->GetIndex() == i ? -1 : 1;
									}
								}
							}
						}
			...
			FOREACH(itclosedloop, closedloops) {
				...
				FOREACH(itlinkindex, *itclosedloop) {
					...
					_vClosedLoops.back().push_back(make_pair(links_vector_.at(*itlinkindex), JointPtr()));
				}
				// fill the joints
				for (size_t i = 0; i < _vClosedLoopIndices.back().size(); ++i) {
					...
					int jointindex = _vAllPairsShortestPaths[nextlink*links_vector_.size() + _vClosedLoopIndices.back()[i].first].second;
		...
		FOREACH(itlink, links_vector_) {
			if ((*itlink)->_info.extra_geometries_map_.find(selfgroup) == (*itlink)->_info.extra_geometries_map_.end()) {
				...
				(*itlink)->_info.extra_geometries_map_.insert(make_pair(selfgroup, vgeoms));
			}
		}
	...
	bool KinBody::_RemoveAttachedBody(KinBodyPtr pbody)
	{
		...
		FOREACH(it, pbody->attached_bodies_list_) {
			if (it->lock().get() == this) { // need to compare lock pointer since cannot rely on shared_kinbody() since in a destructor this will crash
				pbody->attached_bodies_list_.erase(it);
				numremoved++;
				break;
			}
		}
	...
	int8_t KinBody::DoesAffect(int jointindex, int linkindex) const
	{
		...
		OPENRAVE_ASSERT_FORMAT(linkindex >= 0 && linkindex < (int)links_vector_.size(), "body %s linkindex %d invalid (num links %d)", GetName() % linkindex%links_vector_.size(), ORE_InvalidArguments);
		return _vJointsAffectingLinks.at(jointindex*links_vector_.size() + linkindex);
	}
	...
	void KinBody::Clone(InterfaceBaseConstPtr preference, int cloningoptions)
	{
		...
		FOREACHC(itjoint, r->_vecjoints) {
			...
			*pnewjoint = **itjoint; // be careful of copying pointers!
			...
			pnewjoint->_attachedbodies[0] = links_vector_.at((*itjoint)->_attachedbodies[0]->GetIndex());
			pnewjoint->_attachedbodies[1] = links_vector_.at((*itjoint)->_attachedbodies[1]->GetIndex());
		...
		FOREACHC(itloop, _vClosedLoops) {
			...
			FOREACHC(it, *itloop) {
				_vClosedLoops.back().push_back(make_pair(links_vector_.at(it->first->GetIndex()), JointPtr()));
