...
namespace OpenRAVE
{
	...
	public:
		...
		public:
			...
			virtual bool IsParentLink(std::shared_ptr<Link const> plink) const;
			...
			virtual bool IsRigidlyAttached(std::shared_ptr<Link const> plink) const;
		...
		public:
			...
			struct DOFFormat
			{
				...
				bool operator ==(const DOFFormat& r) const;
				std::shared_ptr<Joint> GetJoint(KinBodyPtr parent) const;
				std::shared_ptr<Joint const> GetJoint(KinBodyConstPtr parent) const;
			};
			struct DOFHierarchy
			{
		...
		virtual void GetDOFValues(std::vector<dReal>& v, 
		...
		///
		/// \param dofindices the dof indices to return the values for. If empty, will compute for all the dofs
		virtual void GetDOFVelocities(std::vector<dReal>& v, 
			const std::vector<int>& dofindices = std::vector<int>()) const;
    ...
    virtual void SetDOFHardJerkLimits(const std::vector<dReal>& maxlimits);
		...
		/** \en \brief set the transform of the first link (the rest of the links are computed based on the joint values).
			...
			\ja \brief 鑳翠綋銇刀瀵惧Э鍕倰瑷畾銆佹畫銈娿伄銉兂銈伅閬嬪嫊瀛︺伄妲嬮€犮伀寰撱仯銇﹀鎻涖仌銈屻倠锛

			\param transform 澶夋彌琛屽垪
		 */
		virtual void SetTransform(const Transform& transform);
	...
	protected:
		...
		std::vector<int8_t> _vJointsAffectingLinks; //!< joint x link: (jointindex*links_vector_.size()+linkindex). entry is non-zero if the joint affects the link in the forward kinematics. If negative, the partial derivative of ds/dtheta should be negated.
