 ...
 */
	...
	public:
		 ...
		 */
		virtual bool ExtractTransform(Transform& t, std::vector<dReal>::const_iterator itdata,
			KinBodyConstPtr pbody, int timederivative = 0) const;
		...
		/** \brief extracts an ikparameterization given the start of a configuration space point
			...
			\return true if at least one group was found for extracting
		 */
		virtual bool ExtractIkParameterization(IkParameterization& ikparam,
			std::vector<dReal>::const_iterator itdata, int timederivative = 0,
		...
		virtual bool ExtractAffineValues(std::vector<dReal>::iterator itvalues,
			...
			int affinedofs, int timederivative = 0) const;
		...
		virtual bool ExtractJointValues(std::vector<dReal>::iterator itvalues,
			std::vector<dReal>::const_iterator itdata, KinBodyConstPtr pbody,
			const std::vector<int>& indices, int timederivative = 0) const;
		 ...
		 */
		virtual bool InsertJointValues(std::vector<dReal>::iterator itdata,
			std::vector<dReal>::const_iterator itvalues, KinBodyConstPtr pbody,
			const std::vector<int>& indices, int timederivative = 0) const;
		...
		/** \brief Adds a new group to the specification and returns its new offset.
			...
			If the new group's semantic name does not exist in the current specification, adds it and returns the new offset.
			If the new group's semantic name exists in the current specification and it exactly matches,
			then function returns the old group's index. If the semantic names match, but parameters do not match,
			then an OpenRAVEException is thrown.
			This method is not responsible for merging semantic information
		 */
		...
		/** \brief removes all groups that match a name
			...
			\param exactmatch if true, will remove groups only if the full name matches,
			otherwise will remove groups that start with groupname
		 ...
		 */
		virtual void ExtractUsedIndices(KinBodyPtr body,
			std::vector<int>& useddofindices, std::vector<int>& usedconfigindices) const;
		...
		static void ConvertGroupData(std::vector<dReal>::iterator ittargetdata,
			...
			size_t sourcestride, const Group& gsource, size_t numpoints,
			EnvironmentBaseConstPtr penv, bool filluninitialized = true);
		...
		static void ConvertData(std::vector<dReal>::iterator ittargetdata,
			...
			const ConfigurationSpecification& sourcespec, size_t numpoints,
			EnvironmentBaseConstPtr penv, bool filluninitialized = true);
